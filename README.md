...
| Functions with universal theorems | 8 + 3 + 3 (`advance`, `emit`, 4× flow open/close, `pushSequenceIndent`, `pushMappingIndent`, `consumeNewline`, `saveSimpleKey`, `insertAt`, `scanKey` pre-advance — all with full `WellFormed` preservation) |
| Functions with `#guard`-only coverage | ~14 + 2 (`unwindIndents`, `skipSpaces`, `skipWhitespace`, `skipToEndOfLine`, `advanceN`, `scanBlockScalar`, `scanKey`, `scanValue`, etc.) |
| Functions with zero proof coverage | ~26 (`scanNextToken`, all scalar scanners, all document functions) |
| `WellFormed` conjuncts | 4: `indents.size ≥ 1`, `flowLevel = flowStack.size`, `simpleKeyStack.size = flowStack.size`, `offset ≤ inputEnd` |
| Scanner proof theorems | 234 |
| Scanner `#guard` checks | 615 |
