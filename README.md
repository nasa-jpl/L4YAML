...
| Functions with `#guard`-only coverage | ~14 + 2 + 8 + 5 (`unwindIndents`, `skipSpaces`, `skipWhitespace`, `skipToEndOfLine`, `advanceN`, `scanBlockScalar`, `scanKey`, `scanValue`, `processEscape`, `foldQuotedNewlines`, `scanDoubleQuoted`, `scanSingleQuoted`, `scanPlainScalar`, `scanBlockScalar`, `scanAnchorOrAlias`, `scanTag`, `scanDirective`, `scanDocumentStart`, `scanDocumentEnd` etc.) |
| Functions with zero proof coverage | ~13 (`scanNextToken`, remaining dispatch helpers) |
| `WellFormed` conjuncts | 4: `indents.size ≥ 1`, `flowLevel = flowStack.size`, `simpleKeyStack.size = flowStack.size`, `offset ≤ inputEnd` |
| Scanner proof theorems | 252 |
| Scanner `#guard` checks | 871 |
