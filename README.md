...
| **P10.10g** | 3–5 days | **Universal progress theorem.** Prove `∀ s s', s.WellFormed → scanNextToken s = .ok (some s') → s'.offset > s.offset`. This requires showing that every dispatch branch of `scanNextToken` advances the offset by at least one byte. Strategy: (1) prove `advance` strictly increases offset when `offset < inputEnd` (extend `advance_offset_le` from ScannerLoopInvariant to a strict inequality `advance_offset_lt`), (2) prove each sub-scanner (`skipToContent`, `scanPlainScalar`, `scanDoubleQuoted`, `scanSingleQuoted`, `scanBlockScalar`, `scanAnchorOrAlias`, `scanTag`, `scanDirective`, `scanDocumentStart`, `scanDocumentEnd`, `scanFlowSequenceStart`/`End`, `scanFlowMappingStart`/`End`, `scanFlowEntry`, `scanBlockEntry`, `scanKey`, `scanValue`) returns with `offset > input_offset` on success, (3) compose via the dispatch match. The corollary `scan_terminates` (fuel `(input.utf8ByteSize + 1) * 4` suffices for any input) follows from progress + offset boundedness. P10.10f §11 already validates the property on concrete states via 22 `#guard` checks. |
