...
| **3.3** | **Convert Group B self-recursive parsers (~32) to `def`** — Each needs `termination_by Stream.remaining s` and a proof that `Stream.remaining` strictly decreases across recursive calls. Subgroups: Combinators (2: `skipBlankLines`, `checkContinuation`), Scalar (9: `foldQuotedNewlines`, `doubleQuotedScalar`, `singleQuotedScalar`, `plainScalarContent`, etc.), Flow (7: `flowWhitespace`, `flowValue`↔`flowSequence`↔`flowMapping` mutual recursion), Block (11: `blockValue`↔`blockSequence`↔`blockMapping` mutual recursion), Document (3: `directives`, `document`, `yamlStream`). The `next_decreasing` lemma (proved in `Termination.lean`) provides the core decreasing argument. | ← **NEXT** |
...
3. **Step 3.3 — Convert Group B self-recursive parsers (~32)** ← **NEXT**: Add `termination_by Stream.remaining s` and prove `Stream.remaining` strictly decreases across recursive calls. Subgroups: Combinators (2), Scalar (9), Flow (7, mutual recursion), Block (11, mutual recursion), Document (3). The `next_decreasing` lemma provides the core decreasing argument.
