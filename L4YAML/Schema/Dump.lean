...
Wraps the value in a `YamlDocument` and delegates to `L4YAML.Dump.dumpDocument`.
...
def dumpTypedDocument {α : Type} [ToYaml α]
  ...
  L4YAML.Dump.dumpDocument { value := toYaml value, directives } cfg
...
def dumpTypedDocuments {α : Type} [ToYaml α]
  ...
  L4YAML.Dump.dumpDocuments docs cfg
