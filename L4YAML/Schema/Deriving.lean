...
structure Config where
  ...
  deriving L4YAML.Schema.FromYaml, L4YAML.Schema.ToYaml
...
def mkFromYamlEnumInstance (declName : Name) (ctors : Array Name) : CommandElabM Bool := do
  ...
  let mut code := s!"instance : L4YAML.Schema.FromYaml {declName} where\n"
  code := code ++ "  fromYaml? v := do\n"
  code := code ++ "    let str ← L4YAML.Schema.getString v\n"
  ...
  code := code ++ s!"    | other => .error (L4YAML.SchemaError.unknownVariant other \"{declShort}\")\n"
...
def mkFromYamlInstanceHandler (declNames : Array Name) : CommandElabM Bool := do
  ...
  if let some info := getStructureInfo? env declName then
    ...
    let mut code := s!"instance : L4YAML.Schema.FromYaml {declName} where\n"
    code := code ++ "  fromYaml? v := do\n"
    code := code ++ "    let pairs ← L4YAML.Schema.getMapping v\n"
    ...
    for fieldName in fields do
      ...
      if isOpt then
        code := code ++ s!"    let {fieldName} ← L4YAML.Schema.getFieldOpt pairs \"{fieldNameStr}\"\n"
      else
        code := code ++ s!"    let {fieldName} ← L4YAML.Schema.getField pairs \"{fieldNameStr}\"\n"
...
initialize
  registerDerivingHandler ``L4YAML.Schema.FromYaml mkFromYamlInstanceHandler
...
def mkToYamlEnumInstance (declName : Name) (ctors : Array Name) : CommandElabM Bool := do
  ...
  let mut code := s!"instance : L4YAML.Schema.ToYaml {declName} where\n"
  code := code ++ "  toYaml v :=\n"
  ...
  code := code ++ "    L4YAML.YamlValue.scalar {\n"
  code := code ++ "      content := str,\n"
  code := code ++ "      style := L4YAML.ScalarStyle.plain,\n"
  code := code ++ "      tag := none\n"
  code := code ++ "    }\n"
...
def mkToYamlInstanceHandler (declNames : Array Name) : CommandElabM Bool := do
  ...
  if let some info := getStructureInfo? env declName then
    ...
    let mut code := s!"instance : L4YAML.Schema.ToYaml {declName} where\n"
    code := code ++ "  toYaml cfg := L4YAML.Schema.mkMapping [\n"

    for i in [:fields.size] do
      let fieldName := fields[i]!
      let fieldNameStr := fieldName.toString
      code := code ++ s!"    (\"{fieldNameStr}\", L4YAML.Schema.toYaml cfg.{fieldName})"
      if i < fields.size - 1 then
        code := code ++ ",\n"
      else
        code := code ++ "\n"
...
initialize
  registerDerivingHandler ``L4YAML.Schema.ToYaml mkToYamlInstanceHandler
