...
/-! # `IndexedWfa` — Phase 3 Step 6d.2 indexed WFA proofs (staging)
...
Indexed twin of `L4YAML/Proofs/Parser/ParserWfaProofs.lean`: the
`WellFormedAnchors` preservation proofs reparented onto
`ParseStateIx input` and the indexed `parseNode` / `parseDocument`
/ `parseStreamIx` family in `L4YAML.TokenParser.Indexed`.

Architecture mirrors the legacy file (strong induction on fuel),
substituting state type and consuming the indexed WellBehaved
substrate (`parseNode_wb_all_ix`, `parseNodeContent_wb_ix`,
`parseNodeProperties_tokens_ix`) directly. Only `parseNodeProperties`
needs a new `_anchors_eq_ix` companion since the legacy proof was
written for `ParseState`-shaped tokens; the body is a near-verbatim
copy with `ParseStateIx.advance` substitutions.

## Phase 3 Step 6f cutover

At cutover, this file is renamed to `Proofs/Parser/ParserWfaProofs.lean`
(overwriting the legacy file) and the namespace
`L4YAML.Proofs.Indexed.WfaProofs` reverts to
`L4YAML.Proofs.ParserWfaProofs`.
-/

set_option autoImplicit false

namespace L4YAML.Proofs.Indexed.WfaProofs
...
def ParseNodeWFAIx (tokens : Indexed.TokenStream input) (n : Nat) : Prop :=
...
theorem addAnchor_wfa (ps : ParseStateIx input) (name : String) (val : YamlValue)
...
theorem applyNodeFinalization_wfa
    (val : YamlValue) (ps : ParseStateIx input) (props : NodeProperties) (nodeStartPos : YamlPos)
    (h_wfa : WellFormedAnchors ps.anchors)
    (h_scannable : Scannable val false)
    (h_aar : AllAliasesResolve val ps.anchors) :
...
-- lemmas (from ParseNodeWBIx and advance/tryConsume).

-- Helper: advance preserves anchors and tokens.
theorem advance_anchors_ix (ps : ParseStateIx input) :
    ps.advance.anchors = ps.anchors := by simp [ParseStateIx.advance]

theorem advance_tokens_wfa_ix (ps : ParseStateIx input) :
    ps.advance.tokens = ps.tokens := by simp [ParseStateIx.advance]

-- Helper: tryConsume preserves anchors and tokens.
theorem tc_anchors_ix (ps : ParseStateIx input) (tok : YamlToken) :
    (ps.tryConsume tok).2.anchors = ps.anchors := by
  unfold ParseStateIx.tryConsume; split <;> (try split) <;> simp [ParseStateIx.advance]

theorem tc_tokens_wfa_ix (ps : ParseStateIx input) (tok : YamlToken) :
    (ps.tryConsume tok).2.tokens = ps.tokens := by
  unfold ParseStateIx.tryConsume; split <;> (try split) <;> simp [ParseStateIx.advance]

-- Get tokens preservation from ParseNodeWBIx.
theorem parseNode_tokens_of_wb_ix
...
theorem parseBlockSequenceLoop_wfa
    {tokens : Indexed.TokenStream input} {n : Nat}
    (h_ih : ParseNodeWFAIx tokens n) (h_wb : ParseNodeWBIx tokens n)
  ...
  | succ k ih_fuel =>
    ...
    · -- peek? = some .blockEntry
      have h_wfa_adv : WellFormedAnchors ps.advance.anchors :=
        advance_anchors_ix ps ▸ h_wfa
      have h_tok_adv : ps.advance.tokens = tokens :=
        (advance_tokens_wfa_ix ps).trans h_tok
      split at h_ok
      ...
      · generalize h_ps_a : ps.advance = ps_a at h_ok
        ...
        · rename_i pn_res heq_pn
          ...
          have h_tok_n := parseNode_tokens_of_wb_ix h_wb ps_wf k (by omega) h_tok_wf _ _ heq_pn
  ...
  · rename_i k
    ...
    · rename_i loop_res heq_loop
      ...
      have h_wfa_loop := parseBlockSequenceLoop_wfa h_ih h_wb
        ps.advance k #[] (by omega) ((advance_tokens_wfa_ix ps).trans h_tok)
        items ps_loop heq_loop (advance_anchors_ix ps ▸ h_wfa)
      split at h_ok <;> (
        simp only [Except.ok.injEq] at h_ok
        obtain ⟨_, rfl⟩ := Prod.mk.inj h_ok)
      · rw [advance_anchors_ix]; exact h_wfa_loop
      · exact h_wfa_loop
...
theorem parseBlockMappingEntryValue_wfa
  ...
  have h_tc_wfa : WellFormedAnchors (ps.tryConsume .value).2.anchors := by
    rw [tc_anchors_ix]; exact h_wfa
  have h_tc_tok : (ps.tryConsume .value).2.tokens = tokens := by
    rw [tc_tokens_wfa_ix]; exact h_tok
  split at h_ok
  · -- consumed = true
...
theorem parseExplicitKey_tok
  ...
  all_goals (first
    ...
    | exact parseNode_tokens_of_wb_ix h_wb ps fuel h_fuel h_tok _ _ h_ok)
...
theorem parseBlockMappingEntryValue_tok
  ...
  unfold parseBlockMappingEntryValue at h_ok
  simp only [bind, Except.bind, pure, Except.pure] at h_ok
  have h_tc_tok : (ps.tryConsume .value).2.tokens = tokens := (tc_tokens_wfa_ix _ _).trans h_tok
  split at h_ok
  · all_goals (first | (split at h_ok <;> first | contradiction | skip) | skip)
    all_goals (first | (split at h_ok <;> first | contradiction | skip) | skip)
    all_goals (first | (split at h_ok <;> first | contradiction | skip) | skip)
...
theorem handleBlockMappingValueEntry_tok
  ...
  · split at h_ok
    ...
    · rename_i pn_res heq_pn; obtain ⟨v, ps_n⟩ := pn_res
      ...
      obtain ⟨_, rfl⟩ := Prod.mk.inj h_ok
      dsimp only []
      exact parseNode_tokens_of_wb_ix h_wb _ fuel h_fuel
...
theorem pn_tok_pair
  ...
  obtain ⟨v, ps'⟩ := result
  exact parseNode_tokens_of_wb_ix h_wb ps fuel h_fuel h_tok v ps' h_ok
...
theorem parseFlowMappingValue_tok
  ...
  have h_tc2 : (({ ps with currentPath := savedPath.push (.key keyContent) }.tryConsume .key).2.tryConsume .value).2.tokens
      = tokens := by rw [tc_tokens_wfa_ix]; exact h_tc1
  split at h_ok <;> first | contradiction | skip
  all_goals (first | (split at h_ok <;> first | contradiction | skip) | skip)
  all_goals (first | (split at h_ok <;> first | contradiction | skip) | skip)
  all_goals (first | (split at h_ok <;> first | contradiction | skip) | skip)
  ...
  all_goals (
    dsimp only []
    exact parseNode_tokens_of_wb_ix h_wb _ fuel h_fuel h_tc2 _ _ (by assumption))
...
theorem parseSinglePairMapping_tok
  ...
  · rename_i k
    ...
    all_goals (first
      | exact h_tok_adv
      | exact (tc_tokens_wfa_ix _ _).trans h_tok_adv
      | (refine pn_tok_pair h_wb ?_ k (by omega) ?pntok ?_ ?pnok
         case pnok => assumption
         case pntok => exact (tc_tokens_wfa_ix _ _).trans h_tok_adv)
      | (refine pn_tok_pair h_wb ?_ k (by omega) ?pntok ?_ ?pnok
         case pnok => assumption
         case pntok =>
           refine (tc_tokens_wfa_ix _ _).trans ?_
  ...
  · rename_i k
    ...
    · rename_i loop_res heq_loop
      ...
      have h_wfa_loop := parseBlockMappingLoop_wfa h_ih h_wb ps.advance k #[] (by omega)
          ((advance_tokens_wfa_ix ps).trans h_tok) pairs ps_loop heq_loop (advance_anchors_ix ps ▸ h_wfa)
      split at h_ok <;> (
        simp only [Except.ok.injEq] at h_ok
        obtain ⟨_, rfl⟩ := Prod.mk.inj h_ok)
      · rw [advance_anchors_ix]; exact h_wfa_loop
...
theorem parseSinglePairMapping_wfa
  ...
  · rename_i k
    ...
    all_goals (first
      | exact h_wfa_adv
      | (rw [tc_anchors_ix]; exact h_wfa_adv)
      | (refine pn_wfa_pair h_ih ?_ k (by omega) ?pntok ?_ ?pnok ?pnwfa
         case pnok => assumption
         case pntok => exact (tc_tokens_wfa_ix _ _).trans h_tok_adv
         case pnwfa => rw [tc_anchors_ix]; exact h_wfa_adv)
      | (refine pn_wfa_pair h_ih ?_ k (by omega) ?pntok ?_ ?pnok ?pnwfa
         case pnok => assumption
         case pntok =>
           refine (tc_tokens_wfa_ix _ _).trans ?_
    ...
    next => -- other
      ...
      · -- items.size > 0
        ...
        next => -- flowEntry → advance then dispatch
          ...
          next => -- flowSequenceEnd
            simp only [Except.ok.injEq] at h_ok
            obtain ⟨_, rfl⟩ := Prod.mk.inj h_ok; exact advance_anchors_ix ps ▸ h_wfa
          next => -- other → parseNode
            ...
            next pn_res heq_pn =>
              ...
              have h_wfa3 := h_ih ({ ps.advance with currentPath := _ }) k v ps3 (by omega)
                ...
                (by dsimp only []; exact h_wfa_adv)
              have h_tok3 := parseNode_tokens_of_wb_ix h_wb
                ({ ps.advance with currentPath := _ }) k (by omega)
      ...
      · -- items.size = 0
        ...
        next => -- key → parseSinglePairMapping
          ...
          next spm_res heq_spm =>
            obtain ⟨mv, ps3⟩ := spm_res; dsimp only [] at h_ok
            let ps_wf : ParseStateIx input := { ps with currentPath := ps.currentPath.push (.index items.size) }
        ...
        next => -- other → parseNode
          ...
          next pn_res heq_pn =>
            ...
            have h_tok3 : ps3.tokens = tokens :=
              parseNode_tokens_of_wb_ix h_wb ps_wf k (by omega) h_tok v ps3 heq_pn
            generalize h_ps_r : { ps3 with currentPath := _ } = ps_r at h_ok
            exact ih_fuel ps_r _ (by omega)
              (by subst h_ps_r; dsimp only []; exact h_tok3) h_ok
              (by subst h_ps_r; dsimp only []; exact h_wfa3)
  ...
  · rename_i k
    ...
    · rename_i loop_res heq_loop
      ...
      have h_wfa_loop := parseFlowSequenceLoop_wfa h_ih h_wb ps.advance k #[] (by omega)
          ((advance_tokens_wfa_ix ps).trans h_tok) items ps_loop heq_loop (advance_anchors_ix ps ▸ h_wfa)
      split at h_ok
      · simp only [Except.ok.injEq] at h_ok; obtain ⟨_, rfl⟩ := Prod.mk.inj h_ok
        rw [advance_anchors_ix]; exact h_wfa_loop
      · simp at h_ok
        ...
        next => -- flowEntry → advance then dispatch
          ...
          next => -- key → advance → parseExplicitKey → FMV → recurse
            have h_wfa_adv2 : WellFormedAnchors ps.advance.advance.anchors := by
              rw [advance_anchors_ix]; exact h_wfa_adv
            have h_tok_adv2 : ps.advance.advance.tokens = tokens :=
              (advance_tokens_wfa_ix ps.advance).trans h_tok_adv
            split at h_ok
            next => simp at h_ok
            next ek_res heq_ek =>
          ...
          next => -- other → parseNode → FMV → recurse
            ...
            next pn_res heq_pn =>
              ...
              have h_tok_pn := parseNode_tokens_of_wb_ix h_wb ps.advance k (by omega) h_tok_adv
                  key_v ps_pn heq_pn
              split at h_ok
              next => simp at h_ok
              next fmv_res heq_fmv =>
      ...
      · -- pairs.size = 0
        ...
        next => -- other → parseNode → FMV → recurse
          ...
          next pn_res heq_pn =>
            ...
            have h_tok_pn := parseNode_tokens_of_wb_ix h_wb ps k (by omega) h_tok key_v ps_pn heq_pn
            split at h_ok
            next => simp at h_ok
            next fmv_res heq_fmv =>
              obtain ⟨val_v, ps_fmv⟩ := fmv_res; dsimp only [] at h_ok
...
theorem parseNodeProperties_anchors_eq_ix
  ...
  -- Same loop unrolling as parseNodeProperties_tokens_ix (IndexedWellBehaved.lean)
  ...
  · simp only [Except.ok.injEq, Prod.mk.injEq] at h
    ...
    · rename_i v heq_first
      ...
      | done x =>
        ...
        all_goals (first | rfl | simp [ParseStateIx.advance])
      | yield x =>
        simp (config := { iota := true }) only [] at heq
        split at heq
        · contradiction
...
theorem parseNode_wfa_all
  ...
  | succ n ih =>
    ...
    | succ k =>
      ...
      have h_pnwb : ParseNodeWBIx tokens n := parseNode_wb_all_ix tokens h_fpsv h_matched n
      ...
      · -- Non-alias: properties → validate → content → finalize
        ...
        · rename_i props_res heq_props; obtain ⟨props, ps_props⟩ := props_res
          ...
          have h_tok_props : ps_props.tokens = tokens :=
            (parseNodeProperties_tokens_ix ps props ps_props heq_props).trans h_tok
          have h_anch_props := parseNodeProperties_anchors_eq_ix ps props ps_props heq_props
...
theorem parseDirectives_anchors_ix (ps : ParseStateIx input) :
  ...
  suffices h : ∀ (acc : ParseStateIx input × Array Directive),
      ...
      (do
          ...
          pure (__s.snd, __s.fst)).snd.anchors = ps.anchors by
    exact h (ps, #[]) rfl
  intro acc h_inv
  ...
  | cons _ xs ih =>
    ...
    · rename_i _ heq
      ...
      · intro heq
        ...
        subst this; simp [ParseStateIx.advance, h_inv]
      · intro heq
        have := ForInStep.yield.inj heq
        subst this; simp [ParseStateIx.advance, h_inv]
      · intro heq; contradiction

theorem prepareDocumentState_anchors_eq_ix
  ...
  have h_anch :
    ...
    calc
      ...
      _ = ps.anchors := parseDirectives_anchors_ix ps
...
theorem prepareDocumentState_tokens_eq_ix
    (ps : ParseStateIx input) (dirs : Array Directive) (ps' : ParseStateIx input)
    (h : prepareDocumentState ps = .ok (dirs, ps')) :
    ps'.tokens = ps.tokens :=
  prepareDocumentState_tokens_preserved_ix ps dirs ps' h
...
theorem parseDocument_wfa
  ...
  · rename_i prep_result h_prep
    ...
    have h_prep_tok : ps1.tokens = tokens :=
      (prepareDocumentState_tokens_eq_ix ps dirs ps1 h_prep).trans h_tok
    have h_prep_anch : ps1.anchors = ps.anchors :=
      prepareDocumentState_anchors_eq_ix ps dirs ps1 h_prep
...
-- ============================================================
-- §7  parseStreamIx produces documents with WFA anchors
-- ============================================================

-- parseDocument preserves tokens (for stream loop token threading).
theorem parseDocument_tokens_eq_ix
    (tokens : Indexed.TokenStream input)
    (h_fpsv : FlowAwarePSVIx tokens)
    (h_matched : FlowBracketsMatchedIx tokens)
    (ps : ParseStateIx input) (doc : YamlDocument) (ps' : ParseStateIx input)
    ...
    (h_ok : parseDocument ps = .ok (doc, ps')) :
    ps'.tokens = ps.tokens :=
  parseDocument_tokens_preserved_ix ps doc ps' (h_tok ▸ h_fpsv) (h_tok ▸ h_matched) h_ok

-- expect preserves tokens.
theorem expect_tokens_wfa_ix (ps : ParseStateIx input) (tok : YamlToken) (msg : String)
  ...
  · -- some case: if t = tok then ok advance else error
    split at h
    · simp only [Except.ok.injEq] at h; subst h; simp [ParseStateIx.advance]
    · simp at h
      ...
      · -- valid token → parseDocument
        ...
        | ok val =>
          ...
          have h_tok_doc : ps_doc.tokens = tokens :=
            (parseDocument_tokens_eq_ix tokens h_fpsv h_matched ps doc_new ps_doc h_tok h_pd).trans h_tok
...
-- The final theorem: parseStreamIx output has well-formed anchors.
theorem parseStreamIx_output_anchors_wellformed
    (tokens : Indexed.TokenStream input)
  ...
  · rename_i ps_init heq_expect
    have h_tok_init : ps_init.tokens = tokens :=
      expect_tokens_wfa_ix { tokens := tokens, trackPositions := false } .streamStart _ ps_init heq_expect
    have h_wfa_init : WellFormedAnchors ps_init.anchors := by
      have : ps_init.anchors = #[] := by
        unfold ParseStateIx.expect at heq_expect
        split at heq_expect
...
end L4YAML.Proofs.Indexed.WfaProofs
