...
-/
lemma isBlank_def (c : Char) : isBlankBool c = (isWhiteSpaceBool c || isLineBreakBool c) := rfl

/--
Characterization of `isLineBreakBool`: exactly `'\n'` and `'\r'`.
-/
lemma isLineBreak_iff (c : Char) : isLineBreakBool c = true ↔ (c = '\n' ∨ c = '\r') := by
  constructor
  · intro h
...
-/
lemma isWhiteSpace_iff (c : Char) : isWhiteSpaceBool c = true ↔ (c = ' ' ∨ c = '\t') := by
  constructor
  · intro h
...
lemma isLineBreak_nl    : isLineBreakBool '\n' = true  := by native_decide
lemma isLineBreak_cr    : isLineBreakBool '\r' = true  := by native_decide
lemma isLineBreak_space : isLineBreakBool ' '  = false := by native_decide
lemma isLineBreak_tab   : isLineBreakBool '\t' = false := by native_decide

lemma isWhiteSpace_space : isWhiteSpaceBool ' '  = true  := by native_decide
lemma isWhiteSpace_tab   : isWhiteSpaceBool '\t' = true  := by native_decide
lemma isWhiteSpace_nl    : isWhiteSpaceBool '\n' = false := by native_decide
lemma isWhiteSpace_cr    : isWhiteSpaceBool '\r' = false := by native_decide

-- Flow indicators are a subset of indicators (per-character)
lemma flowIndicator_comma    : isFlowIndicatorBool ',' = true ∧ isIndicatorBool ',' = true := by constructor <;> native_decide
lemma flowIndicator_lbracket : isFlowIndicatorBool '[' = true ∧ isIndicatorBool '[' = true := by constructor <;> native_decide
lemma flowIndicator_rbracket : isFlowIndicatorBool ']' = true ∧ isIndicatorBool ']' = true := by constructor <;> native_decide
lemma flowIndicator_lbrace   : isFlowIndicatorBool '{' = true ∧ isIndicatorBool '{' = true := by constructor <;> native_decide
lemma flowIndicator_rbrace   : isFlowIndicatorBool '}' = true ∧ isIndicatorBool '}' = true := by constructor <;> native_decide
...
-/
lemma isFlowIndicator_implies_isIndicator (c : Char)
...
lemma isIndicator_alpha : isIndicatorBool 'a' = false := by native_decide
lemma isIndicator_digit : isIndicatorBool '0' = false := by native_decide
...
-- Virtual tokens
lemma streamStart_isVirtual     : YamlToken.isVirtual .streamStart       = true := rfl
lemma streamEnd_isVirtual       : YamlToken.isVirtual .streamEnd         = true := rfl
lemma blockSeqStart_isVirtual   : YamlToken.isVirtual .blockSequenceStart = true := rfl
lemma blockMapStart_isVirtual   : YamlToken.isVirtual .blockMappingStart  = true := rfl
lemma blockEnd_isVirtual        : YamlToken.isVirtual .blockEnd           = true := rfl

-- Non-virtual tokens
lemma scalar_not_isVirtual (v : String) (s : ScalarStyle) :
    YamlToken.isVirtual (.scalar v s) = false := rfl
lemma flowSeqStart_not_isVirtual : YamlToken.isVirtual .flowSequenceStart = false := rfl
lemma key_not_isVirtual          : YamlToken.isVirtual .key               = false := rfl

-- canStartNode tokens
lemma scalar_canStartNode (v : String) (s : ScalarStyle) :
    YamlToken.canStartNode (.scalar v s) = true := rfl
lemma alias_canStartNode (n : String) :
    YamlToken.canStartNode (.alias n) = true := rfl
lemma flowSeqStart_canStartNode :
    YamlToken.canStartNode .flowSequenceStart = true := rfl
lemma flowMapStart_canStartNode :
...
-/
lemma isVirtual_not_isFlowIndicator (t : YamlToken) :
...
-/
lemma scannerEscapeChar_deterministic (c : Char) (r₁ r₂ : Char)
...
lemma mk'_offset (input : String) : (ScannerState.mk' input).offset = 0 := rfl
lemma mk'_line (input : String) : (ScannerState.mk' input).line = 0 := rfl
lemma mk'_col (input : String) : (ScannerState.mk' input).col = 0 := rfl
lemma mk'_flowLevel (input : String) : (ScannerState.mk' input).flowLevel = 0 := rfl
lemma mk'_tokens_empty (input : String) : (ScannerState.mk' input).tokens = #[] := rfl
lemma mk'_simpleKeyAllowed (input : String) :
    (ScannerState.mk' input).simpleKeyAllowed = true := rfl
lemma mk'_needIndentCheck (input : String) :
    (ScannerState.mk' input).needIndentCheck = true := rfl

-- inFlow is false in initial state (flowLevel = 0)
lemma mk'_not_inFlow (input : String) : (ScannerState.mk' input).inFlow = false := rfl

-- emit appends exactly one token
lemma emit_tokens_size (s : ScannerState) (tok : YamlToken) :
    (s.emit tok).tokens.size = s.tokens.size + 1 := by
  simp [ScannerState.emit, Array.size_push]

-- hasMore reflects offset comparison
lemma hasMore_def (s : ScannerState) : s.hasMore = decide (s.offset < s.inputEnd) := rfl

-- inFlow reflects flowLevel
lemma inFlow_def (s : ScannerState) : s.inFlow = decide (s.flowLevel > 0) := rfl
...
lemma pushMappingIndent_grows (s : ScannerState) (col : Int)
    (h : col > s.currentIndent) :
...
-/
lemma TokenStream_ofTokens_pos (tokens : Array (Positioned YamlToken)) :
    (TokenStream.ofTokens tokens).pos = 0 := rfl

/--
`remaining` of a freshly created stream equals the token array size.
-/
lemma TokenStream_remaining_ofTokens (tokens : Array (Positioned YamlToken)) :
...
-/
lemma TokenStream_remaining_decreases
...
-/
lemma TokenStream_peek_some_iff (s : TokenStream) :
