...
/-- **Tail-not-separator (`getLast?` form)** for a `RecSeqBody` — the `SepAdj` mirror of
...
theorem RecSeqBody.getLast?_not_sep : {l : List (Positioned YamlToken)} →
  ...
  | _, .single e h_ne h_e h_head =>
      getLast?_not_sep_of_lastNonSep e h_ne
        (lastNonSep_of_entryUnit_contentHead e h_e.toEntryUnit ⟨h_ne, h_head⟩)
  | _, .cons e _fe rest _h_ne _h_e _h_head _h_fe h_rest => by
  ...
  | cons v tail ih =>
    ...
    | [], _ =>
      ...
      exact ⟨n, s', block, h_chain, h_corr, h_fl', h_dp, h_ids, h_ek', h_col', h_flow',
        ...
        RecSeqBody.single block h_cs_ne h_e h_cs_val, h_oa, h_sa⟩
    | v' :: vs, ih =>
      ...
      obtain ⟨n₃, s_end, block_rest, h_chain₃, h_corr_end, h_fl_end, h_dp_end, h_ids_end,
              h_ek_end, h_col_end, h_flow_end, h_indent_end, h_line_end, h_atol_end, h_endline_end, h_stack_end, h_fmc₃, h_block_eq_end, h_wb_rest, h_wt_rest, h_rec_rest, h_oa_rest, h_sa_rest⟩ :=
      ...
      · -- OpenerAdj (block₁ ++ [feTok] ++ block_rest): R403 seam over the recursive entries;
        -- block₁'s tail-not-opener is its `EntryUnit` last token (R404 substrate)
        exact OpenerAdj_seam block₁ block_rest feTok h_oa₁ h_oa_rest
          (by rw [h_feTok_val]; decide) (lastNonOpener_of_entryUnit block₁ h_eu₁)
      · -- SepAdj (block₁ ++ [feTok] ++ block_rest): R421 seam — block_rest's head is a content start
        -- (from its `RecSeqBody` → `SafeBody` head, gate `≠ .key` discarded); block₁'s tail is not a
        -- separator (its `EntryUnit` + content-head, `lastNonSep_of_entryUnit_contentHead`)
        have h_head_rest : ∀ (h0 : 0 < block_rest.length),
            (block_rest[0]'h0).val ≠ .key → isFlowContentStart (block_rest[0]'h0).val := by
          intro h0 _
          obtain ⟨_hl, hcs⟩ := SafeBody.head_Q (RecSeqBody.toSafeBody h_rec_rest)
          exact hcs
        exact SepAdj_seam block₁ block_rest feTok h_sa₁ h_sa_rest h_feTok_val h_head_rest
          (lastNonSep_of_entryUnit_contentHead block₁ h_eu₁ ⟨h_cs₁_ne, h_cs₁_val⟩)
...
/-- **Deep seq-body recursive-deliverable assembler** (STEP D step 8 — the `RecSeqBodyDeep`
    strengthening of `emitList_scans_recseqbody`).  A verbatim mirror of `emitList_scans_recseqbody`
    over the SAME induction on `items`, keyed on the *deep* per-item hypothesis
    `EmitScansInFlowRecEntryDeep` (the severance-free superset of `EmitScansInFlowRecEntry`, carrying
    `RecEntryDeep` in place of `RecSeqEntry`): the comma-separated body block of `emitList items` is a
    full `RecSeqBodyDeep` — `.single`/`.cons` accumulated by the identical induction, only the leaf
    constructors `RecSeqBodyDeep.single`/`.cons` and the head projection `RecSeqBodyDeep.toSafeBody`
    differ from the flat proof (every scan-state step is byte-identical, since the deep family carries
    no scan facts; [[ref-recursive-producer-mirrors-flat-over-shared-induction]]).  This is the body-level
    consumer of the deep per-entry deliverable the deep per-entry PRODUCER will feed — the seq-arm
...
theorem emitList_scans_recseqbodyDeep (items : List YamlValue) (h_ne : items ≠ [])
  ...
  | cons v tail ih =>
    ...
    | [], _ =>
      ...
      exact ⟨n, s', block, h_chain, h_corr, h_fl', h_dp, h_ids, h_ek', h_col', h_flow',
        h_indent', h_line_v, h_atol', h_endline', h_stack', h_fmc', h_block_eq, h_wb, h_wt,
        RecSeqBodyDeep.single block h_cs_ne h_e h_cs_val, h_oa, h_sa⟩
    | v' :: vs, ih =>
      have h_eq : (emit.emitList (v :: v' :: vs)).toList ++ rest_chars =
          (emit v).toList ++ ([',', ' '] ++ (emit.emitList (v' :: vs)).toList ++ rest_chars) := by
        simp [emit.emitList, String.toList_append, List.append_assoc]
      rw [h_eq] at hcorr
      -- Step 1: Scan emit v via EmitScansInFlowRecEntryDeep (item block `block₁`, with RecEntryDeep + head)
      have h_ev : EmitScansInFlowRecEntryDeep v := h_all v (.head _)
      obtain ⟨n₁, s₁, block₁, h_chain₁, h_corr₁, h_fl₁, h_dp₁, h_ids₁, h_ek₁, h_col₁, h_flow₁,
              h_indent₁, _h_line₁, _h_ska₁, h_last₁, h_atol₁, h_endline₁, h_stack₁, h_fmc₁,
              h_block_eq₁, h_wb₁, h_wt₁, _h_es₁, h_eu₁, h_e₁, h_cs₁, h_oa₁, h_sa₁⟩ :=
        h_ev s ([',', ' '] ++ (emit.emitList (v' :: vs)).toList ++ rest_chars)
      ...
      have h_corr₃' : ScannerSurfCorr s₃
        ...
        have : c :: (rest' ++ rest_chars) = (emit.emitList (v' :: vs)).toList ++ rest_chars := by
          rw [h_first]; simp only [List.cons_append]
        rwa [this] at h_corr₃
      -- Step 4: Recursive scan of emitList (v' :: vs) from s₃ (tail block `block_rest`, with RecSeqBodyDeep)
      have h_tail_all : ∀ w ∈ v' :: vs, EmitScansInFlowRecEntryDeep w :=
        fun w hw => h_all w (.tail _ hw)
      obtain ⟨n₃, s_end, block_rest, h_chain₃, h_corr_end, h_fl_end, h_dp_end, h_ids_end,
              h_ek_end, h_col_end, h_flow_end, h_indent_end, h_line_end, h_atol_end, h_endline_end, h_stack_end, h_fmc₃, h_block_eq_end, h_wb_rest, h_wt_rest, h_rec_rest, h_oa_rest, h_sa_rest⟩ :=
        ih (by simp) h_tail_all s₃ rest_chars h_corr₃'
      ...
      · -- WellTyped (block₁ ++ [feTok] ++ block_rest) — typed twin of the WellBracketed bullet.
        exact WellTyped_append _ _
          (WellTyped_append _ _ h_wt₁
            (WellTyped_singleton_delta_zero feTok (by rw [h_feTok_val]; exact flowBracketDelta_flowEntry)))
          h_wt_rest
      · -- RecSeqBodyDeep (block₁ ++ [feTok] ++ block_rest)
        have h_reassoc : block₁ ++ [feTok] ++ block_rest = block₁ ++ feTok :: block_rest := by
          rw [List.append_assoc]; rfl
        rw [h_reassoc]
        exact RecSeqBodyDeep.cons block₁ feTok block_rest h_cs₁_ne h_e₁ h_cs₁_val h_feTok_val h_rec_rest
      · -- OpenerAdj (block₁ ++ [feTok] ++ block_rest): R403 seam over the recursive entries;
        -- block₁'s tail-not-opener is its `EntryUnit` last token (R404 substrate)
        exact OpenerAdj_seam block₁ block_rest feTok h_oa₁ h_oa_rest
          (by rw [h_feTok_val]; decide) (lastNonOpener_of_entryUnit block₁ h_eu₁)
      · -- SepAdj (block₁ ++ [feTok] ++ block_rest): R421 seam — block_rest's head is a content start
        -- (from its `RecSeqBodyDeep` → `SafeBody` head, gate `≠ .key` discarded); block₁'s tail is not a
        -- separator (its `EntryUnit` + content-head, `lastNonSep_of_entryUnit_contentHead`)
        have h_head_rest : ∀ (h0 : 0 < block_rest.length),
            (block_rest[0]'h0).val ≠ .key → isFlowContentStart (block_rest[0]'h0).val := by
          intro h0 _
          obtain ⟨_hl, hcs⟩ := SafeBody.head_Q (RecSeqBodyDeep.toSafeBody h_rec_rest)
          exact hcs
        exact SepAdj_seam block₁ block_rest feTok h_sa₁ h_sa_rest h_feTok_val h_head_rest
          (lastNonSep_of_entryUnit_contentHead block₁ h_eu₁ ⟨h_cs₁_ne, h_cs₁_val⟩)
...
/-- **Deep map-body recursive-deliverable assembler** (STEP D step 8 — the `RecMapBodyDeep`
    strengthening of `emitPairList_scans_recmapbody`).  A verbatim mirror of `emitPairList_scans_recmapbody`
    over the SAME induction on `pairs`, keyed on the *deep* per-entry hypotheses
    `EmitScansInFlowSavedKeyRecEntryDeep` (key) / `EmitScansInFlowRecEntryDeep` (value), each carrying
    `RecEntryDeep` in place of `RecSeqEntry`: the `, `-separated key/value body block of
    `emit.emitPairList pairs` is a full `RecMapBodyDeep` — `.single`/`.cons` accumulated by the identical
    induction, the per-pair `RecMapPairDeep.mk` built from the deep key/value entries `h_ke`/`h_ve`.  Per
    [[ref-mirror-reads-conjunct-not-projection]] the mirror cost was QUANTIFIED before writing: four global
    text swaps (`EmitScansInFlow{SavedKey,}RecEntry`→`…Deep`, `RecMapPair`→`RecMapPairDeep`,
    `RecMapBody`→`RecMapBodyDeep`) plus exactly TWO reroutes — the flat proof read the value `EntryUnit`
    via the projection `h_ve.toEntryUnit`, but `RecEntryDeep` sheds `.toEntryUnit`
    ([[ref-parallel-family-sheds-orthogonal-field]]), so both reads reroute to the value predicate's
    STANDALONE `EntryUnit` conjunct `_h_eu_v` (which the deep predicate keeps precisely because the deep
  ...
  | cons p tail ih =>
    ...
    | [], _ =>
      have h_eq : (emit.emitPairList [p]).toList ++ rest_chars =
        ...
        simp [emit.emitPairList, String.toList_append, List.append_assoc]
      rw [h_eq] at hcorr
      have h_ek_key : EmitScansInFlowSavedKeyRecEntryDeep p.1 := h_all_k p (.head _)
      obtain ⟨n₁, s₁, block_k, h_chain₁, h_corr₁, h_fl₁, h_dp₁, h_ids₁, h_ek₁, h_col₁,
              h_flow₁, h_indent₁, _h_line₁, h_atol₁, h_endline₁, h_stack₁, h_fmc₁,
              h_ska₁, h_poss₁, h_tidx₁, h_szlt₁, _h_ph0₁, h_ph1₁, h_blockeq_k, h_take_k, h_wb_k, h_wt_k, _h_es_k, h_ke, h_tail_k, h_oa_k, h_lns_k, h_sa_k⟩ :=
        h_ek_key s ([':', ' '] ++ (emit p.2).toList ++ rest_chars)
      ...
      have h_block_end : (s_end.tokens.filter (fun t => t.val != .placeholder)).toList
        ...
        rw [h_blockeq_v, h_s3_s2, h_block_kc, List.append_assoc]
      -- Per-pair entry `EntrySafe`: `.key`/`.value` glue (delta 0) around `EntrySafe` blocks.
      have h_pair : RecMapPairDeep ((⟨s₁.simpleKey.pos, .key, s₁.simpleKey.pos⟩ ::
            (block_k ++ [⟨pos_v, .value, pos_v⟩])) ++ block_v) := by
        have h_reassoc_pair : (⟨s₁.simpleKey.pos, .key, s₁.simpleKey.pos⟩ ::
              (block_k ++ [⟨pos_v, .value, pos_v⟩])) ++ block_v
            = ⟨s₁.simpleKey.pos, .key, s₁.simpleKey.pos⟩ ::
          ...
          rw [List.cons_append, List.append_assoc]; rfl
        rw [h_reassoc_pair]
        exact RecMapPairDeep.mk _ block_k _ block_v (by rfl) h_ke (by rfl) h_ve
      refine ⟨n₁ + 1 + (n_v' + 1), s_end,
        (⟨s₁.simpleKey.pos, .key, s₁.simpleKey.pos⟩ :: (block_k ++ [⟨pos_v, .value, pos_v⟩])) ++ block_v,
        h_arith ▸ h_chain_all, h_corr_end, ?_, ?_, ?_, ?_, h_col_end, h_flow_end, h_indent_end,
        ?_, h_atol_end, h_endline_end, ?_, h_arith ▸ h_fmc_all, h_block_end, ?_, ?_, ?_, ?_, ?_, by omega⟩
      ...
      · -- WellTyped ((.key :: block_k ++ [.value]) ++ block_v) — typed twin.
        exact WellTyped_cons_delta_zero _ _ (by rfl)
          (WellTyped_append _ _
            (WellTyped_append _ _ h_wt_k (WellTyped_singleton_delta_zero _ (by rfl)))
            h_wt_v)
      · -- RecMapBodyDeep ((.key :: block_k ++ [.value]) ++ block_v)
        exact RecMapBodyDeep.single _ (by exact List.cons_ne_nil _ _) h_pair (by rfl)
      · -- OpenerAdj ((.key :: block_k ++ [.value]) ++ block_v): the R403 map-single glue
        exact OpenerAdj_map_single _ _ block_k block_v (fun h => YamlToken.noConfusion h)
          (fun h => YamlToken.noConfusion h) h_oa_k h_oa_v h_tail_k
      · -- SepAdj ((.key :: block_k ++ [.value]) ++ block_v): the R420 map-single glue
    ...
    | p' :: ps, ih =>
      ...
      have h_corr_pp' : ScannerSurfCorr s_pp
        ...
        have : c_p :: (rest_p ++ rest_chars) =
          ...
          rw [h_first_p]; simp only [List.cons_append]
        rwa [this] at h_corr_pp
      have h_tail_all_k : ∀ q ∈ p' :: ps, EmitScansInFlowSavedKeyRecEntryDeep q.1 :=
        fun q hq => h_all_k q (.tail _ hq)
      have h_tail_all_v : ∀ q ∈ p' :: ps, EmitScansInFlowRecEntryDeep q.2 :=
        fun q hq => h_all_v q (.tail _ hq)
      obtain ⟨n_r, s_end, block_rest, h_chain_r, h_corr_end, h_fl_end, h_dp_end, h_ids_end,
              h_ek_end, h_col_end, h_flow_end, h_indent_end, h_line_end, h_atol_end, h_endline_end, h_stack_end, h_fmc_r, h_blockeq_rest, h_wb_rest, h_wt_rest, h_rec_rest, h_oa_rest, h_sa_rest, h_n_r_ge3⟩ :=
        ih (by simp) h_tail_all_k h_tail_all_v s_pp rest_chars h_corr_pp'
        ...
        exact WellTyped_append _ _
          (WellTyped_append _ _
            ...
            (WellTyped_singleton_delta_zero feTok
              (by rw [h_feTok_val]; exact flowBracketDelta_flowEntry)))
          h_wt_rest
      · -- RecMapBodyDeep ((entry ++ [feTok]) ++ block_rest)
        have h_reassoc : (((⟨s₁.simpleKey.pos, .key, s₁.simpleKey.pos⟩ ::
              (block_k ++ [⟨pos_v, .value, pos_v⟩])) ++ block_v) ++ [feTok]) ++ block_rest
            = ((⟨s₁.simpleKey.pos, .key, s₁.simpleKey.pos⟩ ::
              (block_k ++ [⟨pos_v, .value, pos_v⟩])) ++ block_v) ++ feTok :: block_rest := by
          rw [List.append_assoc]; rfl
        rw [h_reassoc]
        exact RecMapBodyDeep.cons _ feTok block_rest (by exact List.cons_ne_nil _ _)
          h_pair (by rfl) h_feTok_val h_rec_rest
      · -- OpenerAdj ((entry ++ [feTok]) ++ block_rest): per-pair `OpenerAdj_map_single` glued to the
        -- recursive tail's OpenerAdj across the delta-`0` `.flowEntry` seam; the entry tail-not-opener
        -- is the VALUE block's (the entry ends in `block_v`), off its `EntryUnit`.
        exact OpenerAdj_seam _ block_rest feTok
          ...
          h_oa_rest (by rw [h_feTok_val]; decide)
          (lastNonOpener_append_right _ block_v h_bv_ne
            (lastNonOpener_of_entryUnit block_v _h_eu_v))
      · -- SepAdj ((entry ++ [feTok]) ++ block_rest): per-pair `SepAdj_map_single` glued to the tail's
        -- SepAdj; the `.flowEntry` seam's successor is `block_rest`'s `.key` head (the `≠ .key` gate is
        -- FALSE, vacuous), and the entry tail-not-sep is the VALUE block's last (off `EntryUnit` + head).
        exact SepAdj_seam _ block_rest feTok
          ...
          h_sa_rest h_feTok_val
          (by intro h0 hne
              obtain ⟨_hl, hkey⟩ := SafeBody.head_Q (RecMapBodyDeep.toSafeBody h_rec_rest)
              exact absurd hkey hne)
          (lastNonSep_append_right _ block_v h_bv_ne
            (lastNonSep_of_entryUnit_contentHead block_v _h_eu_v _h_cs_v))
...
/-- **Combined value + saved-key `RecSeqEntry` producer** (R452 — conjunction-motive merge of
    `emit_scans_in_flow_rec_entry` (#1, value) and `emit_scans_in_flow_saved_key_rec_entry` (#2, saved
    key)).  Proves the CONJUNCTION of the two per-entry deliverables by a SINGLE `induction hg` on the
    shared `Grammable v inFlow` — the one induction hypothesis carries BOTH deliverables at every
    sub-value, so the saved-key `.seq` arm's value-body dependency (formerly the by-name #2 → #1 citation
    `emit_scans_in_flow_rec_entry _ (h ⟨i,_⟩)`) is now `(ih ⟨i,_⟩).1` off the conjunction IH.  No producer
    cross-citation survives, so no `mutual theorem` block is needed (R451 de-risk validated on the real
    recursor by `MapProducerMutualResolutionProbe`; the data-level mutual recursion stays in
    `RecSeqEntry` ↔ `RecMapBody`).  #1/#2 re-expose below as thin `.1`/`.2` projections — signatures (and
  ...
  | scalar sc _ h =>
      ...
      · intro s_state rest hcorr h_flow h_fl h_indent h_col h_ek h_atol h_endline _h_sync
        ...
        refine ⟨1, s', [tok], ScanChainGrew.single h_snt h_grew, h_corr', h_fl', h_dp', h_ids', h_ek',
          ...
          OpenerAdj_singleton tok, SepAdj_singleton tok⟩
      ...
      · intro s_state rest hcorr h_flow h_fl h_indent h_col h_ek h_atol h_endline h_ska _h_sync
        ...
        refine ⟨1, s'', [tok], ScanChainGrew.single h_snt'' h_grew, h_corr', h_fl', h_dp', h_ids', h_ek',
          ...
          lastNonSep_singleton tok (by rw [h_tok_val]; exact fun h => YamlToken.noConfusion h),
          SepAdj_singleton tok⟩
        · unfold ScannerState.inFlow; rw [h_fl']
          unfold ScannerState.inFlow at h_flow; exact h_flow
        · unfold ScannerState.currentIndent; rw [h_ids']; exact h_indent
        · rw [h_push, Array.toList_push]
  | sequence style items tag anchor _ h ih =>
      ...
      · intro s_state rest hcorr h_flow h_fl h_indent h_col h_ek h_atol h_endline h_sync
        ...
        obtain ⟨n₂, s₂, bodyBlock, h_chain₂, h_corr₂, h_fl₂, h_dp₂, h_ids₂, h_ek₂, h_col₂,
                ...
                h_body_append, h_body_wb, h_body_wt, h_entry_builder, h_body_oa, h_body_head, h_body_tail, h_body_sa, h_body_lns⟩ :
            (∃ n₂ s₂ bodyBlock,
              ScanChainGrew (fun t => t.val != .placeholder) s₁ n₂ s₂
              ∧ ScannerSurfCorr s₂ ⟨[']'] ++ rest, s₂.col⟩
              ∧ s₂.flowLevel = s₁.flowLevel
              ...
              ∧ (∀ (fss fse : Positioned YamlToken),
                  fss.val = .flowSequenceStart → fse.val = .flowSequenceEnd →
                    RecSeqEntry (fss :: (bodyBlock ++ [fse])))
              ∧ OpenerAdj bodyBlock
              ∧ (∀ (h0 : 0 < bodyBlock.length),
                  (bodyBlock[0]'h0).val ≠ .flowSequenceEnd → isFlowContentStart (bodyBlock[0]'h0).val)
          ...
          | [] =>
            refine ⟨0, s₁, [], .zero, ?_, rfl, rfl, rfl, rfl, h_s1_col, h_s1_inflow, h_s1_indent, rfl,
                    ...
                    OpenerAdj_nil, (by intro h0; simp at h0), (by intro hla; simp at hla), SepAdj_nil,
          ...
          | w :: ws =>
            ...
            exact ⟨n₂, s₂, bodyBlock, h_chain₂, h_corr₂, h_fl₂, h_dp₂, h_ids₂, h_ek₂, h_col₂,
                   ...
                   h_body_oa, h_body_rec.openerAdjHead, h_body_rec.lastNonOpener, h_body_sa,
                   h_body_rec.lastNonSep⟩
        ...
        refine ⟨(1 + n₂) + 1, s₃, fssTok :: (bodyBlock ++ [fseTok]),
          ...
          SepAdj_wrap_seq fssTok fseTok bodyBlock h_fss_val h_body_sa h_body_lns⟩
      ...
      · intro s_state rest hcorr h_flow h_fl h_indent h_col h_ek h_atol h_endline h_ska h_sync
        ...
        refine ⟨(1 + n₂) + 1, s₃, fssTok :: (bodyBlock ++ [fseTok]),
          ...
          OpenerAdj_wrap_seq fssTok fseTok bodyBlock h_fss_val h_fse_val h_body_oa h_body_head h_body_tail,
          lastNonSep_wrap fssTok fseTok bodyBlock (by rw [h_fse_val]; decide),
          SepAdj_wrap_seq fssTok fseTok bodyBlock h_fss_val h_body_sa h_body_lns⟩
        · rw [h_fl₃, h_fl₂, h_fl₁]; omega
        · rw [h_dp₃, h_dp₂, h_dp₁]
        · rw [h_ids₃, h_ids₂, h_ids₁]
...
/-- **Deep per-entry recursive-deliverable producer** (STEP D step 9 — the recursion-CLOSING
    `RecEntryDeep` strengthening of `emit_scans_in_flow_rec_entry_both`).  Mirrors the flat producer
    over the SAME `Grammable` induction, delivering the severance-free deep per-entry predicates
    `EmitScansInFlowRecEntryDeep v ∧ EmitScansInFlowSavedKeyRecEntryDeep v` (identical to the flat
    predicates with the lone structural conjunct `RecSeqEntry block` refined to `RecEntryDeep block`).
    Unlike the deep BODY producers (`emitList_scans_recseqbodyDeep` / `emitPairList_scans_recmapbodyDeep`,
    R464/R465), this is NOT a pure text-swap (`[[ref-mirror-constructor-arity-reads-storage]]`): the deep
    leaf constructors have DIFFERENT arity — `RecEntryDeep.seq` / `.mapRec` DROP the orthogonal
    `WellBracketed` field the flat `RecSeqEntry.seq` / `.mapRec` store, and the flat empty-map leaf
    `RecSeqEntry.map fms fme [] … WellBracketed_nil` splits into the dedicated `RecEntryDeep.mapEmpty`
    (the no-`nil` `RecMapBodyDeep` cannot hold `{ }`).  The boundary reads
    `RecSeqBody.{openerAdjHead,lastNonOpener,lastNonSep}` / `RecMapBody.{lastNonOpener,lastNonSep}` are
    COERCE-TO-WEAKER reroutes (`[[ref-coerce-to-weaker-reuse-wrapper]]`): the deep body type sheds these
    projections, so each reads off the `.toFlat`-projected body (`RecSeqBodyDeep.toFlat` /
    `RecMapBodyDeep.toFlat`).  The deep body producers feed the seq / map interiors; `scalar` / `seqEmpty`
  ...
  | scalar sc _ h =>
      ...
      · intro s_state rest hcorr h_flow h_fl h_indent h_col h_ek h_atol h_endline _h_sync
        ...
        refine ⟨1, s', [tok], ScanChainGrew.single h_snt h_grew, h_corr', h_fl', h_dp', h_ids', h_ek',
          h_col', ?_, ?_, _h_line', h_ska', h_tok', h_atol', h_endline', h_stack',
          FlowMonoChain.single h_snt (Nat.le.refl) (by omega), ?_, ?_, ?_, ?_,
          EntryUnit_scalar tok str st h_tok_val, RecEntryDeep.scalar tok str st h_tok_val, ?_,
          OpenerAdj_singleton tok, SepAdj_singleton tok⟩
        · unfold ScannerState.inFlow; rw [h_fl']
          unfold ScannerState.inFlow at h_flow; exact h_flow
        · unfold ScannerState.currentIndent; rw [h_ids']; exact h_indent
      ...
      · intro s_state rest hcorr h_flow h_fl h_indent h_col h_ek h_atol h_endline h_ska _h_sync
        ...
        refine ⟨1, s'', [tok], ScanChainGrew.single h_snt'' h_grew, h_corr', h_fl', h_dp', h_ids', h_ek',
          ...
          FlowMonoChain.single h_snt'' (Nat.le.refl) (by omega),
          h_ska', h_poss'', h_tidx'', h_size'', h_ph'', h_ph1'', ?_, h_take,
          h_wb_es.1, h_wb_es.2.1, h_wb_es.2.2, RecEntryDeep.scalar tok str st h_tok_val,
          lastNonOpener_singleton tok (by rw [h_tok_val]; exact fun h => YamlToken.noConfusion h),
          OpenerAdj_singleton tok,
          lastNonSep_singleton tok (by rw [h_tok_val]; exact fun h => YamlToken.noConfusion h),
          SepAdj_singleton tok⟩
      ...
      · intro s_state rest hcorr h_flow h_fl h_indent h_col h_ek h_atol h_endline h_sync
        ...
        obtain ⟨n₂, s₂, bodyBlock, h_chain₂, h_corr₂, h_fl₂, h_dp₂, h_ids₂, h_ek₂, h_col₂,
            ...
            (∃ n₂ s₂ bodyBlock,
              ...
              ∧ (∀ (fss fse : Positioned YamlToken),
                  fss.val = .flowSequenceStart → fse.val = .flowSequenceEnd →
                    RecEntryDeep (fss :: (bodyBlock ++ [fse])))
              ∧ OpenerAdj bodyBlock
              ∧ (∀ (h0 : 0 < bodyBlock.length),
                  (bodyBlock[0]'h0).val ≠ .flowSequenceEnd → isFlowContentStart (bodyBlock[0]'h0).val)
              ∧ (∀ (hla : 0 < bodyBlock.length),
          ...
          | [] =>
            refine ⟨0, s₁, [], .zero, ?_, rfl, rfl, rfl, rfl, h_s1_col, h_s1_inflow, h_s1_indent, rfl,
                    h_atol₁, h_endline₁, rfl, .zero (Nat.le.refl), ?_, WellBracketed_nil, WellTyped_nil,
                    fun fss fse hf1 hf2 => RecEntryDeep.seqEmpty fss fse hf1 hf2,
                    OpenerAdj_nil, (by intro h0; simp at h0), (by intro hla; simp at hla), SepAdj_nil,
                    (by intro hla; simp at hla)⟩
            · have h_e : (emit.emitList items.toList).toList ++ ([']'] ++ rest) = [']'] ++ rest := by
                rw [h_list]; simp only [emit.emitList]; rfl
            ...
            · simp
          | w :: ws =>
            have h_all_rec : ∀ u ∈ (w :: ws), EmitScansInFlowRecEntryDeep u := fun u hu => by
              have hu' : u ∈ items.toList := h_list ▸ hu
              have ⟨i, hi, h_eq⟩ := List.getElem_of_mem hu'
              have h_sz : i < items.size := by rwa [Array.length_toList] at hi
              exact h_eq ▸ (ih ⟨i, h_sz⟩).1
            ...
            exact ⟨n₂, s₂, bodyBlock, h_chain₂, h_corr₂, h_fl₂, h_dp₂, h_ids₂, h_ek₂, h_col₂,
                   h_s2_inflow, h_s2_indent, _h_line₂, h_atol₂, h_endline₂, h_stack₂, h_fmc₂,
                   h_body_append, h_body_wb, h_body_wt,
                   (fun fss fse hf1 hf2 => RecEntryDeep.seq fss fse bodyBlock hf1 hf2 h_body_rec),
                   h_body_oa, (RecSeqBodyDeep.toFlat h_body_rec).openerAdjHead, (RecSeqBodyDeep.toFlat h_body_rec).lastNonOpener, h_body_sa,
                   (RecSeqBodyDeep.toFlat h_body_rec).lastNonSep⟩
        have h_fl₂_ge2 : s₂.flowLevel ≥ 2 := by rw [h_fl₂, h_fl₁]; omega
        have h_stack_endline₂ : StackEndLineOnLine s₂ s₂.line := by
          unfold StackEndLineOnLine at h_stack_endline₁ ⊢
          rw [h_stack₂, _h_line₂]; exact h_stack_endline₁
  ...
  | mapping style pairs tag anchor _ _hk _hv ihk ihv =>
      ...
      · intro s_state rest hcorr h_flow h_fl h_indent h_col h_ek h_atol h_endline h_sync
        ...
        have h_corr₁_assoc : ScannerSurfCorr s₁
          ...
          rw [List.append_assoc] at h_corr₁; exact h_corr₁
        -- STEP D wiring (value arm): scan the body with the *recursive* assembler
        -- `emitPairList_scans_recmapbodyDeep` (non-empty) so the entry builder is `RecEntryDeep.mapRec`
        -- carrying `RecMapBodyDeep bodyBlock`; the empty `{}` body stays a flat `RecEntryDeep.mapEmpty`.  The
        -- body tail-not-`[`/`,` facts the map wrap consumes are read off `RecMapBody.lastNonOpener`/
        -- `.lastNonSep` (in scope after the step-2c move).
        obtain ⟨n₂, s₂, bodyBlock, h_chain₂, h_corr₂, h_fl₂, h_dp₂, h_ids₂, h_ek₂, h_col₂,
                h_s2_inflow, h_s2_indent, _h_line₂, h_atol₂, h_endline₂, h_stack₂, h_fmc₂,
            ...
            (∃ n₂ s₂ bodyBlock,
              ...
              ∧ WellTyped bodyBlock
              ∧ (∀ fms fme : Positioned YamlToken, fms.val = .flowMappingStart → fme.val = .flowMappingEnd →
                   RecEntryDeep (fms :: (bodyBlock ++ [fme])))
              ∧ OpenerAdj bodyBlock
              ∧ (∀ (hla : 0 < bodyBlock.length),
                  (bodyBlock[bodyBlock.length - 1]'(Nat.sub_lt hla Nat.one_pos)).val ≠ .flowSequenceStart)
              ∧ SepAdj bodyBlock
          ...
          | [] =>
            refine ⟨0, s₁, [], .zero, ?_, rfl, rfl, rfl, rfl, h_s1_col, h_s1_inflow, h_s1_indent, rfl,
                    h_atol₁, h_endline₁, rfl, .zero (Nat.le.refl), ?_, WellBracketed_nil, WellTyped_nil,
                    (fun fms fme hf1 hf2 => RecEntryDeep.mapEmpty fms fme hf1 hf2),
                    OpenerAdj_nil, (by intro hla; simp at hla), SepAdj_nil,
                    (by intro hla; simp at hla)⟩
            · have h_e : (emit.emitPairList pairs.toList).toList ++ (['}'] ++ rest) = ['}'] ++ rest := by
                rw [h_list]; simp only [emit.emitPairList]; rfl
            ...
            · simp
          | p :: ps =>
            have h_all_k : ∀ q ∈ (p :: ps), EmitScansInFlowSavedKeyRecEntryDeep q.1 := fun q hq => by
              have hq' : q ∈ pairs.toList := h_list ▸ hq
              have ⟨i, hi, h_eq⟩ := List.getElem_of_mem hq'
              have h_sz : i < pairs.size := by rwa [Array.length_toList] at hi
              exact h_eq ▸ (ihk ⟨i, h_sz⟩).2
            have h_all_v : ∀ q ∈ (p :: ps), EmitScansInFlowRecEntryDeep q.2 := fun q hq => by
              have hq' : q ∈ pairs.toList := h_list ▸ hq
              have ⟨i, hi, h_eq⟩ := List.getElem_of_mem hq'
              have h_sz : i < pairs.size := by rwa [Array.length_toList] at hi
              exact h_eq ▸ (ihv ⟨i, h_sz⟩).1
            ...
            exact ⟨n₂, s₂, bodyBlock, h_chain₂, h_corr₂, h_fl₂, h_dp₂, h_ids₂, h_ek₂, h_col₂,
                   h_s2_inflow, h_s2_indent, _h_line₂, h_atol₂, h_endline₂, h_stack₂, h_fmc₂,
                   h_body_append, h_body_wb, h_body_wt,
                   (fun fms fme hf1 hf2 => RecEntryDeep.mapRec fms fme bodyBlock hf1 hf2 h_body_rec),
                   h_body_oa, RecMapBody.lastNonOpener (RecMapBodyDeep.toFlat h_body_rec), h_body_sa, RecMapBody.lastNonSep (RecMapBodyDeep.toFlat h_body_rec)⟩
        have h_fl₂_ge2 : s₂.flowLevel ≥ 2 := by rw [h_fl₂, h_fl₁]; omega
        have h_stack_endline₂ : StackEndLineOnLine s₂ s₂.line := by
          unfold StackEndLineOnLine at h_stack_endline₁ ⊢
          rw [h_stack₂, _h_line₂]; exact h_stack_endline₁
      ...
      · intro s_state rest hcorr h_flow h_fl h_indent h_col h_ek h_atol h_endline h_ska h_sync
        ...
        have h_corr₁_assoc : ScannerSurfCorr s₁
            ⟨(emit.emitPairList pairs.toList).toList ++ (['}'] ++ rest), s₁.col⟩ := by
          rw [List.append_assoc] at h_corr₁; exact h_corr₁
        -- STEP D wiring (saved-key arm): mirror of the value arm — recursive `emitPairList_scans_recmapbodyDeep`
        -- (non-empty) builds `RecEntryDeep.mapRec`; the empty body stays a flat `RecEntryDeep.mapEmpty`.
        obtain ⟨n₂, s₂, bodyBlock, h_chain₂, h_corr₂, h_fl₂, h_dp₂, h_ids₂, h_ek₂, h_col₂,
                h_s2_inflow, h_s2_indent, _h_line₂, h_atol₂, h_endline₂, h_stack₂, h_fmc₂,
                h_body_append, h_body_wb, h_body_wt, h_entry_builder, h_body_oa, h_body_tail, h_body_sa, h_body_lns⟩ :
            (∃ n₂ s₂ bodyBlock,
...
theorem flowSubrangesOk_of_locators (tokens : Array (Positioned YamlToken))
  ...
  seq := fun lo hi h_lo_hi h_hi_sz h_tpe h_bal h_open h_floor =>
    (Nat.eq_or_lt_of_le h_lo_hi).elim
      (fun h_eq => seqBodyProps_empty tokens lo hi h_eq)
      (fun h_lt =>
        let L := h_seq lo hi h_lt h_hi_sz h_tpe h_bal h_open h_floor
        seqBodyProps_of_located_entry tokens lo hi L.pos h_lo_hi h_hi_sz h_tpe h_bal L.dyck L.wt
          h_open L.entry)
  map := fun lo hi h_lo_hi h_hi_sz h_tpe h_bal h_open h_floor =>
    (Nat.eq_or_lt_of_le h_lo_hi).elim
      (fun h_eq => mapBodyProps_empty tokens lo hi h_eq)
      (fun h_lt =>
        let L := h_map lo hi h_lt h_hi_sz h_tpe h_bal h_open h_floor
...
theorem seqLocator_of_window_recseqbody (tokens : Array (Positioned YamlToken))
    ...
    ∀ lo hi, lo < hi → hi < tokens.size →
      ...
      (∀ i, lo ≤ i → i ≤ hi → flowBracketBalance tokens lo i ≥ 0) →
      SeqLocated tokens lo hi := by
  intro lo hi h_lo_lt h_hi_sz h_close h_bal h_open h_floor
  ...
  exact seqLocated_of_recseqbody_outer tokens 2 (tokens.size - 2) lo hi
    ...
    (h_seq_rec lo hi h_lo2 h_lo_lt h_hi2 h_hi_sz h_close h_bal h_open h_floor)
...
/-- **The map locate DRIVER** (Phase J, map side — `(i'-b-B2c-desc-map-navigator-driver)`, R528).
    The symmetric mirror of `recseqbody_navigator_driver` (the R264 entry-boundary-input-shape split
    [[ref-entry-boundary-input-shape-split]] across the map axis): the seq driver named a
    collection-specific deliverable type (`RecSeqEntry`/`RecSeqBody`, via `recseqbody_window_assemble`),
    so by that discriminator the map axis re-splits and owes this brick.  The map structural moves are
    complete (DESCEND `recmapbody_window_of_located_entry`, ADVANCE `recmapbody_cons_window`, TERMINATE
    `recmapbody_single_window`, and their selector `recmapbody_window_assemble`), and the abstract
    width-recursion plumbing is the shared `windowWidth_strongRecOn`.  This brick **stitches** them: it
    instantiates the combinator at `P := fun lo hi => RecMapBody ((tokens.toList.take hi).drop lo)`,
    bakes in the `recmapbody_window_assemble` consumer, and re-expresses the per-window step as the
    navigator's *contract* — so the whole map navigator now reduces to **exactly two named
    obligations**:

    * `locate` — the per-window first-PAIR *classify*: given the guard `G lo hi` and the recursion's
      own width-oracle (delivering `RecMapBody` for nested descent), locate the first pair's depth-`0`
      extent `m`, the marker `m = hi ∨ tokens[m] = .flowEntry`, and the pair
      `RecMapPair ((take m).drop lo)`.  The pair's interior splits into a key and a value sub-block, each
      owing a single `RecSeqEntry` — supplied by `recseqentry_whole_window_seq` (R527) on the R524
      sub-block windows (the seq dispatch reused under a no-interior-boundary certificate).  The genuine
      remaining knot is the cross-deliverable IH source: the width-oracle here delivers `RecMapBody`,
      but a nested *sequence* inside a key/value needs `RecSeqBody` — resolved by a joint `P` dispatched
      on the window close token (`recseqbody_navigator_driver` consumes the seq half).
    * `descend_tail` — the guard descends from `[lo, hi)` to the *suffix* window `[m+1, hi)` past a
      depth-`0` `.flowEntry` separator ([[ref-guard-threading-skeleton-before-grammar]], the
      producer's GIFT / consumer's DEBT [[ref-guarded-universal-fold-relocates-guard]]); `RecMapBody`'s
      separator is the same `.flowEntry` as `RecSeqBody`'s, so this obligation is collection-agnostic and
      its skeleton is the seq side's verbatim.

    The guard `G` is left **abstract**, exactly as on the seq side: the driver is agnostic to its
    definition, making visible that the hard guard-reconciliation reduces to `locate` + `descend_tail`
    and nothing else.  This is [[ref-fold-consumer-chain-to-producer-contract]] at the recursion-driver
    layer, the dual of `windowWidth_strongRecOn`'s [[ref-consumer-joint-before-producer]].  The proof is
    the seq driver transported verbatim — same `windowWidth_strongRecOn` refine, same `locate`/oracle
    destructure, same `recmapbody_window_assemble` selector, same `resolve_left`/`descend_tail` advance —
    swapping only the deliverable types (`RecMapBody`/`RecMapPair` for `RecSeqBody`/`RecSeqEntry`) and
    the assembler (`recmapbody_window_assemble` for `recseqbody_window_assemble`); the marker token and
    tail-oracle guard are shared, so this is a pure mirror with no new analytical content.
    Verified-but-unconsumed until the concrete `G`/`locate`/`descend_tail` are supplied (R528): composes
...
theorem recmapbody_navigator_driver (tokens : Array (Positioned YamlToken))
    ...
    (descend_tail : ∀ lo hi m, G lo hi → lo < m → m < hi →
        tokens[m]!.val = .flowEntry → G (m + 1) hi) :
    ∀ lo hi, G lo hi → RecMapBody ((tokens.toList.take hi).drop lo) := by
  refine windowWidth_strongRecOn G (fun lo hi h_g oracle => ?_)
  obtain ⟨m, h_lo_m, h_m_hi, h_marker, h_pair⟩ := locate lo hi h_g oracle
