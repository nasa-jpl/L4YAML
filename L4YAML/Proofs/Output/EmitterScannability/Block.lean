...
/-- A unit entry with a content-start head has a non-separator (`≠ .flowEntry`) last token —
    the `lastNonSep` mirror of `lastNonOpener_of_entryUnit`.  A `.flowEntry` last has bracket
    delta `0` (unlike an opener's `+1`), so balance ALONE does not rule it out for a SINGLETON;
    there the content-start head does the work (head = last, and a content-start `.scalar`/`[`/`{`
...
theorem lastNonSep_of_entryUnit_contentHead (e : List (Positioned YamlToken))
    ...
    ∀ (hla : 0 < e.length),
      (e[e.length - 1]'(Nat.sub_lt hla Nat.one_pos)).val ≠ .flowEntry := by
  intro hla hsep
  obtain ⟨h_bal, h_prefix⟩ := h_eu
  obtain ⟨hne, hcs⟩ := h_head
  rcases Nat.lt_or_ge 1 e.length with hge2 | hle1
  · -- length ≥ 2: the proper prefix at `length-1` has balance `≥ 1`, but total balance is `0`
    have hgl : e.getLast hne = e[e.length - 1]'(Nat.sub_lt hla Nat.one_pos) :=
      List.getLast_eq_getElem hne
    have hsep' : (e.getLast hne).val = .flowEntry := by rw [hgl]; exact hsep
    ...
    have hdelta : flowBracketDelta (e.getLast hne).val = 0 := by
      rw [hsep']; exact flowBracketDelta_flowEntry
    have hp := h_prefix (e.length - 1) (by omega) (by omega)
    rw [hdelta] at hsplit
    omega
  · -- length = 1: last = head, a content-start, never a `.flowEntry`
    have h1 : e.length = 1 := by omega
    have hidx : e.length - 1 = 0 := by omega
    have hval_eq : (e.head hne).val
        = (e[e.length - 1]'(Nat.sub_lt hla Nat.one_pos)).val := by
      rw [List.head_eq_getElem hne]; simp only [hidx]
    rw [hval_eq, hsep] at hcs
    simp [ContentStartTok] at hcs
...
theorem emitList_scans_block_empty : EmitListScansInFlowBlock [] := by
  ...
  exact ⟨0, s, [], .zero, hcorr, rfl, rfl, rfl, rfl, h_col, h_flow, h_indent, rfl,
    ...
    (by intro hla; simp at hla), (by intro h0; simp at h0), SepAdj_nil⟩
  ...
  | cons v tail ih =>
    ...
    | v' :: vs, ih =>
      ...
      obtain ⟨n₃, s_end, block_rest, h_chain₃, h_corr_end, h_fl_end, h_dp_end, h_ids_end,
              h_ek_end, h_col_end, h_flow_end, h_indent_end, h_line_end, h_atol_end, h_endline_end, h_stack_end, h_fmc₃, h_block_eq_end, h_wb_rest, h_wt_rest, _h_head_rest, h_tail_rest, h_oa_rest, h_lns_rest, h_keyhead_rest, h_sa_rest⟩ :=
      ...
      refine ⟨n₁ + 1 + (n₃' + 1), s_end, block₁ ++ [feTok] ++ block_rest,
        ...
        ?_, h_atol_end, h_endline_end, ?_, h_arith ▸ h_fmc_all, ?_, ?_, ?_, ?_, ?_, ?_, ?_, ?_, ?_⟩
      ...
      · -- WellBracketed (block₁ ++ [feTok] ++ block_rest)
        exact WellBracketed_append _ _
          ...
          h_wb_rest
      · -- WellTyped (block₁ ++ [feTok] ++ block_rest) — typed twin of the WellBracketed glue
      ...
      · -- OpenerAdj (block₁ ++ [feTok] ++ block_rest): the R403 seam glue
        exact OpenerAdj_seam block₁ block_rest feTok h_oa₁ h_oa_rest
          (by rw [h_feTok_val]; decide) h_tail₁
      · -- lastNonSep (block₁ ++ [feTok] ++ block_rest): block_rest ≠ [] (R423: chain grew ≥ 1 step)
      ...
      · -- field (iii) SepAdj (block₁ ++ [feTok] ++ block_rest): the R421 seam glue — the `.flowEntry`
        -- separator IS the trigger, so its successor (block_rest's head) must be a content start
        -- (h_keyhead_rest = field (ii) of block_rest); block₁'s tail is not a separator (h_lns₁)
        exact SepAdj_seam block₁ block_rest feTok h_sa₁ h_sa_rest h_feTok_val h_keyhead_rest h_lns₁
...
theorem emitPairList_scans_block_empty : EmitPairListScansInFlowBlock [] := by
  ...
  exact ⟨0, s, [], .zero, hcorr, rfl, rfl, rfl, rfl, h_col, h_flow, h_indent, rfl,
    ...
    (by intro h0; simp at h0), SepAdj_nil⟩
  ...
  | cons p tail ih =>
    ...
    | [], _ =>
      ...
      refine ⟨n₁ + 1 + (n_v' + 1), s_end,
        ...
        h_arith ▸ h_chain_all, h_corr_end, ?_, ?_, ?_, ?_, h_col_end, h_flow_end, h_indent_end,
        ?_, h_atol_end, h_endline_end, ?_, h_arith ▸ h_fmc_all, h_block_end, ?_, ?_, ?_, ?_, ?_, ?_, ?_⟩
      · rw [h_fl_end, h_fl₃, h_fl₂, h_fl₁]
      · rw [h_dp_end, h_dp₃, h_dp₂, h_dp₁]
      · rw [h_ids_end, h_ids₃, h_ids₂, h_ids₁]
      ...
      · -- WellTyped ((.key :: block_k ++ [.value]) ++ block_v) — typed twin
        exact WellTyped_append _ _
          (WellTyped_cons_delta_zero _ _ flowBracketDelta_key
            (WellTyped_append _ _ h_wt_k
              (WellTyped_singleton_delta_zero _ (by rfl))))
          h_wt_v
      · -- tail not opener: block ends in `block_v` (the value block, non-empty by its
        -- content-start head field), whose last token is not a seq-opener
        obtain ⟨hne_v, _⟩ := h_cs_v
        exact lastNonOpener_append_right _ block_v hne_v h_last_v
      · -- OpenerAdj ((.key :: block_k ++ [.value]) ++ block_v): the R403 map-single glue
        exact OpenerAdj_map_single _ _ block_k block_v (fun h => YamlToken.noConfusion h) (fun h => YamlToken.noConfusion h)
          h_oa_k h_oa_v h_tail_k
      ...
      · -- field (iii) SepAdj ((.key :: block_k ++ [.value]) ++ block_v): the R420 map-single glue —
        -- `.key`/`.value` are NEITHER the `.flowEntry` trigger, so the verbatim clone applies
    ...
    | p' :: ps, ih =>
      ...
      obtain ⟨n_v, s_v, block_v, h_chain_v, h_corr_v, h_fl_v, h_dp_v, h_ids_v,
              ...
              h_atol_v, h_endline_v, h_stack_v, h_fmc_v, h_blockeq_v, h_wb_v, h_wt_v, _h_es_v, _h_eu_v, h_cs_v, h_btail_v, h_oa_v, h_lns_v, h_sa_v⟩ :=
      ...
      obtain ⟨n_r, s_end, block_rest, h_chain_r, h_corr_end, h_fl_end, h_dp_end, h_ids_end,
              h_ek_end, h_col_end, h_flow_end, h_indent_end, h_line_end, h_atol_end, h_endline_end, h_stack_end, h_fmc_r, h_blockeq_rest, h_wb_rest, h_wt_rest, h_tail_rest, h_oa_rest, h_lns_rest, h_keyhead_rest, h_sa_rest⟩ :=
      ...
      · -- WellBracketed (((block_kc ++ block_v) ++ [feTok]) ++ block_rest)
        exact WellBracketed_append _ _
          (WellBracketed_append _ _
            ...
            (WellBracketed_singleton_delta_zero feTok (by rw [h_feTok_val]; exact flowBracketDelta_flowEntry)))
          h_wb_rest
      · -- WellTyped (((block_kc ++ block_v) ++ [feTok]) ++ block_rest) — typed twin
        exact WellTyped_append _ _
          (WellTyped_append _ _
            ...
            (WellTyped_singleton_delta_zero feTok (by rw [h_feTok_val]; exact flowBracketDelta_flowEntry)))
          h_wt_rest
      · -- tail not opener: block ends in `block_rest` (the recursive pair-list tail) via the
        -- `Y ++ [feTok] ++ block_rest` seam; `feTok` is `.flowEntry`, `block_rest`'s tail not an opener
        exact lastNonOpener_append3 _ block_rest feTok
          (by rw [h_feTok_val]; decide) h_tail_rest
      · -- OpenerAdj (((mapSingle ++ [feTok]) ++ block_rest)): R403 seam over the map-single pair `Y`
        obtain ⟨hne_v, _⟩ := h_cs_v
        exact OpenerAdj_seam _ block_rest feTok
          (OpenerAdj_map_single _ _ block_k block_v (fun h => YamlToken.noConfusion h) (fun h => YamlToken.noConfusion h) h_oa_k h_oa_v h_tail_k)
          h_oa_rest (by rw [h_feTok_val]; decide)
          (lastNonOpener_append_right _ block_v hne_v h_btail_v)
      ...
      · -- field (ii) ≠ .key-gated head: the seam block still starts with the leading `.key` marker
        ...
        apply absurd _ hne
        simp only [List.cons_append, List.getElem_cons_zero]
      · -- field (iii) SepAdj over the `Y ++ [feTok] ++ block_rest` seam, where `Y` is the map-single
        -- pair `(.key :: block_k ++ [.value]) ++ block_v`: glue `Y`'s SepAdj (`SepAdj_map_single`) and
        -- block_rest's, supplying block_rest's head obligation (h_keyhead_rest = field ii) and `Y`'s
        -- tail-not-separator (block_v's last, h_lns_v)
        obtain ⟨hne_v, _⟩ := h_cs_v
        exact SepAdj_seam _ block_rest feTok
          (SepAdj_map_single _ _ block_k block_v (fun h => YamlToken.noConfusion h)
            (fun h => YamlToken.noConfusion h) h_sa_k h_sa_v h_lns_k)
          h_sa_rest h_feTok_val h_keyhead_rest
          (lastNonSep_append_right _ block_v hne_v h_lns_v)
