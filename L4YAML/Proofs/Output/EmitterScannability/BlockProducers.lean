  ...
  | scalar sc _ h =>
    ...
    case savedkey =>
      ...
      have h_wb_es : WellBracketed [tok] ∧ WellTyped [tok] ∧ EntrySafe [tok] ∧
          ...
          OpenerAdj [tok] ∧
          (∀ (hla : 0 < [tok].length),
            ([tok][[tok].length - 1]'(Nat.sub_lt hla Nat.one_pos)).val ≠ .flowEntry) ∧
          SepAdj [tok] :=
  ...
  | cons v tail ih =>
    ...
    | [], _ =>
      ...
      exact ⟨n, s', block, h_chain, h_corr, h_fl', h_dp, h_ids, h_ek', h_col', h_flow',
        ...
        SafeBodyUnit.single block h_cs_ne h_eu h_cs_val, h_oa_v, h_lns_v, h_sa_v⟩
    | v' :: vs, ih =>
      ...
      obtain ⟨n₃, s_end, block_rest, h_chain₃, h_corr_end, h_fl_end, h_dp_end, h_ids_end,
              h_ek_end, h_col_end, h_flow_end, h_indent_end, h_line_end, h_atol_end, h_endline_end, h_stack_end, h_fmc₃, h_block_eq_end, h_wb_rest, h_wt_rest, h_sb_rest, h_sbu_rest, h_oa_rest, h_lns_rest, h_sa_rest⟩ :=
      ...
      · -- SafeBodyUnit ContentStartTok (block₁ ++ [feTok] ++ block_rest) — unit-entry refinement
        have h_reassoc : block₁ ++ [feTok] ++ block_rest = block₁ ++ feTok :: block_rest := by
          rw [List.append_assoc]; rfl
        rw [h_reassoc]
      ...
      · -- lastNonSep (block₁ ++ [feTok] ++ block_rest): block_rest ≠ [] (R423: chain grew)
        exact lastNonSep_append_right (block₁ ++ [feTok]) block_rest
          (block_ne_nil_of_chainGrew h_chain₃ h_block_eq_end) h_lns_rest
      · -- SepAdj (block₁ ++ [feTok] ++ block_rest): R421 seam — the `.flowEntry` separator's
        -- successor (block_rest's head) is a content start, sourced from block_rest's `SafeBody`
        -- head (`SafeBody.head_Q`, gate `≠ .key` discarded); block₁'s tail is not a separator (h_lns₁)
        have h_head_rest : ∀ (h0 : 0 < block_rest.length),
            (block_rest[0]'h0).val ≠ .key → isFlowContentStart (block_rest[0]'h0).val := by
          intro h0 _
          obtain ⟨_hl, hcs⟩ := SafeBody.head_Q h_sb_rest
          exact hcs
        exact SepAdj_seam block₁ block_rest feTok h_sa₁ h_sa_rest h_feTok_val h_head_rest h_lns₁
  ...
  | cons p tail ih =>
    ...
    | [], _ =>
      ...
      refine ⟨n₁ + 1 + (n_v' + 1), s_end,
        ...
        h_arith ▸ h_chain_all, h_corr_end, ?_, ?_, ?_, ?_, h_col_end, h_flow_end, h_indent_end,
        ?_, h_atol_end, h_endline_end, ?_, h_arith ▸ h_fmc_all, h_block_end, ?_, ?_, ?_, ?_, by omega, ?_, ?_⟩
      · rw [h_fl_end, h_fl₃, h_fl₂, h_fl₁]
      · rw [h_dp_end, h_dp₃, h_dp₂, h_dp₁]
      · rw [h_ids_end, h_ids₃, h_ids₂, h_ids₁]
      ...
      · -- WellBracketed ((.key :: block_k ++ [.value]) ++ block_v): delta-`0` `.key`/`.value`
        -- glue around the key/value `WellBracketed` blocks (mirrors `h_es_entry`).
      ...
      · -- OpenerAdj ((.key :: block_k ++ [.value]) ++ block_v): R403 map-single glue, the FLAT
        -- mirror of `emitList_scans_recseqbody`/Block.lean's singleton map case.
      ...
      · -- SepAdj ((.key :: block_k ++ [.value]) ++ block_v): R420 map-single glue (`.key`/`.value`
        -- are NEITHER the `.flowEntry` trigger ⇒ verbatim clone)
    ...
    | p' :: ps, ih =>
      ...
      obtain ⟨n_v, s_v, block_v, h_chain_v, h_corr_v, h_fl_v, h_dp_v, h_ids_v,
              ...
              h_atol_v, h_endline_v, h_stack_v, h_fmc_v, h_blockeq_v, h_wb_v, h_wt_v, h_es_v, _h_eu_v, h_cs_v, h_btail_v, h_oa_v, h_lns_v, h_sa_v⟩ :=
      ...
      obtain ⟨n_r, s_end, block_rest, h_chain_r, h_corr_end, h_fl_end, h_dp_end, h_ids_end,
              h_ek_end, h_col_end, h_flow_end, h_indent_end, h_line_end, h_atol_end, h_endline_end, h_stack_end, h_fmc_r, h_blockeq_rest, h_wb_rest, h_wt_rest, h_sb_rest, h_oa_rest, h_n_r_ge3, h_lns_rest, h_sa_rest⟩ :=
      ...
      · -- OpenerAdj (((mapSingle ++ [feTok]) ++ block_rest)): R403 seam over the map-single pair,
        -- the FLAT mirror of `emitList_scans_recseqbody`/Block.lean's cons map case.
        obtain ⟨hne_v, _⟩ := h_cs_v
      ...
      · -- SepAdj over the `Y ++ [feTok] ++ block_rest` seam (Y = the map-single pair): glue Y's
        -- SepAdj (`SepAdj_map_single`) and block_rest's; block_rest's head is `.key`
        -- (`SafeBody (· = .key)`), so SepAdj_seam's `≠ .key`-gated head obligation is VACUOUS; Y's
        -- tail-not-separator is block_v's last (`h_lns_v`)
        obtain ⟨hne_v, _⟩ := h_cs_v
        have h_head_rest : ∀ (h0 : 0 < block_rest.length),
            (block_rest[0]'h0).val ≠ .key → isFlowContentStart (block_rest[0]'h0).val := by
          intro h0 hne
          obtain ⟨_hl, hkey⟩ := SafeBody.head_Q h_sb_rest
        ...
        exact SepAdj_seam _ block_rest feTok
          (SepAdj_map_single _ _ block_k block_v (fun h => YamlToken.noConfusion h)
            (fun h => YamlToken.noConfusion h) h_sa_k h_sa_v h_lns_k)
          h_sa_rest h_feTok_val h_head_rest
