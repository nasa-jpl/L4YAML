...
//!
//! ```no_run
//! use lean4yaml::{load, LimitsPreset, Kind};
//!
...
//! Call all lean4yaml functions from the thread that called `initialize()`.
...
///
/// After calling this, no further lean4yaml calls are valid.
pub fn finalize() {
    unsafe {
        lean4yaml_finalize();
    }
...
pub fn load(input: &str, preset: LimitsPreset) -> Result<YamlValue> {
    initialize();
    unsafe {
        let result = lean4yaml_parse_single(input.as_ptr().cast(), input.len(), preset.as_u8());
...
pub fn load_all(input: &str, preset: LimitsPreset) -> Result<Vec<YamlDocument>> {
    initialize();
    unsafe {
        let result = lean4yaml_parse(input.as_ptr().cast(), input.len(), preset.as_u8());
...
pub fn dump(value: &YamlValue) -> Result<String> {
    let ptr = unsafe { lean4yaml_dump(value.as_raw()) };
    if ptr.is_null() {
...
pub fn load_all_configured(input: &str, config_yaml: &str) -> Result<Vec<YamlDocument>> {
    initialize();
    unsafe {
        let result = lean4yaml_parse_configured(
            input.as_ptr().cast(),
            input.len(),
            config_yaml.as_ptr().cast(),
            config_yaml.len(),
...
unsafe fn extract_multi_result(result: *mut c_void) -> Result<Vec<YamlDocument>> {
    ...
    let docs_handle = lean4yaml_result_docs(result);
    lean4yaml_free(result);
    ...
    let count = lean4yaml_docs_count(docs_handle) as usize;
    let mut docs = Vec::with_capacity(count);
    for i in 0..count {
        let doc_handle = lean4yaml_docs_get(docs_handle, i as u32);
        docs.push(YamlDocument::from_raw(doc_handle)?);
    }
    lean4yaml_free(docs_handle);
    Ok(docs)
}

unsafe fn extract_error_message(result: *mut c_void) -> String {
    let ptr = lean4yaml_result_error_message(result);
    if ptr.is_null() {
