...
pub fn parse_limits_yaml(yaml: &str) -> Result<ParserLimitsHandle> {
    unsafe {
        let result = l4yaml_parse_limits_yaml(yaml.as_ptr().cast(), yaml.len());
        if result.is_null() {
            return Err(Error::NullHandle);
        }
        let ok = l4yaml_config_is_ok(result);
        if ok == 0 {
            let err_ptr = l4yaml_config_error_message(result);
        ...
        let limits = l4yaml_config_get_limits(result);
        l4yaml_free(result);
