...
/// object. `Drop` calls `l4yaml_free` to release it. Child values
...
impl YamlValue {
    ...
    /// one of the `l4yaml_*` C functions. The caller transfers ownership.
    ...
    pub fn kind(&self) -> Kind {
        let k = unsafe { l4yaml_value_kind(self.handle) };
        Kind::from_u8(k)
    }

    /// Scalar string content. Returns `""` for non-scalar nodes.
    pub fn as_str(&self) -> Result<&str> {
        let ptr = unsafe { l4yaml_value_string(self.handle) };
        if ptr.is_null() {
            return Ok("");
        }
    ...
    pub fn len(&self) -> usize {
        match self.kind() {
            Kind::Sequence => unsafe { l4yaml_value_seq_length(self.handle) as usize },
            Kind::Mapping => unsafe { l4yaml_value_map_length(self.handle) as usize },
            _ => 0,
        }
    }
    ...
    pub fn get(&self, key: &str) -> Option<YamlValue> {
        ...
        let h = unsafe { l4yaml_value_lookup(self.handle, key_cstr.as_ptr()) };
        if h.is_null() {
            None
        } else {
    ...
    pub fn tag(&self) -> Option<&str> {
        let ptr = unsafe { l4yaml_value_tag(self.handle) };
        if ptr.is_null() {
            return None;
        }
        let cstr = unsafe { CStr::from_ptr(ptr) };
    ...
    pub fn anchor(&self) -> Option<&str> {
        let ptr = unsafe { l4yaml_value_anchor(self.handle) };
        if ptr.is_null() {
            return None;
        }
        let cstr = unsafe { CStr::from_ptr(ptr) };
...
impl Drop for YamlValue {
    fn drop(&mut self) {
        if !self.handle.is_null() {
            unsafe { l4yaml_free(self.handle) };
        }
    }
}
...
impl fmt::Display for YamlValue {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let ptr = unsafe { l4yaml_dump(self.handle) };
        if ptr.is_null() {
            return write!(f, "<null>");
        }
        let cstr = unsafe { CStr::from_ptr(ptr) };
