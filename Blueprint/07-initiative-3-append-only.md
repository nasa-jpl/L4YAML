        ...
        - 6c-ii. **Part3-final-discharge-bridge-inversion** — decomposed
          ...
          - 6c-ii-γ. **inversion enumeration + Part (3) discharge** —
            ...
            *exhaustiveness* fact needed to invert `k → i`.  This is a
            ...
            - 6c-ii-γ-2. **discharge chain-side exhaustiveness sorrys**
              ← *current next step*: requires strengthening
              `EmitScansInFlow` with a "no outer-level `.flowEntry`
              pushed during emit body" invariant (key/value emit
              segments push only inner-level flowEntries, since
              they're nested inside `[` / `{` flow scopes).  Singleton
              case discharges via this invariant directly; cons case
              lifts via comma push (outer-level by construction) +
              IH on tail's exhaustiveness.  Sorry count: 10 → 8 (-2
              chain-side; depending on whether `EmitScansInFlow`
              strengthening adds one new sorry, may net to 9 or 8).
...
1. **2d-key-chain-Part3** (after-flowEntry splice locator; refined
   ~12–13 cadence steps; scope-investigation done 2026-05-02,
   ...
   strengthening done 2026-05-03) — Part3 is now decomposed into 13
   cadence-sized sub-steps (sub-steps 1–5 + 6a + 6b + 6c-i + 6c-ii-α
   + 6c-ii-β + 6c-ii-γ-1 ✓ done; 6c-ii-γ-2 + 6c-ii-γ-3 remaining):
