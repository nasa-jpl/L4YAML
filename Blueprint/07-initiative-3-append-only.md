          ...
          - 6c-ii-γ. **inversion enumeration + Part (3) discharge** —
            ...
            *exhaustiveness* fact needed to invert `k → i`.  This is a
            ...
            - 6c-ii-γ-2b. **discharge chain-side γ-1 sorrys using new
              conjunct** ← *current next step*: in `emitPairList_scans_
              nonempty` singleton case, decompose chain `s → s_end` into
              `emit(p.1) → ":" → ws → emit(p.2)` segments and discharge
              the outer-flowEntry exhaustiveness via the bundled
              `h_balfacts_k_singleton` and `h_balfacts_v_singleton`
              hypotheses (no comma push, so no outer-level flowEntry).
              Cons case: same per-segment decomposition + outer-level
              flowEntry at `kk = s_v.tokens.size` matches `qs[1] =
              qs_tail[0]` + IH's `_h_exh_t` for tail-range kk.  Requires
              token-prefix preservation (`FlowMonoChain_preserves_existing_
              tokens` already in tree) and a *new* helper lemma
              `flowBracketBalance_chain_extension` (balance unchanged when
              chain extends array past `hi`).  Sorry count: 13 → 11
              (-2 chain-side; helper lemma may be inline so no new sorry,
              or +1 if extracted with proof deferred).
            - 6c-ii-γ-2c. **discharge scalar case in `emit_scans_in_flow`**:
              expose `s'.tokens.size = s_state.tokens.size + 1` and
              `s'.tokens[s_state.tokens.size].val = .scalar` from
              `scanNextToken_flow_scanDoubleQuoted` (likely needs a
              stronger variant exposing token push details).  Then
              discharge balance = 0 (single non-bracket token) and
              no-outer-flowEntry (vacuous: only token is `.scalar`,
              not `.flowEntry`).  Sorry count: 11 → 10.
...
**Concrete next steps (in order, as of 2026-05-03, sorry count 13):**
...
1. **2d-key-chain-Part3** (after-flowEntry splice locator; refined
   ...
   EmitScansInFlow-strengthening done 2026-05-03) — Part3 is now
   decomposed into 15 cadence-sized sub-steps (sub-steps 1–5 + 6a + 6b
   + 6c-i + 6c-ii-α + 6c-ii-β + 6c-ii-γ-1 + 6c-ii-γ-2a ✓ done;
   6c-ii-γ-2b + 6c-ii-γ-2c + 6c-ii-γ-2d + 6c-ii-γ-3 remaining):
