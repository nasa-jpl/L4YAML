            ...
            - 6c-ii-γ-3b. **discharge Part (3) using γ-3a + γ-1**.
              ...
              **only if** `k = j_k + foldlSum 0 qs[i]`.  This size relation
              ...
              - 6c-ii-γ-3b-iii. **final discharge** (decomposed
                ...
                - **γ-3b-iii-B** (~250–400 lines, no sorry change):
                  add save-time monotonicity conjunct to
                  `EmitScansInFlow` / `EmitListScansInFlow` /
                  `EmitPairListScansInFlow`: `∀ p ≤ q (with q <
                  s'.pendingKeys.size), pks[p].insertBeforeIdx ≤
                  pks[q].insertBeforeIdx` (paralleling the kind
                  disjunctive conjunct from γ-3b-ii-predicate).
                  Discharge in all leaf sites (`saveSimpleKey` push
                  monotonicity), composition sites
                  (`emit_scans_in_flow` cases), and recursive sites
                  (`emit{,Pair}List_scans_nonempty` singleton + cons).
                  Also add `pks[r].insertBeforeIdx ≤ s'.tokens.size`
                  bound (needed by γ-3b-iii-A's precondition).
...
1. **2d-key-chain-Part3** (after-flowEntry splice locator; refined
   ...
   done 2026-05-03) — Part3 is now decomposed into 21 cadence-sized
   ...
   6c-ii-γ-3b-iii-{B,C,D} (save-time-mono predicate strengthening,
   walk-through-unresolved-keyOnly lemma, final Part (3) discharge —
   sorry 9 → 8) remaining):
