            ...
            *exhaustiveness* fact needed to invert `k → i`.  This is a
            ...
            - 6c-ii-γ-3b. **discharge Part (3) using γ-3a + γ-1**.
              ...
              **only if** `k = j_k + foldlSum 0 qs[i]`.  This size relation
              ...
              - 6c-ii-γ-3b-iii. **final discharge** (PENDING, sorry
                count 9 → 8).  Apply strengthened γ-3a (γ-3b-i) to obtain
                `(j_k, p_k, acc')` with `acc'.size = k + 1` and transport
                eq.  Apply γ-1 exhaustiveness to identify pair index
                `i ≥ 1` with `j_k + 1 = pks[qs[i]].insertBeforeIdx`.
                Walk from `(j_k+1, p_k, acc')` through any intervening
                `.unresolved` entries (kind size 0, acc.size preserved)
                to state `(j_k+1, qs[i], acc'')` with `acc''.size = k + 1`,
                using γ-3b-ii's flow-kind restriction to rule out
                `.blockMappingStartAndKey` interfering with the walk.
                Apply `linearise_splice_keyonly_at_index` at
                `(j_k+1, qs[i], acc'', k + 1)` using
                `pks[qs[i]].kind = .keyOnly` (from γ-1) and
                `pks[qs[i]].insertBeforeIdx = j_k + 1 ≤ j_k + 1`.
                Sorry count: 9 → 8.
...
1. **2d-key-chain-Part3** (after-flowEntry splice locator; refined
   ...
   done 2026-05-03) — Part3 is now decomposed into 18 cadence-sized
   sub-steps (sub-steps 1–5 + 6a + 6b + 6c-i + 6c-ii-α + 6c-ii-β +
   6c-ii-γ-1 + 6c-ii-γ-2a + 6c-ii-γ-2b + 6c-ii-γ-2c + 6c-ii-γ-2d +
   6c-ii-γ-3a + 6c-ii-γ-3b-i + 6c-ii-γ-3b-ii ✓ done;
   6c-ii-γ-2d-ii (emitPairList cons stub discharge) +
   6c-ii-γ-3b-iii (final Part (3) discharge using γ-3b-i + γ-3b-ii)
   remaining):
