   ...
   - **2d-key-chain-Part3 (after-flowEntry splice locator)**: for each
     ...
     **scope-investigated and decomposed** into a multi-cadence
     ...
     6. **Part3-final-discharge** (split into 6a + 6b — bridge sub-cadence
        ...
        - 6c-ii. **Part3-final-discharge-bridge-inversion** ← *current
          next step*: (i) build the bracket-balance inversion: for every
          outer-level flowEntry at linearise position `k`, identify the
          unique pair index `i ≥ 1` such that `k + 1 =
          pks[qs[i]].insertBeforeIdx + (pks.foldl 0 0 qs[i])`.  This
          requires accounting for the inner flow scopes within each
          value's emit and showing they contribute only inner-level
          (non-zero balance) flowEntries.  Key observation: spliced
          tokens (`.key`, `.blockMappingStart`) have `flowBracketDelta =
          0`, so flow bracket balance on linearise corresponds 1:1 with
          flow bracket balance on `s'.tokens` modulo splice-position
          shifts.  Likely route: (a) lemma
          `flowBracketBalance_linearise_eq_tokens` showing the index
          correspondence; (b) per-pair counting argument tying outer-
          level flowEntries to the qs-array predecessor positions.
          (ii) discharge the Part (3) sorry at
          `emitPairList_body_linearise_characterization` (line ~12550)
          using 6c-i's forward lemma + the inversion.  Sorry count:
          8 → 7.  Estimate: 2-3 cadence steps (the bracket-balance
          preservation lemma alone is non-trivial).
...
1. **2d-key-chain-Part3** (after-flowEntry splice locator; refined
   ~8–9 cadence steps; scope-investigation done 2026-05-02,
   ...
   Part3 is now decomposed into 9 cadence-sized sub-steps (sub-steps 1–5
   + 6a + 6b + 6c-i ✓ done; 6c-ii is the only remaining):
