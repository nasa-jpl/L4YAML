...
| J.4.2.b-2d-key-chain-Part3-final-discharge-bridge-6c-ii-γ-3b-iii-A | Decode-strengthening cadence for the J.4.2.b-2d-key Part3 cascade's sub-step 6c-ii-γ-3b-iii-A (first of γ-3b-iii's four sub-cadences after γ-3b-iii was further decomposed: γ-3b-iii-A (decode-strengthening, this cadence) + γ-3b-iii-B (save-time-mono predicate strengthening) + γ-3b-iii-C (walk-through-unresolved-keyOnly lemma) + γ-3b-iii-D (final Part (3) discharge, sorry 9 → 8)).  **Decomposition rationale:** the original γ-3b-iii plan (post-γ-3b decomposition into γ-3b-i + γ-3b-ii + γ-3b-iii) assumed walking from `(j_k+1, p_k, acc') → (j_k+1, qs[i], acc'')` through `.unresolved` entries was straightforward given γ-3b-ii's kind restriction.  Investigation revealed two missing prerequisites: (a) γ-3b-i's strengthened decode lacks a fact tying `p_k` to `j_k` (need `pks[r].insertBeforeIdx ≤ j_k` for `r < p_k` to derive `qs[i] ≥ p_k`); (b) save-time monotonicity of `pks[..].insertBeforeIdx` is required for the splice loop to advance through `[p_k, qs[i])` without blocking on an out-of-order entry — captured by `LineariseFit` in `ScannerCorrectness.lean` but not threaded through any of the three flow-context predicates.  Decomposed into 4 sub-cadences.  **Strengthening:** added a new conjunct to `linearise_outer_flowEntry_decode` (and its `_top` wrapper) — `∀ r (h_r_ge : p ≤ r) (h_r_lt : r < p_k) (h_r_pks : r < pks.size), pks[r].insertBeforeIdx ≤ j_k`.  Intrinsic to linearise.go's execution: every splice that fired during the walk to `(j_k+1, p_k, acc')` did so at cursor ≤ j_k (cursor is monotone), so `insertBeforeIdx ≤ cursor at fire time ≤ j_k`.  Added precondition `h_idx_le_tok : ∀ r < pks.size, pks[r].insertBeforeIdx ≤ tokens.size` (rules out the tail-splice case where `insertBeforeIdx` could exceed `j_k` if not bounded by `tokens.size`).  **Discharge structure:** `splice_handler` takes two new arguments — `h_e_idx_le_j : e.insertBeforeIdx ≤ j` (case 1: direct from `hsplice`; case 3 derived from `h_idx_le_tok p hp` + `j ≥ tokens.size`) and `h_e_eq : ∀ (h_p : p < pks.size), e = pks[p]'h_p` (identifies `e` with `pks[p]` so the new conjunct's `r = p` sub-case rewrites back to `pks[r]` form via proof-irrelevance).  In the recursive sub-case (k beyond extension), the splice handler's conjunct discharge case-splits on `r = p` (use `h_e_eq` + `h_e_idx_le_j` chained with `j ≤ j_k`) vs `r > p` (delegate to IH's `h_idx_inv`).  `copy_handler` adds the new conjunct via direct IH propagation (no new pks pushed).  Terminating copy step (k = acc.size) returns `p_k = p`, making `[p, p) = ∅`, vacuously discharged via `omega`.  **Top wrapper:** specialises at `(j, p, acc) = (0, 0, #[])` with the new conjunct's range `[0, p_k)`.  **Originally also planned `j_k < pks[p_k].insertBeforeIdx` (when p_k < pks.size)** but γ-3b-iii-D's discharge plan only consumes the weaker `∀ r < p_k, pks[r].insertBeforeIdx ≤ j_k` conjunct (which suffices to derive `qs[i] ≥ p_k` from γ-1's `pks[qs[i]].insertBeforeIdx = j_k + 1 > j_k`), so the second conjunct was deferred.  **Consumer destructures:** none yet (the only consumer of `linearise_outer_flowEntry_decode_top` is γ-3b-iii-D itself, which has not landed).  **Lean-tactic notes:** the `h_e_eq` parameter exploits proof-irrelevance of `Array.GetElem` witnesses — passing `fun _ => rfl` works because `pks[p]'h₁ = pks[p]'h₂` definitionally for any two `h₁ h₂ : p < pks.size`.  Refine slot count for the inner splice handler grew from 2 trailing `?_` to 3 (added new conjunct's discharge as the third bullet).  Splice handler's recursive sub-case in `linearise_outer_flowEntry_decode` had 3 bullets in original order (balance, transport, _) — I added the new conjunct as the third bullet (no `rotate_left` needed; goals appear in the order `?_` slots are listed in the `refine`).  **Sorry count unchanged at 9** (decode-strengthening only — γ-3b-iii-A is reusable infrastructure consumed in γ-3b-iii-D; the in-tree Part (3) sorry comment was updated to document γ-3b-iii-A's landing and the γ-3b-iii → γ-3b-iii-A/B/C/D decomposition).  Build green (453/453 jobs) | 0 (build clean across 453 jobs; ~70-line net change in the inversion lemma — added precondition + new conjunct + 3 discharges across splice/copy/terminating-copy cases + top-wrapper extension; no sorry change) | `Proofs/Output/EmitterScannability.lean`, `Blueprint/07-initiative-3-append-only.md` | ✓ done 2026-05-03 |
| J.4.2.b-2d-key-chain-Part3-final-discharge-bridge-6c-ii-γ-3b-ii-predicate | Predicate-level cadence for the J.4.2.b-2d-key Part3 cascade's sub-step 6c-ii-γ-3b-ii-predicate (final cadence of γ-3b-ii after foundation + leaves-A + leaves-B + leaves-C; consumes the per-leaf `kind = .unresolved` conjuncts and lifts them to the predicate-level disjunctive form `kind ∈ {.unresolved, .keyOnly}` to accommodate the colon's `.unresolved → .keyOnly` transition).  **Predicate extension:** added a uniform new-kind disjunctive conjunct of shape `∀ (q : Nat) (h_q : q < s'.pendingKeys.size) (_h_q_ge : s.pendingKeys.size ≤ q), (s'.pendingKeys[q]'h_q).kind = .unresolved ∨ (s'.pendingKeys[q]'h_q).kind = .keyOnly` to all three predicate definitions: `EmitScansInFlow`, `EmitListScansInFlow`, `EmitPairListScansInFlow`.  Positioned between the existing pkRec preservation conjunct (`∀ j < s.pks.sz, ib + kind preserved`) and the pendingKeyStack equality conjunct.  **Strong vs disjunctive form choice:** the per-leaf cadences (leaves-A/B/C) used the strict `kind = .unresolved` form because each leaf either pushes a fresh `.unresolved` (saveSimpleKey via leaves-A) or doesn't push at all (leaves-B/C with vacuous discharge).  At the predicate level, however, composition through the colon (`scanNextToken_flow_value_pkResolve`) transitions the active pending-key entry from `.unresolved` to `.keyOnly` via `setPendingKeyKind`, which would falsify the strict form for any q at the active index.  The disjunction `{.unresolved, .keyOnly}` accommodates both states without requiring colon-aware specialisation.  **Foundation helper added:** `newkind_disj_compose` (placed near `pkRec_size_compose` at line ~3690), parallel structure but for the disjunctive new-kind: takes `h_pkRec_end : pkRec preservation s_mid → s_end at j < s_mid.pks.sz` + `h_new_mid : disj newkind for s → s_mid` + `h_new_end : disj newkind for s_mid → s_end`, returns disj newkind for s → s_end.  Proof: case-split on `q < s_mid.pks.sz` (use `h_new_mid` lifted via pkRec preservation) vs `q ≥ s_mid.pks.sz` (use `h_new_end` directly).  **`emitList_scans_empty` / `emitPairList_scans_empty` discharges:** vacuous via range collapse — `s'.pendingKeys = s.pendingKeys` (hcorr unchanged through 0-step chain), so `s.pks.sz ≤ q < s.pks.sz` is empty; dispatched by `fun _ h_q h_q_ge => absurd h_q_ge (Nat.not_le_of_lt h_q)` inserted in the `exact ⟨...⟩` tuple between pendingKeyStack equality and the balance bundled conjunct.  **`emit_scans_in_flow` scalar discharge:** lifts the leaf-A `_h_newkind'` (now `h_newkind'`, renamed since it's no longer unused) to the disjunctive form via `Or.inl`: `fun q h_q h_q_ge => Or.inl (h_newkind' q h_q h_q_ge)`.  **`emit_scans_in_flow` sequence + mapping discharges:** compose three steps via `newkind_disj_compose`.  Pre-build leaf-A `[`/`{` disj-lifted via `Or.inl` (h_newkind₁_disj), leaf-B `]`/`}` disj-lifted similarly (h_newkind₃_disj).  Body's disj newkind comes from the inner predicate (h_newkind₂).  Compose: `newkind_disj_compose h_pkRec₂ h_newkind₁_disj h_newkind₂` for s → s₂, then `newkind_disj_compose h_pkRec₃ ⟨↑⟩ h_newkind₃_disj` for s → s₃.  Each adds ~14 lines.  **`emitList_scans_nonempty` cons discharge:** chain s → s₁ (key emit) → s₂ (comma leaf-B) → s₃ (ws1 leaf-C) → s_end (recursive IH).  Compose four steps via `newkind_disj_compose` triple chain.  Key emit's newkind already disj (from `EmitScansInFlow`).  Comma + ws1 vacuously disj (size preserved).  Specifically for ws1, since `_h_pks_pp₃ : s₃.pendingKeys = s₂.pendingKeys`, the disj newkind for s₂ → s₃ is `fun q h_q h_q_ge => absurd h_q_ge (Nat.not_le_of_lt (h_q rewritten via _h_pks_pp₃))`.  **`emitPairList_scans_nonempty` discharges (singleton + cons):** **direct case-split discharge** rather than `newkind_disj_compose` chain.  This is necessary because the colon (`scanNextToken_flow_value_pkResolve`) transitions kind `.unresolved → .keyOnly` at index `s.pks.sz`, which BREAKS the pkRec preservation invariant required by `newkind_disj_compose` (the helper requires kind preserved at ALL j < s_mid.pks.sz, but the colon transitions the entry at s.pks.sz).  Instead, for each q ∈ [s.pks.sz, s_end.pks.sz), case on `q < s₁.pks.sz`: (i) **pushed during key emit** — at s₁, kind ∈ disj from `_h_newkind_k`; lift to s_end via sub-case `q = s.pks.sz` (transitioned to `.keyOnly` by colon, kind at s_end = `.keyOnly` directly from h_kd_s_end already in tree) vs `q ≠ s.pks.sz` (kind preserved through colon via `h_pks_other_pk`, through ws1 via `_h_pks_pp₃`, through value emit via `_h_pkRec₃`, through comma via `_h_pkRec_c`, through ws1_pp via `_h_pks_pp_pks`, through IH via `h_pkRec_r`); (ii) **q ≥ s₁.pks.sz** — for singleton, q ≥ s₃.pks.sz (since size preserved by colon + ws1) and use `_h_newkind_v` directly; for cons, sub-case on whether q < s_v.pks.sz (pushed during value emit, use `_h_newkind_v`), q ∈ [s_v.pks.sz, s_c.pks.sz) (pushed during comma, use `_h_newkind_c` which is `.unresolved`, lift to disj via `Or.inl`), or q ≥ s_pp.pks.sz (pushed during recursive IH, use `h_newkind_r`).  Each lifts kind through subsequent pkRec-preserving steps.  **Singleton:** ~30 lines.  **Cons:** ~80 lines (more sub-cases due to the longer chain through comma + ws1_pp + recursive IH).  **Leaves-A coverage extension:** `scanNextToken_flow_open_mapping_nested` was missing the new-kind conjunct (originally γ-3b-ii-leaves-A only covered `scanNextToken_flow_scanDoubleQuoted` + `scanNextToken_flow_open_nested`).  Added the conjunct + discharge (mirrors the seq-open's discharge: chain `scanFlowMappingStart_preserves_pendingKeys` + `s_ad.pendingKeys = (saveSimpleKey s).pendingKeys` to obtain `s'.pendingKeys = (saveSimpleKey s).pendingKeys`, bridge via `congr 1`, dispatch via `saveSimpleKey_new_kind_unresolved`).  Refine slot count grew from 11 to 12 to accommodate the new bullet.  **Consumer destructure binders updated (12 sites):** `emitList_scans_nonempty` cons (h_ev rename `h_pks_v` → `h_newkind₁` + new `h_pks_v`; h_ih_list rename `h_pks_end` → `h_newkind_tail` + new `h_pks_end`); comma destructure rename `_h_newkind₂` → `h_newkind₂`; `emitPairList_scans_nonempty` singleton (h_ek_key + h_ev each get `_h_newkind_k`/`_h_newkind_v`); cons (h_ek_key + h_ev + h_ih_list each get `_h_newkind_k`/`_h_newkind_v`/`h_newkind_r`); `emit_scans_in_flow` seq (open `_h_newkind₁` → `h_newkind₁`; body adds `h_newkind₂`; close `_h_newkind₃` → `h_newkind₃`); map (open adds `h_newkind₁`; body adds `h_newkind₂`; close `_h_newkind₃` → `h_newkind₃`); `emitPairList_body_filtered_characterization` adds `_h_newkind`.  **Refine slot updates (each refine that supplies the predicate's existential gains one `?_` slot for newkind):** `emit_scans_in_flow` scalar (1 → 2 trailing `?_`), seq (1 → 2), map (1 → 2); `emitList_scans_nonempty` singleton + cons (2 → 3 trailing `?_`); `emitPairList_scans_nonempty` singleton + cons inner refine (4 → 5 slots after the size+pkRec head).  **Lean-tactic notes:** the colon's `h_size_pk` direction is `s₂.pendingKeys.size = s₁.pendingKeys.size` (NOT s₁→s₂), so use `h_size_pk.symm` to get `s₁.pendingKeys.size = s₂.pendingKeys.size`.  `▸` chaining over multiple equalities in a single line failed for `s₃.size ≤ q` from `s₁.size ≤ q` — use `by rw [← h_eq_s2_s3, ← h_eq_s1_s2]; exact h_q_ge_s1` instead.  For the foundation helper, the unused parameter `h_size_mid` was removed (Lean linter warning) — the helper only needs `h_pkRec_end + h_new_mid + h_new_end`.  In `emitPairList_scans_nonempty` cons, `_h_newkind_c` is the comma's leaf-B `.unresolved`-form newkind; I used `Or.inl h_kd_unres` to lift to disj via `left; rw [...]; exact h_kd_unres`.  **Sorry count unchanged at 9** (predicate-level threading only — no Part (3) sorry site touched; γ-3b-iii will consume γ-3b-ii's predicate-level conjunct alongside γ-3b-i + γ-1 to discharge the final Part (3) sorry).  Build green (453/453 jobs).  **Cascade after γ-3b-ii-predicate:** 9 → 8 (γ-3b-iii: final Part (3) discharge using γ-3b-i + γ-3b-ii + γ-1). | 0 (build clean across 453 jobs; ~250-line net change spread across 3 predicate definitions + 1 foundation helper + 4 vacuous discharges + 5 composition discharges + 1 leaves-A coverage extension + 12 consumer destructure updates; no sorry change) | `Proofs/Output/EmitterScannability.lean`, `Blueprint/07-initiative-3-append-only.md` | ✓ done 2026-05-03 |
| J.4.2.b-2d-key-chain-Part3-final-discharge-bridge-6c-ii-γ-3b-ii-leaves-C | Per-leaf cadence for the J.4.2.b-2d-key Part3 cascade's sub-step 6c-ii-γ-3b-ii-leaves-C (third and final of γ-3b-ii's leaf-extension cadences; covers the colon-resolve and ws1-preprocess leaves whose new-kind conjunct is vacuously discharged via size preservation).  **Predicate extension:** added the same uniform "new pendingKey entries kind = .unresolved" universal conjunct of shape `∀ (q : Nat) (h_q : q < s'.pendingKeys.size) (_h_q_ge : s.pendingKeys.size ≤ q), (s'.pendingKeys[q]'h_q).kind = .unresolved` (introduced in γ-3b-ii-leaves-A and propagated in leaves-B) to two additional leaf-helper signatures: (a) `scanNextToken_flow_value_pkResolve` (the `:` value-resolution helper that overwrites a pre-existing `.unresolved` entry to `.keyOnly` via `setPendingKeyKind`), (b) `scanNextToken_preprocess_flow_ws1` (single leading-space preprocessing helper that absorbs the space via `skipToContent`).  **Conjunct placement:** for value_pkResolve, inserted between the per-existing-entry preservation `∀ j ≠ i → s'.pendingKeys[j] = s.pendingKeys[j]` and the `pendingKeyStack = s.pendingKeyStack` conjunct (the `?_` slot count in the closing `refine` grew from 4 to 5); for ws1 (which uses `exact ⟨...⟩` rather than `refine`), inserted as a new term between `s₁.pendingKeys = s.pendingKeys` and `s₁.pendingKeyStack = s.pendingKeyStack`.  **Discharge approach:** both vacuous because `s'.pendingKeys.size = s.pendingKeys.size`.  At the colon, `setPendingKeyKind` reduces to `Array.setIfInBounds` at index `i = s.pendingKeyActive`, which preserves size — the existing `h_pks_full : s'.pendingKeys = s.pendingKeys.setIfInBounds i ...` already in tree gives this directly via `Array.size_setIfInBounds`.  In ws1, `skipToContent_preserves_pendingKeys` gives the equation `s₁.pendingKeys = s.pendingKeys` (full equality, not just size).  In both cases, after rewriting `h_q : q < s'.pendingKeys.size` to `h_q : q < s.pendingKeys.size`, the hypotheses `h_q` and `h_q_ge : s.pendingKeys.size ≤ q` contradict, dispatched via `exact absurd h_q_ge (Nat.not_le_of_lt h_q)`.  Unlike leaves-A (with kind-touching `setPendingKeyEndLine` wrap requiring `setPendingKeyEndLine_kind`) and leaves-B (with chain through `_preserves_pendingKeys` helpers + foundation helper `saveSimpleKey_new_kind_unresolved`), leaves-C's discharge does not invoke the foundation helper at all — the size-preservation argument alone collapses the universal quantifier's domain.  **Refine slot updates:** added one `?_` slot to value_pkResolve's top-level `refine ⟨..., ?_, ?_, ?_, ?_⟩` (4 slots → 5 slots) between the `∀ j ≠ i` per-entry slot and the `pendingKeyStack` slot; ws1's `exact ⟨...⟩` form takes a new term `fun q h_q h_q_ge => by rw [skipToContent_preserves_pendingKeys s s₁ h_stc_ok] at h_q; exact absurd h_q_ge (Nat.not_le_of_lt h_q)` between the pendingKeys equality and the pendingKeyStack equality.  **Consumer destructures:** updated 6 sites that pattern-match the leaves' existential — 4 ws1 sites (line ~9265 in `emitList_scans_nonempty` cons post-comma, line ~10388 in `emitPairList_scans_nonempty` value between key and value, line ~10868 in `emitPairList_scans_nonempty` cons between key and value, line ~10961 in `emitPairList_scans_nonempty` cons post-comma — the first three bind `_h_newkind_pp₃` between `_h_pks_pp₃` and `h_pks_pp₃`; the fourth at ~10961 binds `_h_newkind_pp` between `_h_pks_pp_pks` and `_h_pks_pp_stk`) plus 2 value_pkResolve sites (line ~10484 in `emitPairList_scans_nonempty` value, line ~11082 in `emitPairList_scans_nonempty` cons; both bind `_h_newkind_pkr` between `h_pks_other_pk` and `h_pks_pk_pkr`).  All six bindings are placeholder underscores until γ-3b-ii-predicate composes them at the predicate level.  **Lean-tactic notes:** for value_pkResolve's `setIfInBounds`-based discharge, do not invoke any chain helper — `rw [h_pks_full, Array.size_setIfInBounds] at h_q` reduces directly to a `Nat`-level contradiction with `h_q_ge`, dispatched by `Nat.not_le_of_lt`.  For ws1's discharge, inline the `skipToContent_preserves_pendingKeys` rewrite term inside the lambda since the existing tuple body uses no `have` bindings before the return — keeps the proof local to the conjunct site.  Both discharges are 3-line stubs.  **Sorry count unchanged at 9** (per-leaf strengthening only — no predicate or composition site touched; this is the last γ-3b-ii leaf cadence before γ-3b-ii-predicate consumes the per-leaf conjuncts at the predicate level).  Build green (453/453 jobs) | 0 (build clean across 453 jobs; ~30-line net addition for the 2 leaf signatures + discharge bodies + 6 consumer destructure binders; no sorry change) | `Proofs/Output/EmitterScannability.lean`, `Blueprint/07-initiative-3-append-only.md` | ✓ done 2026-05-03 |
| J.4.2.b-2d-key-chain-Part3-final-discharge-bridge-6c-ii-γ-3b-ii-leaves-B | Per-leaf cadence for the J.4.2.b-2d-key Part3 cascade's sub-step 6c-ii-γ-3b-ii-leaves-B (second of γ-3b-ii's three leaf-extension cadences; covers the close-side and comma leaves at flow-context exit/separator).  **Predicate extension:** added the same uniform "new pendingKey entries kind = .unresolved" universal conjunct of shape `∀ (q : Nat) (h_q : q < s'.pendingKeys.size) (_h_q_ge : s.pendingKeys.size ≤ q), (s'.pendingKeys[q]'h_q).kind = .unresolved` (introduced in γ-3b-ii-leaves-A) to three additional leaf-helper signatures: (a) `scanNextToken_flow_close_seq_nested` (`]` close-flow-sequence leaf), (b) `scanNextToken_flow_close_mapping_nested` (`}` close-flow-mapping leaf), (c) `scanNextToken_flow_comma` (`,` flow-entry separator).  **Conjunct placement:** inserted between the existing `pkRec preserved at j < initial size` conjunct and the close-side restore conjuncts (`s'.simpleKey = s.simpleKeyStack.back?.getD {}` etc.) for the close-side leaves; for comma, inserted between `pkRec preserved` and `s'.pendingKeyStack = s.pendingKeyStack`.  **Discharge approach:** identical pattern to leaves-A — chain the leaf's `_preserves_pendingKeys` helper (`ScannerCorrectness.scanFlowSequenceEnd_preserves_pendingKeys`, `ScannerCorrectness.scanFlowMappingEnd_preserves_pendingKeys`, or for comma the composition `advance_preserves_pendingKeys + emit_preserves_pendingKeys`) with `s_ad.pendingKeys = (saveSimpleKey s).pendingKeys` (the allowDirectives if-update preserves pendingKeys via `simp only [s_ad]; split <;> rfl`) to obtain `s'.pendingKeys = (saveSimpleKey s).pendingKeys`.  Bridge `s'.pendingKeys[q] = (saveSimpleKey s).pendingKeys[q]` via `congr 1`, then dispatch via the foundation helper `saveSimpleKey_new_kind_unresolved`.  Unlike leaves-A's regular scalar (which has a `simpleKey.possible = true` branch with `setPendingKeyEndLine` wrap requiring `setPendingKeyEndLine_kind`), the close-side and comma leaves do NOT enter `dispatchContent` — their underlying scanners (`scanFlowSequenceEnd`, `scanFlowMappingEnd`, `scanFlowEntry` via `s_ad.emit + advance + record-update`) preserve pendingKeys directly without any kind-touching wrap, so the discharge is the simpler "no-wrap" form.  **Refine slot updates:** added one `?_` slot to each of the three theorems' top-level `refine ⟨...⟩` between the pkRec-preserved slot and the next conjunct slot (close-side: between pkRec and simpleKey-restored; comma: between pkRec and pendingKeyStack).  **Consumer destructures:** updated 4 sites that pattern-match the leaves' existential — (a) line ~9166 in `emitList_scans_nonempty` cons (binds `_h_newkind₂` between `h_pkRec₂` and `h_pks₂`); (b) line ~10874 in `emitPairList_scans_nonempty` cons (binds `_h_newkind_c` between `_h_pkRec_c` and `_h_pks_c`); (c) line ~11785 in `emit_scans_in_flow` sequence case (binds `_h_newkind₃` between `h_pkRec₃` and `_h_sk_restore₃`); (d) line ~12064 in `emit_scans_in_flow` mapping case (binds `_h_newkind₃` between `h_pkRec₃` and `_h_sk_restore₃`).  All four bindings are placeholder underscores until γ-3b-ii-predicate composes them at the predicate level.  **Lean-tactic notes:** the comma's discharge does NOT use a per-leaf `_preserves_pendingKeys` helper directly — instead it derives the pendingKeys equation inline by `show (s_ad.emit .flowEntry).advance.pendingKeys = (saveSimpleKey s).pendingKeys; rw [advance_preserves_pendingKeys, emit_preserves_pendingKeys]; show s_ad.pendingKeys = (saveSimpleKey s).pendingKeys; simp only [s_ad]; split <;> rfl`.  This pattern mirrors the existing pkRec-preservation discharge in the same theorem (which also derives the pendingKeys equation inline) — adding a top-level `scanFlowEntry_pendingKeys`-style helper would be redundant with the in-place chain.  **Sorry count unchanged at 9** (per-leaf strengthening only — no predicate or composition site touched).  Build green (453/453 jobs) | 0 (build clean across 453 jobs; ~140-line net addition spread across signatures + discharge bodies + 4 consumer destructure binders; no sorry change) | `Proofs/Output/EmitterScannability.lean`, `Blueprint/07-initiative-3-append-only.md` | ✓ done 2026-05-03 |
| J.4.2.b-2d-key-chain-Part3-final-discharge-bridge-6c-ii-γ-3b-ii-leaves-A | Per-leaf cadence for the J.4.2.b-2d-key Part3 cascade's sub-step 6c-ii-γ-3b-ii-leaves-A (first of γ-3b-ii's three leaf-extension cadences; covers the scalar leaf and the `[`/open-flow-sequence leaf at flow-context entry).  **Predicate extension:** added a uniform "new pendingKey entries kind = .unresolved" universal conjunct of shape `∀ (q : Nat) (h_q : q < s'.pendingKeys.size) (_h_q_ge : s.pendingKeys.size ≤ q), (s'.pendingKeys[q]'h_q).kind = .unresolved` to three leaf-helper signatures: (a) `scanNextToken_flow_scanDoubleQuoted` (regular scalar leaf), (b) `scanNextToken_flow_scanDoubleQuoted_pkPush` (gated scalar variant), (c) `scanNextToken_flow_open_nested` (`[` flow-sequence leaf).  Inserted between the existing `pkRec preserved at j < initial size` conjunct and the `pendingKeyStack.pop = s.pendingKeyStack` conjunct (or at the end for `_pkPush` which has no pendingKeyStack conjunct).  **Discharge approach:** in each leaf, the per-step pendingKeys equation chains `s_dq.pendingKeys = s_ad.pendingKeys = (saveSimpleKey s).pendingKeys` (regular scalar) or `(scanFlowSequenceStart s_ad).pendingKeys = s_ad.pendingKeys = (saveSimpleKey s).pendingKeys` (open `[`).  The `simpleKey.possible = true` branch of regular scalar adds `setPendingKeyEndLine` on top, which is kind-preserving via `ScannerCorrectness.setPendingKeyEndLine_kind`.  The new universal conjunct at every leaf reduces via the chain to `(saveSimpleKey s).pendingKeys[q].kind = .unresolved` for `q ≥ s.pendingKeys.size`, discharged by `saveSimpleKey_new_kind_unresolved` (foundation helper from γ-3b-ii-foundation).  Bridging dependent array indices through the equation uses `congr 1`.  **Inner `h_content` lemma signatures (regular scalar):** the new conjunct propagates through `scanNextToken_flow_scanDoubleQuoted`'s inner `h_content` 19-conjunct existential and is bound at the outer destructure as `h_newkind_f`; the 5-`?_` refines in both `cases h_skp` branches grew to 6-`?_`.  Similarly for `_pkPush`'s 5-`?_` h_content refine, which grew to 6-`?_` with the new conjunct at the end.  **Consumer destructures:** updated 2 sites that pattern-match the leaves' existential — (a) line ~11626 in `emit_scans_in_flow`'s scalar case (binds `_h_newkind'`; not yet consumed since γ-3b-ii-predicate has not landed), (b) line ~11685 in `emit_scans_in_flow`'s sequence case (binds `_h_newkind₁`).  Both bindings are placeholder underscores until γ-3b-ii-predicate composes them at the predicate level.  **`scanNextToken_flow_open_nested_pkPush` not strengthened:** its existing pkPush-specific conjuncts (size = +1, ⟨h_lt, ib, kind⟩ for index `s.pendingKeys.size`) already imply the universal form trivially; downstream callers can derive it without an explicit conjunct.  **Lean-tactic notes:** for the `simpleKey.possible = true` branch of regular scalar, the goal pendingKeys are `setPendingKeyEndLine s_dq.pendingKeys s_dq.pendingKeyActive s_dq.line` after the wrap; needed `ScannerCorrectness.setPendingKeyEndLine_kind` (kind preserved) followed by chain-bridging `s_dq.pendingKeys[q]'h_dq_lt = (saveSimpleKey s).pendingKeys[q]'h_q_sk` via `congr 1`.  In `_pkPush`, since `s_dq.pendingKeys.size = s.pendingKeys.size + 1`, the `q ≥ s.pendingKeys.size` + `q < s_dq.pendingKeys.size` constraints force `q = s.pendingKeys.size` via `omega`, then the `Array.getElem_push_eq` discharges directly after `subst h_q_eq` and `dif_neg (Nat.lt_irrefl _)`.  **Sorry count unchanged at 9** (per-leaf strengthening only — no predicate or composition site touched).  Build green (453/453 jobs) | 0 (build clean across 453 jobs; ~140-line net addition spread across signatures + discharge bodies + 2 consumer destructure binders; no sorry change) | `Proofs/Output/EmitterScannability.lean`, `Blueprint/07-initiative-3-append-only.md` | ✓ done 2026-05-03 |
| J.4.2.b-2d-key-chain-Part3-final-discharge-bridge-6c-ii-γ-3b-ii-foundation | Foundation cadence for the J.4.2.b-2d-key Part3 cascade's sub-step 6c-ii-γ-3b-ii (chain-side flow-context kind restriction, further decomposed at this cadence into γ-3b-ii-foundation (this cadence) + γ-3b-ii-leaves-A/B/C + γ-3b-ii-predicate).  **Decomposition rationale (2026-05-04):** the original γ-3b-ii plan estimated ~150 lines for "predicate def + ~6 leaf discharge sites".  In practice each `scanNextToken_flow_*` helper has an internal `h_content` lemma (~70 lines) with two-branch case split (`simpleKey.possible = true/false` after content scan), and the new-kind conjunct needs discharge in each branch.  The internal preservation chain `s → saveSimpleKey s → s_ad → s_dq → s_final` must be threaded through `setPendingKeyEndLine` (kind-preserving in the skp=true branch) on top of the `saveSimpleKey` push fact.  Realistic scope is ~250-400 lines.  Decomposed into γ-3b-ii-foundation (this cadence: reusable helper) + γ-3b-ii-leaves-A (scalar + open: ~120 lines) + γ-3b-ii-leaves-B (close-seq + close-map + comma: ~100 lines) + γ-3b-ii-leaves-C (value-pkResolve + ws1: ~30 lines, both vacuous) + γ-3b-ii-predicate (predicate definitions + composition discharge: ~120 lines).  **Foundation contribution:** new theorem `saveSimpleKey_new_kind_unresolved : ∀ q (h_q : q < (saveSimpleKey s).pendingKeys.size) (_h_q_ge : s.pendingKeys.size ≤ q), ((saveSimpleKey s).pendingKeys[q]'h_q).kind = .unresolved`.  **Proof structure:** explicit decomposition of `saveSimpleKey` into push-or-id branches via `unfold + split + split` returning `Or (saveSimpleKey s).pendingKeys = s.pendingKeys` or `Or (saveSimpleKey s).pendingKeys = s.pendingKeys.push ⟨insertBeforeIdx, pos, endLine, .unresolved⟩`.  Identity branches: `q < s.pendingKeys.size` contradicts `s.pendingKeys.size ≤ q`.  Push branch: `q < s.pendingKeys.size + 1 ∧ q ≥ s.pendingKeys.size` forces `q = s.pendingKeys.size`, the freshly-pushed entry, with kind `.unresolved` by `Array.getElem_push_eq`.  **Lean-tactic notes:** `congr 1` bridges `((saveSimpleKey s).pendingKeys[q]'h_q)` and `((s.pendingKeys.push _)[q]'h_q')` across the equation `(saveSimpleKey s).pendingKeys = s.pendingKeys.push _`, since the underlying equality is on the array (not the index).  `subst h_q_eq` after the bridge lets `Array.getElem_push_eq` discharge the goal directly.  **Sorry count unchanged at 9** (foundation is reusable infrastructure consumed in γ-3b-ii-leaves; no predicate or composition site touched).  Build green (453/453 jobs) | 0 (build clean across 453 jobs; ~40-line net change — single helper theorem with manifest+blueprint update; no sorry change) | `Proofs/Output/EmitterScannability.lean`, `Blueprint/07-initiative-3-append-only.md` | ✓ done 2026-05-04 |
| J.4.2.b-2d-key-chain-Part3-final-discharge-bridge-6c-ii-γ-3b-i | Inversion-lemma walk-state strengthening cadence for the J.4.2.b-2d-key Part3 cascade's sub-step 6c-ii-γ-3b-i (first of γ-3b's three sub-steps after γ-3b was further decomposed: γ-3b-i (inversion-lemma walk-state strengthening, this cadence) + γ-3b-ii (chain-side flow-context kind restriction) + γ-3b-iii (final Part (3) discharge using γ-3b-i + γ-3b-ii + γ-1 exhaustiveness)).  **Decomposition rationale:** the original γ-3b plan called for walking from `(0, 0, #[])` to `(pks[qs[i]].insertBeforeIdx, qs[i], acc_witness)` via `linearise_go_walk_eq_top` and reading `acc_witness.size = k + 1` via `linearise_go_walk_size`.  But the walk-size formula gives `acc_witness.size = pks[qs[i]].insertBeforeIdx + foldlSum 0 qs[i] = (j_k + 1) + foldlSum 0 qs[i]`, which equals `k + 1` only if the size relation `k = j_k + foldlSum 0 qs[i]` holds.  This relation is NOT a corollary of γ-3a's bracket-balance equation `flowBracketBalance lin 0 k = flowBracketBalance tokens 0 j_k` — bracket balance and array size are independent invariants.  The relation IS implicit in `linearise.go`'s walk: when the copy step pushes `tokens[j_k] = .flowEntry` to `linearise` at position `k`, the just-after-copy state `(j_k+1, p_k, acc')` has `acc'.size = k + 1` by construction (one push extends acc by 1).  Exposing this state from γ-3a's recursive proof gives the size relation directly, bypassing the need for chain-side strict-monotonicity reasoning on `pks[r].insertBeforeIdx` values.  **Strengthening:** added a fourth conjunct to `linearise_outer_flowEntry_decode` (and its `_top` wrapper) — `∃ (p_k : Nat) (acc' : Array (Positioned YamlToken)), p ≤ p_k ∧ p_k ≤ pks.size ∧ linearise.go tokens pks j p acc = linearise.go tokens pks (j_k + 1) p_k acc' ∧ acc'.size = k + 1`.  The conjunct pin-points the walk state immediately after the copy step that pushed `tokens[j_k] = .flowEntry`.  Added precondition `p ≤ pks.size` (preserved through splice/copy steps; both call sites — the top wrapper and the recursive ih — supply it trivially).  **Splice handler update:** receives `h_p1_le : p + 1 ≤ pks.size` (derived from `hp : p < pks.size` at the dispatch site) and threads through the IH; in the recursive case, composes the IH's transport equation with `h_step` via `rw [h_step]`.  **Copy handler update:** in the recursive case (k > acc.size), passes the IH's witnesses through; in the terminating case (k = acc.size), constructs `(p_k, acc') = (p, acc.push tokens[j])` directly — `p ≤ p` trivially, `acc'.size = acc.size + 1 = k + 1` via `Array.size_push`, transport equation IS exactly `h_step`.  **Top wrapper:** specialises at `(j, p, acc) = (0, 0, #[])` with `p_k ≤ pks.size` plumbed from the inner conjunct (the `p ≤ p_k` conjunct is dropped since `0 ≤ p_k` is trivial); the transport equation specialises to `linearise tokens pks = linearise.go tokens pks (j_k + 1) p_k acc'` via `linearise = linearise.go ... 0 0 #[]` (rfl).  **Sorry count unchanged at 9** (γ-3b-i is reusable infrastructure consumed in γ-3b-iii; the in-tree Part (3) sorry comment was updated to document γ-3b-i's landing and the γ-3b → γ-3b-i/γ-3b-ii/γ-3b-iii decomposition).  Build green (453/453 jobs) | 0 (build clean across 453 jobs; ~30-line net change in the inversion lemma — added 4-conjunct return shape + walk-state construction in 3 case branches + precondition plumbing + top-wrapper extension; no sorry change) | `Proofs/Output/EmitterScannability.lean`, `Blueprint/07-initiative-3-append-only.md` | ✓ done 2026-05-04 |
| J.4.2.b-2d-key-chain-Part3-final-discharge-bridge-6c-ii-γ-3a | Linearise outer-flowEntry inversion lemma cadence for the J.4.2.b-2d-key Part3 cascade's sub-step 6c-ii-γ-3a (decomposes γ-3 into γ-3a (linearise-side inversion lemma now) + γ-3b (consume γ-3a + γ-1 exhaustiveness in `emitPairList_body_linearise_characterization` Part (3) discharge next)).  **Goal:** add a reusable inversion lemma `linearise_outer_flowEntry_decode` (the inverse direction of `linearise_walk_at_kth_predecessor_token`): given `(linearise.go tokens pks j p acc)[k] = .flowEntry` for `k ≥ acc.size`, produce a token cursor `j_k ≥ j` with `tokens[j_k] = .flowEntry` and the bracket-balance translation equation `flowBracketBalance lin acc.size k = flowBracketBalance tokens j j_k`.  **New theorems (~270 lines total) added to `Proofs/Output/EmitterScannability.lean` (right after 6c-ii-β's `linearise_go_walk_flowBracketBalance_top`):** (a) `linearise_outer_flowEntry_decode` — the main lemma, by induction on the lex-measure `n = (tokens.size - j) + (pks.size - p)`.  **Sub-proof structure:** factored via two inline `have` handlers in the `succ` case before the main 5-way case-split on linearise.go's dispatch.  **`splice_handler`** (covers cases 1+3: `p < pks.size ∧ insertBeforeIdx ≤ j` and `p < pks.size ∧ ¬insertBeforeIdx ≤ j ∧ ¬j < tokens.size`) takes the step equation `linearise.go ... = linearise.go ... j (p+1) (acc ++ expandKind e)` and either: (i) refutes (k in extension) via `expandKind_val_neutral` — splice tokens are `.key`/`.blockMappingStart`, both contradicted via `YamlToken.noConfusion`; or (ii) recurses (k beyond extension) via IH, then composes balance `[acc.size, k]` = `[acc.size, (acc ++ expandKind e).size]` (zero, via `linearise_go_eq_acc_append` + `flowBracketBalance_append_left` + `flowBracketBalance_splice_unchanged`) + IH's `[(acc ++ expandKind e).size, k]` value.  **`copy_handler`** (covers cases 2+4: `j < tokens.size ∧ ¬splice` and `p ≥ pks.size ∧ j < tokens.size`) takes the step equation `linearise.go ... = linearise.go ... (j+1) p (acc.push tokens[j])` and either: (i) at `k = acc.size`, reads `linearise[acc.size] = (acc.push tokens[j])[acc.size] = tokens[j]` via `linearise_go_getElem_lt_acc` + `Array.getElem_push_eq`, sets `j_k := j`, balance trivially `0 = 0`; or (ii) at `k > acc.size`, recurses via IH, then composes balance with `flowBracketBalance_push_extend` (linearise side adds `flowBracketDelta tokens[j].val`) + `flowBracketBalance_compose` + `flowBracketBalance_single` (tokens side splits `[j, j_k] = [j, j+1] + [j+1, j_k]` with `[j, j+1]` = `flowBracketDelta tokens[j].val`).  **5-way case-split:** dispatches to splice_handler (cases 1, 3) or copy_handler (cases 2, 4) using the existing `linearise_go_step_splice`/`_step_copy`/`_step_token` lemmas; case 3 (tail-splice when `j ≥ tokens.size`) uses an inline `rw [linearise.go]; simp [hp, hsplice, hk]` to derive the step equation; case 5 (done) contradicts `h_lt` via `linearise_go_done`.  (b) `linearise_outer_flowEntry_decode_top` — top-level wrapper specialising at `(j, p, acc) = (0, 0, #[])` for direct use on `linearise tokens pks` instead of `linearise.go`.  **Lean-tactic notes:** `push_neg at h_in_ext` is unavailable (Mathlib tactic, not in scope) — use `Nat.le_of_not_lt` directly to convert `¬ k < (acc ++ expandKind e).size` to `(acc ++ expandKind e).size ≤ k`.  `Array.size_append` and `Array.size_push` are no-arg propositions (universe-polymorphic, all implicit args) — `Array.size_append acc _` triggers "Function expected at Array.size_append" because the term *is* the equation, not a function; use `by rw [Array.size_append]` instead.  The inline-`have`-handler approach lets the same logic be reused across cases 1+3 (splice variants) and cases 2+4 (copy variants) without code duplication, since each pair shares its step equation's shape; this halves the proof size compared to inline duplication.  **Sorry count unchanged at 9** (inversion lemma is reusable infrastructure consumed in γ-3b; the in-tree Part (3) sorry comment was updated to document 6c-ii-γ-3a's landing and the γ-3 → γ-3a/γ-3b decomposition).  Build green (453/453 jobs) | 0 (build clean across 453 jobs; ~270-line addition for the main lemma + top wrapper; no sorry change) | `Proofs/Output/EmitterScannability.lean`, `Blueprint/07-initiative-3-append-only.md` | ✓ done 2026-05-04 |
          ...
          - 6c-ii-γ. **inversion enumeration + Part (3) discharge** —
            ...
            *exhaustiveness* fact needed to invert `k → i`.  This is a
            ...
            - 6c-ii-γ-3a. ✓ **linearise outer-flowEntry inversion
              ...
              top-level wrapper `linearise_outer_flowEntry_decode_top`.
              Walks `linearise.go` step-by-step (induction on
              `(tokens.size - j) + (pks.size - p)`); splice case
              refutes via `expandKind_val_neutral` (splice tokens are
              `.key`/`.blockMappingStart`, never `.flowEntry`); copy
              case yields the matching token cursor `j_k`.  Provides
              the translation equation `flowBracketBalance lin
              acc.size k = flowBracketBalance tokens j j_k`,
              composing splice-step (zero balance via
              `flowBracketBalance_splice_unchanged`) and copy-step
              (`flowBracketDelta tokens[j].val` via
              `flowBracketBalance_push_extend`) contributions.  Sub-
              proof factored via two inline `have` handlers
              (`splice_handler` covers cases 1+3; `copy_handler`
              covers cases 2+4) reused across the case-split on
              linearise.go's dispatch.  Sorry count unchanged at 9.
              Build green (453/453 jobs).  See manifest entry
              `J.4.2.b-2d-key-chain-Part3-final-discharge-bridge-6c-ii-γ-3a`.
            - 6c-ii-γ-3b. **discharge Part (3) using γ-3a + γ-1**.
              **Decomposed (2026-05-04)**: the original plan called for
              walking from `(0, 0, #[])` to `(pks[qs[i]].insertBeforeIdx,
              qs[i], acc_witness)` and using `linearise_go_walk_size` to
              read off `acc_witness.size = k + 1`.  But the formula gives
              `acc_witness.size = pks[qs[i]].insertBeforeIdx + foldlSum 0
              qs[i] = (j_k + 1) + foldlSum 0 qs[i]`, which equals `k + 1`
              **only if** `k = j_k + foldlSum 0 qs[i]`.  This size relation
              does not follow from γ-3a's bracket-balance equation alone;
              it requires either (a) strengthening γ-3a to expose the
              walk-state-after-copy directly, or (b) chain-side reasoning
              about the absence of intervening `.keyOnly` /
              `.blockMappingStartAndKey` entries between `qs[i-1]` and
              `qs[i]` in `pks`.  Decomposition into three sub-cadences:
              - 6c-ii-γ-3b-i. ✓ **strengthen γ-3a inversion lemma with
                walk-state-after-copy** [done 2026-05-04]: extended
                `linearise_outer_flowEntry_decode` (and its `_top`
                wrapper) with a fourth conjunct exposing
                `∃ p_k acc', p_k ≤ pks.size ∧ linearise.go ... 0 0 #[] =
                linearise.go ... (j_k + 1) p_k acc' ∧ acc'.size = k + 1`
                — pin-pointing the walk state immediately after the copy
                step that pushed `tokens[j_k] = .flowEntry`.  Added a
                precondition `p ≤ pks.size` (preserved through splice/copy
                steps).  Splice/copy handlers updated to construct the
                new conjunct via composition with `h_step` and pass-through
                from IH; terminating copy case (`k = acc.size`) directly
                witnesses `(p_k, acc') = (p, acc.push tokens[j_k])` with
                size = `acc.size + 1 = k + 1`.  Sorry count unchanged at
                9.  Build green (453/453 jobs).  See manifest entry
                `J.4.2.b-2d-key-chain-Part3-final-discharge-bridge-6c-ii-γ-3b-i`.
              - 6c-ii-γ-3b-ii. **chain-side flow-context kind restriction**
                (further decomposed 2026-05-04).  Strengthen
                `EmitScansInFlow` / `EmitListScansInFlow` /
                `EmitPairListScansInFlow` with a conjunct asserting that,
                in flow context, every pendingKey entry pushed during the
                body has kind ∈ {`.unresolved`, `.keyOnly`} — never
                `.blockMappingStartAndKey`.  This is automatic from the
                scanner's structure (flow-context detection branches do
                not call the BMS+key emission path), but must be exposed
                as a chain invariant for γ-3b-iii to reach `.keyOnly`
                rather than `.blockMappingStart` at position `k + 1`.

                **Decomposition rationale (2026-05-04):** the original
                γ-3b-ii plan estimated ~150 lines spanning predicate def
                + ~6 leaf discharge sites.  In practice the discharge
                sites are larger than estimated: each `scanNextToken_flow_*`
                helper has an internal `h_content` lemma (~70 lines) with
                two-branch case split (`simpleKey.possible = true/false`
                after content scan), and the new-kind conjunct needs
                discharge in each branch.  The internal preservation
                chain `s → saveSimpleKey s → s_ad → s_dq → s_final` must
                be threaded through `setPendingKeyEndLine` (kind-preserving
                in the skp=true branch) on top of the saveSimpleKey push
                fact.  Realistic scope is ~250-400 lines.  Decomposed
                into four smaller cadences:

                  - 6c-ii-γ-3b-ii-foundation (DONE 2026-05-04).  Land the
                    reusable helper `saveSimpleKey_new_kind_unresolved`:
                    "any pendingKey entry pushed by `saveSimpleKey` has
                    kind `.unresolved`" (push branch writes `.unresolved`;
                    identity branches don't grow the array, vacuous).
                    Foundation for the four leaf-side discharge cadences.
                    ~40 lines; no sorry change; build green.  See manifest
                    entry `J.4.2.b-2d-key-chain-Part3-final-discharge-bridge-6c-ii-γ-3b-ii-foundation`.

                  - 6c-ii-γ-3b-ii-leaves-A (DONE 2026-05-03).  Added
                    new-kind conjunct
                    `∀ q (h_q : q < s'.pendingKeys.size) (h_q_ge : s.pendingKeys.size ≤ q), (s'.pendingKeys[q]'h_q).kind = .unresolved`
                    to `scanNextToken_flow_scanDoubleQuoted` (scalar leaf),
                    its `_pkPush` variant, and `scanNextToken_flow_open_nested`
                    (`[`).  Discharged in each leaf's internal `h_content`
                    proof using `saveSimpleKey_new_kind_unresolved`
                    (foundation helper) +
                    `ScannerCorrectness.scanDoubleQuoted_preserves_pendingKeys`
                    /
                    `ScannerCorrectness.scanFlowSequenceStart_preserves_pendingKeys`
                    +
                    `ScannerCorrectness.setPendingKeyEndLine_kind` (for the
                    `simpleKey.possible = true` branch wrap in
                    `scanNextToken_flow_scanDoubleQuoted`).  Threaded the
                    chain `s_dq.pendingKeys → (saveSimpleKey s).pendingKeys`
                    via `h_dq_pks.trans h_ad_pks` so the underlying push
                    fact `(saveSimpleKey s).pendingKeys[q].kind = .unresolved`
                    discharges the goal after `congr 1` bridges array
                    indices.  Two consumer destructure sites updated to
                    bind the new conjunct (`emit (.scalar s)` scalar case,
                    `emit (.sequence ...)` open-bracket call site).  Build
                    green 453/453 jobs; sorry count unchanged at 9 (per-leaf
                    strengthening only — no predicate or sorry site touched).
                    ~140 lines net.  See manifest entry
                    `J.4.2.b-2d-key-chain-Part3-final-discharge-bridge-6c-ii-γ-3b-ii-leaves-A`.

                  - 6c-ii-γ-3b-ii-leaves-B (DONE 2026-05-03).  Added
                    universal new-kind conjunct of shape `∀ (q : Nat)
                    (h_q : q < s'.pendingKeys.size)
                    (_h_q_ge : s.pendingKeys.size ≤ q),
                    (s'.pendingKeys[q]'h_q).kind = .unresolved`
                    to three close/comma leaf signatures:
                    (a) `scanNextToken_flow_close_seq_nested` (`]`),
                    (b) `scanNextToken_flow_close_mapping_nested` (`}`),
                    (c) `scanNextToken_flow_comma` (`,`).  In the close-side
                    leaves the conjunct sits between `pkRec preserved`
                    and the `simpleKey/pendingKeyActive/pendingKeyStack`
                    restore facts; in the comma it sits between
                    `pkRec preserved` and `pendingKeyStack = s.pendingKeyStack`.
                    Each discharge follows the leaves-A pattern: chain
                    the leaf's `_preserves_pendingKeys` (`scanFlowSequenceEnd`,
                    `scanFlowMappingEnd`, or `emit/advance` for comma) with
                    `s_ad.pendingKeys = (saveSimpleKey s).pendingKeys` to
                    obtain `s'.pendingKeys = (saveSimpleKey s).pendingKeys`,
                    bridge `s'.pendingKeys[q] = (saveSimpleKey s).pendingKeys[q]`
                    via `congr 1`, then dispatch via the foundation helper
                    `saveSimpleKey_new_kind_unresolved`.  4 consumer
                    destructure binders updated:
                    (a) line ~9166 in `emitList_scans_nonempty` cons
                    (binds `_h_newkind₂`),
                    (b) line ~10874 in `emitPairList_scans_nonempty` cons
                    (binds `_h_newkind_c`),
                    (c) line ~11785 in `emit_scans_in_flow` sequence case
                    (binds `_h_newkind₃`),
                    (d) line ~12064 in `emit_scans_in_flow` mapping case
                    (binds `_h_newkind₃`).  All four bindings are
                    placeholder underscores until γ-3b-ii-predicate consumes
                    them.  **Sorry count unchanged at 9.**  See
                    `J.4.2.b-2d-key-chain-Part3-final-discharge-bridge-6c-ii-γ-3b-ii-leaves-B`.

                  - 6c-ii-γ-3b-ii-leaves-C (DONE 2026-05-03).  Added
                    universal new-kind conjunct of shape `∀ (q : Nat)
                    (h_q : q < s'.pendingKeys.size)
                    (_h_q_ge : s.pendingKeys.size ≤ q),
                    (s'.pendingKeys[q]'h_q).kind = .unresolved` to two
                    vacuous-discharge leaf signatures:
                    (a) `scanNextToken_flow_value_pkResolve` (`:`),
                    (b) `scanNextToken_preprocess_flow_ws1` (single
                    leading-space preprocess).  Both are vacuous because
                    `s'.pendingKeys.size = s.pendingKeys.size`: at the
                    colon, `setPendingKeyKind` overwrites in place via
                    `Array.setIfInBounds` (size preserved, same range);
                    in ws1, `skipToContent_preserves_pendingKeys` makes
                    `s₁.pendingKeys = s.pendingKeys` directly.  No q
                    satisfies `s.pendingKeys.size ≤ q < s'.pendingKeys.size`,
                    so the discharge collapses to
                    `exact absurd h_q_ge (Nat.not_le_of_lt h_q)` after
                    rewriting `h_q` via `h_pks_full + Array.size_setIfInBounds`
                    (colon) or `skipToContent_preserves_pendingKeys` (ws1).
                    6 consumer destructure binders updated:
                    (a) line ~9265 in `emitList_scans_nonempty` cons
                    (binds `_h_newkind_pp₃` post-comma ws1),
                    (b) line ~10388 in `emitPairList_scans_nonempty` value
                    (binds `_h_newkind_pp₃` between key and value via ws1),
                    (c) line ~10484 in `emitPairList_scans_nonempty` value
                    (binds `_h_newkind_pkr` for value_pkResolve at `:`),
                    (d) line ~10868 in `emitPairList_scans_nonempty` cons
                    (binds `_h_newkind_pp₃` between cons key and value),
                    (e) line ~10961 in `emitPairList_scans_nonempty` cons
                    (binds `_h_newkind_pp` post-comma ws1 in cons),
                    (f) line ~11082 in `emitPairList_scans_nonempty` cons
                    (binds `_h_newkind_pkr` for cons-case `:` resolve).
                    All six bindings are placeholder underscores until
                    γ-3b-ii-predicate consumes them.  **Sorry count
                    unchanged at 9.**  See
                    `J.4.2.b-2d-key-chain-Part3-final-discharge-bridge-6c-ii-γ-3b-ii-leaves-C`.

                  - 6c-ii-γ-3b-ii-predicate (DONE 2026-05-03).  Extended
                    `EmitScansInFlow` / `EmitListScansInFlow` /
                    `EmitPairListScansInFlow` predicate definitions with
                    the disjunctive new-kind conjunct
                    `∀ q ∈ [s.pks.sz, s'.pks.sz), kind ∈ {.unresolved,
                    .keyOnly}` (positioned between pkRec preservation and
                    pendingKeyStack equality).  Discharge structure:
                    (a) `emitList_scans_empty` / `emitPairList_scans_empty`
                    via vacuous range collapse (size preserved, no q in
                    range; dispatched by `Nat.not_le_of_lt`); (b) added
                    foundational helper `newkind_disj_compose` (parallels
                    `pkRec_size_compose`, takes pkRec preservation
                    s_mid → s_end + disj newkind on each segment, returns
                    composed disj newkind on s → s_end via case-split on
                    `q < s_mid.pks.sz`); (c) `emit_scans_in_flow` scalar
                    case lifts leaf-A `_h_newkind'` to disj via `Or.inl`;
                    sequence + mapping cases compose leaf-A `[`/`{`
                    (Or.inl-lifted) → body (disj from inner predicate) →
                    leaf-B `]`/`}` (Or.inl-lifted) via two
                    `newkind_disj_compose` calls; (d) `emitList_scans_nonempty`
                    singleton threads inner `EmitScansInFlow` disj
                    directly; cons composes key emit (Or.inl) + comma
                    leaf-B (Or.inl) + ws1 leaf-C (vacuous, hand-built disj)
                    + recursive IH (disj) via `newkind_disj_compose`
                    triple chain; (e) `emitPairList_scans_nonempty`
                    singleton + cons via direct case-split discharge
                    (necessary because the colon transitions kind
                    `.unresolved → .keyOnly` at `s.pks.sz`, breaking the
                    pkRec preservation invariant required by
                    `newkind_disj_compose`; instead, we case on
                    `q < s₁.pks.sz` (pushed during key emit, lift through
                    colon transition by sub-case `q = s.pks.sz`
                    [→ `.keyOnly`] vs `q ≠ s.pks.sz` [pkRec preserved
                    via `h_pks_other_pk`]) vs `q ≥ s₁.pks.sz` (pushed
                    during value emit, use `_h_newkind_v` directly with
                    pkRec lift through comma + ws1 + IH).  Also extended
                    leaves-A coverage to `scanNextToken_flow_open_mapping_nested`
                    (signature was missing the new-kind conjunct, blocking
                    composition in the mapping case of `emit_scans_in_flow`).
                    12 consumer destructure binders updated across
                    `emitList_scans_nonempty` (cons: h_ev + h_ih_list),
                    `emitPairList_scans_nonempty` (singleton: h_ek_key +
                    h_ev; cons: h_ek_key + h_ev + h_ih_list),
                    `emit_scans_in_flow` (seq: open + body + close;
                    map: open + body + close), and
                    `emitPairList_body_filtered_characterization`.  Build
                    green 453/453 jobs; sorry count unchanged at 9.
                    ~250 lines net.  See manifest entry
                    `J.4.2.b-2d-key-chain-Part3-final-discharge-bridge-6c-ii-γ-3b-ii-predicate`.
              - 6c-ii-γ-3b-iii. **final discharge** (decomposed
                2026-05-03 after prerequisite investigation).  Original
                plan assumed walking `(j_k+1, p_k, acc') → (j_k+1, qs[i],
                acc'')` through `.unresolved` entries was straightforward
                given γ-3b-ii's kind restriction.  Investigation revealed
                two missing prerequisites that block the walk:

                (a) **γ-3a output gap**: γ-3b-i's strengthened decode
                returns `p_k ≤ pks.size` but no fact tying `p_k` to `j_k`.
                We need either (i) `pks[r].insertBeforeIdx ≤ j_k` for
                `r < p_k` (intrinsic-walk invariant) to derive `qs[i] ≥
                p_k` from γ-1's `pks[qs[i]].insertBeforeIdx = j_k + 1`,
                or (ii) `qs[i] = p_k` directly.

                (b) **save-time monotonicity gap**: linearise.go fires
                splices in pks-index order, advancing `j` only when the
                next splice can't fire.  For the walk through `[p_k,
                qs[i])` to fire all intermediate splices at cursor
                `j_k+1` (so `acc.size` transitions through k+1 to k+2 at
                qs[i]'s `.keyOnly` push), every `r ∈ [p_k, qs[i])` must
                have `insertBeforeIdx ≤ j_k+1`.  Without save-time mono
                (`pks[r].insertBeforeIdx ≤ pks[qs[i]].insertBeforeIdx`
                for `r ≤ qs[i]`), an out-of-order entry could block the
                splice loop (forcing a token copy of `tokens[j_k+1] =
                .scalar`, NOT `.key`).  Save-time mono is a chain-side
                fact (`saveSimpleKey` records `insertBeforeIdx =
                tokens.size` which grows monotonically; captured by
                `LineariseFit` in `ScannerCorrectness.lean`) but is not
                currently threaded through any of the three flow-context
                predicates.

                Decomposed into 4 cadence sub-steps:

                - **γ-3b-iii-A** (DONE 2026-05-03, ~70 lines, no
                  sorry change): strengthened `linearise_outer_flowEntry_decode`
                  and `_top` with the new conjunct `∀ r ∈ [p, p_k)
                  (with r < pks.size), pks[r].insertBeforeIdx ≤ j_k`
                  (intrinsic to linearise.go's execution: splices that
                  fired during the walk to `(j_k+1, p_k, acc')` had
                  `insertBeforeIdx ≤ cursor at fire time ≤ j_k`).
                  Added precondition `h_idx_le_tok : ∀ r < pks.size,
                  pks[r].insertBeforeIdx ≤ tokens.size` (rules out the
                  tail-splice case where `insertBeforeIdx` could
                  exceed `j_k`).  Discharge in the induction:
                  `splice_handler` takes new arguments
                  `h_e_idx_le_j : e.insertBeforeIdx ≤ j` (case 1:
                  direct from `hsplice`; case 3 derived from
                  `h_idx_le_tok` + `j ≥ tokens.size`) and `h_e_eq`
                  identifying `e` with `pks[p]` (so the `r = p`
                  sub-case rewrites back to `pks[r]` form);
                  `r ∈ [p+1, p_k)` delegates to IH.  `copy_handler`
                  propagates IH unchanged.  Terminating copy step
                  (k = acc.size) discharges vacuously (p_k = p, range
                  empty).  Originally also planned `∀ (h : p_k <
                  pks.size), j_k < pks[p_k].insertBeforeIdx` but
                  γ-3b-iii-D's discharge plan only consumes the
                  weaker `∀ r < p_k, pks[r].insertBeforeIdx ≤ j_k`
                  conjunct, so the second conjunct was deferred.  Build
                  green 453/453 jobs.  Sorry count unchanged at 9.
                  See manifest entry `J.4.2.b-2d-key-chain-Part3-final-discharge-bridge-6c-ii-γ-3b-iii-A`.

                - **γ-3b-iii-B (DONE 2026-05-04, ~150 lines)**: added
                  conditional save-time monotonicity + well-indexed
                  bound conjunct to `EmitScansInFlow` /
                  `EmitListScansInFlow` / `EmitPairListScansInFlow`.
                  **Conditional form** (decided after weighing the
                  alternative of adding `LineariseFit s` as a hard
                  precondition, which would have required ~250–400
                  lines of plumbing through every call site): the
                  conjunct fires under `LineariseFit s ∧ AllKeysValid
                  s` supplied by the consumer, exposing `(∀ p ≤ q,
                  pks[p].insertBeforeIdx ≤ pks[q].insertBeforeIdx) ∧
                  (∀ r, pks[r].insertBeforeIdx ≤ s'.tokens.size)` on
                  `s'.pendingKeys`.  Discharge per producer collapses
                  to a single application of the new chain helper
                  `ScanChainGrew.preserves_LineariseFit_AllKeysValid`
                  (which threads `LineariseFit s ∧ AllKeysValid s` →
                  `LineariseFit s' ∧ AllKeysValid s'` via
                  `scanNextToken_preserves_LineariseFit` +
                  `_preserves_AllKeysValid` per step) followed by
                  `mono_and_bound_of_LineariseFit` (a small helper
                  that projects the two facts from `LineariseFit`
                  with strict-to-non-strict weakening on the sorted
                  conjunct).  Conditional form means existing call
                  sites are **untouched** — only consumers needing the
                  new facts (γ-3b-iii-D, future) provide the
                  `LineariseFit s ∧ AllKeysValid s` invariant.  Build
                  green (453/453 jobs); sorry count unchanged at 9.
                  See manifest entry `J.4.2.b-2d-key-chain-Part3-final-discharge-bridge-6c-ii-γ-3b-iii-B`.

                - **γ-3b-iii-C (DONE 2026-05-04, ~100 lines)**:
                  added `linearise_walk_through_unresolved_keyOnly`
                  lemma to `Proofs/Scanner/ScannerLinearise.lean`.
                  Given a transport equation at `(j, p, acc)` with
                  `acc.size = k`, plus a `.keyOnly` target at index
                  `q ≥ p`, plus save-time-mono splice-readiness
                  `pks[r].insertBeforeIdx ≤ j` for `r ∈ [p, q]`, plus
                  γ-3b-ii kind disjunction `pks[r].kind ∈ {.unresolved,
                  .keyOnly}` for `r ∈ [p, q]`, produces
                  `linearise[k].val = .key`.  Induction on `q - p'`
                  walks through `.unresolved` fires (no change to
                  `acc`) until either `p' = q` (apply Foundation B
                  index form at `pks[q]`) or `p' < q` with
                  `pks[p']` already `.keyOnly` (Foundation B at
                  `pks[p']` directly — first-`.keyOnly`-suffices
                  shortcut).  Fully isolated to `Proofs/Scanner/`; no
                  call site plumbing.  Build green (453/453 jobs);
                  sorry count unchanged at 9.  See manifest entry
                  `J.4.2.b-2d-key-chain-Part3-final-discharge-bridge-6c-ii-γ-3b-iii-C`.

                - **γ-3b-iii-D (DONE 2026-05-04, ~210 lines, sorry 9 → 8)**:
                  final Part (3) discharge in
                  `emitPairList_body_linearise_characterization`.
                  Composes the four prerequisites in pipeline form:
                  γ-3b-iii-A's strengthened decode supplies `(j_k, p_k,
                  acc')` with `acc'.size = k+1`, the walk equation, and
                  the idx-inv conjunct; γ-1's exhaustiveness inverts
                  the outer-flowEntry to a pair index `i ≥ 1` with
                  `j_k + 1 = pks[qs[i]].insertBeforeIdx`; γ-3b-iii-B's
                  conditional save-time-mono yields the bound
                  precondition for γ-3b-iii-A and the
                  `pks[r].insertBeforeIdx ≤ pks[qs[i]].insertBeforeIdx
                  = j_k + 1` chain for `r ≤ qs[i]`; γ-3b-iii-C's walk
                  lemma reads off `linearise[k+1] = .key` from
                  `(j_k+1, p_k, acc')`.  Added two preconditions
                  (`LineariseFit s` + `AllKeysValid s`) to the wrapper
                  to drive γ-3b-iii-B (no current consumers).  Helper
                  `linearise_balance_prefix_eq` (~50 lines) added to
                  `EmitterScannability.lean` to translate
                  `flowBracketBalance lin old_sz k = 0` to
                  `flowBracketBalance s'.tokens s.tokens.size j_k = 0`
                  via `linearise_go_walk_flowBracketBalance_top` (no
                  splice fires before s.tokens.size).  Strengthened
                  `emitPairList_body_filtered_characterization` to
                  expose γ-3b-ii new-kind disjunction + γ-3b-iii-B
                  conditional save-time-mono conjuncts (used by the
                  linearise wrapper).  Case split on `p_k = 0` (j_k =
                  k via `linearise_go_walk_size`) vs `p_k > 0` (idx-inv
                  at r = 0) discharges `s.tokens.size ≤ j_k`.  Build
                  green (453/453 jobs); sorry count: 9 → 8.  See
                  manifest entry
                  `J.4.2.b-2d-key-chain-Part3-final-discharge-bridge-6c-ii-γ-3b-iii-D`.
...
1. **2d-key-chain-Part3** (after-flowEntry splice locator; refined
   ...
   done 2026-05-03 with emitPairList cons stub,
   final-discharge-bridge-6c-ii-γ-3a/linearise-outer-flowEntry-inversion-lemma
   done 2026-05-04, final-discharge-bridge-6c-ii-γ-3b-i/inversion-lemma-walk-state-strengthening
   done 2026-05-04, final-discharge-bridge-6c-ii-γ-3b-ii-leaves-A
   done 2026-05-03, final-discharge-bridge-6c-ii-γ-3b-ii-leaves-B
   done 2026-05-03, final-discharge-bridge-6c-ii-γ-3b-ii-leaves-C
   done 2026-05-03, final-discharge-bridge-6c-ii-γ-3b-ii-predicate
   done 2026-05-03,
   final-discharge-bridge-6c-ii-γ-3b-iii-A/decode-strengthening
   done 2026-05-03,
   final-discharge-bridge-6c-ii-γ-3b-iii-B/save-time-mono-predicate-strengthening
   done 2026-05-04,
