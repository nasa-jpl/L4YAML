              ...
              - 6c-ii-γ-3b-ii. **chain-side flow-context kind restriction**
                ...
                **Decomposition rationale (2026-05-04):** the original
                ...
                into four smaller cadences:
                  ...
                  - 6c-ii-γ-3b-ii-predicate (PENDING).  Add the new-kind
                    conjunct to `EmitScansInFlow` / `EmitListScansInFlow`
                    / `EmitPairListScansInFlow` predicate definitions.
                    Discharge in `*_empty` (vacuous) + `emit_scans_in_flow`
                    (scalar/seq/map cases) + `emitList_scans_nonempty`
                    (singleton/cons) + `emitPairList_scans_nonempty`
                    (singleton/cons) by composing per-leaf conjuncts via
                    pkRec preservation.  Chain reasoning: for `q ∈
                    [s.pks.sz, s'.pks.sz)`, case-split on which leaf-step
                    pushed `q` (using `s_mid.pks.sz` thresholds) — the
                    pushing leaf gives kind `.unresolved`, subsequent steps
                    preserve kind via pkRec (or transition `.unresolved →
                    .keyOnly` at colon for the active entry, both ∈
                    {.unresolved, .keyOnly}).  ~120 lines; no sorry change.
...
1. **2d-key-chain-Part3** (after-flowEntry splice locator; refined
   ...
   done 2026-05-04) — Part3 is now decomposed into 18 cadence-sized
   sub-steps (sub-steps 1–5 + 6a + 6b + 6c-i + 6c-ii-α + 6c-ii-β +
   6c-ii-γ-1 + 6c-ii-γ-2a + 6c-ii-γ-2b + 6c-ii-γ-2c + 6c-ii-γ-2d +
   6c-ii-γ-3a + 6c-ii-γ-3b-i ✓ done; 6c-ii-γ-2d-ii (emitPairList
   cons stub discharge) + 6c-ii-γ-3b-ii (chain-side flow-kind
   restriction) + 6c-ii-γ-3b-iii (final Part (3) discharge using
   γ-3b-i + γ-3b-ii) remaining):
