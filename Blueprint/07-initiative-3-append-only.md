            ...
            - 6c-ii-γ-3b. **discharge Part (3) using γ-3a + γ-1**.
              ...
              **only if** `k = j_k + foldlSum 0 qs[i]`.  This size relation
              ...
              - 6c-ii-γ-3b-ii. **chain-side flow-context kind restriction**
                (PENDING).  Strengthen `EmitScansInFlow` /
                `EmitListScansInFlow` / `EmitPairListScansInFlow` with a
                conjunct asserting that, in flow context, every pendingKey
                entry pushed during the body has kind ∈ {`.unresolved`,
                `.keyOnly`} — never `.blockMappingStartAndKey`.  This is
                automatic from the scanner's structure (flow-context
                detection branches do not call the BMS+key emission path),
                but must be exposed as a chain invariant for γ-3b-iii to
                reach `.keyOnly` rather than `.blockMappingStart` at
                position `k + 1`.  Discharge in each leaf
                (scalar/seq/map/comma/value-resolve/`{`/`}`) by direct
                inspection.  ~150 lines spanning predicate def + ~6
                discharge sites; no sorry change.
