            ...
            - 6c-ii-γ-3b. **discharge Part (3) using γ-3a + γ-1**.
              ...
              **only if** `k = j_k + foldlSum 0 qs[i]`.  This size relation
              ...
              - 6c-ii-γ-3b-iii. **final discharge** (decomposed
                ...
                - **γ-3b-iii-C** (~80 lines, no sorry change): add
                  `linearise_walk_through_unresolved_keyOnly` lemma to
                  `Proofs/Scanner/ScannerLinearise.lean`.  Given a
                  transport equation at `(j, p, acc)` with `acc.size =
                  k`, plus a `.keyOnly` barrier at index `q ≥ p` with
                  `insertBeforeIdx ≤ j`, plus save-time mono on `[p,
                  q+1)`, plus γ-3b-ii kind disjunction on `[p, q]`,
                  produces `linearise[k] = .key` (induction on `q -
                  p`; .unresolved fires advance `p` without changing
                  `acc`, .keyOnly fires apply Foundation B).
...
1. **2d-key-chain-Part3** (after-flowEntry splice locator; refined
   ...
   done 2026-05-04) — Part3 is now decomposed into 22 cadence-sized
   ...
   6c-ii-γ-3b-iii-{C,D} (walk-through-unresolved-keyOnly lemma,
   final Part (3) discharge — sorry 9 → 8) remaining):
