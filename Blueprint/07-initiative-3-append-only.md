     ...
     3. **2d-key-chain-Part2-body-C (compose + dispatch in
        ...
        * **C-foundation-EmitScansInFlow** — scope investigation
          ...
          2. **C-foundation-EmitScansInFlow-defns** (~2.5 steps,
             ...
             Decomposition:
             - **defns-helpers-size** (~1 step): extend the 6
               scanner helpers (`scanNextToken_flow_scanDoubleQuoted`,
               ...
               conjunct; the discharge re-uses each helper's
               existing inner pendingKeys-equation chain plus a new
               foundational lemma `saveSimpleKey_pendingKeys_size_ge`
               (push-or-id ⇒ size grows or preserves).
               `_preprocess_flow_ws1` already exposes
               `s'.pendingKeys = s.pendingKeys`, so trivially.
             - **defns-prove** (~1 step): add size-mono + pkRec
               conjuncts to the three definitions; re-prove
               `emit{List,PairList}_scans_{empty,nonempty}` and
               `emit_scans_in_flow` (scalar/seq/map) using the
               extended helper conjuncts.  Update ~10 caller
               destructure patterns to bind the new conjuncts.
             - **defns-consumers** (~0.5 step): update downstream
               consumers in `emitList_body_filtered_characterization`
               and `emitPairList_body_filtered_characterization`
               (and any other top-level destructures of the three
               EmitScansInFlow definitions) to bind the new
               conjuncts.  Mechanical.
          3. **C-foundation-EmitScansInFlow-stack-restore** (~1
             step): add stack-restore lemmas
             `scanFlowSequenceEnd_simpleKey_eq` /
             `scanFlowSequenceEnd_pendingKeyActive_eq` /
             `scanFlowMappingEnd_…` exposing
             `s'.simpleKey = s.simpleKeyStack.back?.getD {}` and
             `s'.pendingKeyActive = s.pendingKeyStack.back?.getD none`
             (the existing scanner ops do this in their bodies; just
             needs lemma extraction).  Plus extend
             `scanNextToken_flow_close_seq_nested` /
             `scanNextToken_flow_close_mapping_nested` to expose
             these.  Plus add `pendingKeyStack` preservation
             conjunct to `EmitListScansInFlow` /
             `EmitPairListScansInFlow` (mirrors the existing
             `simpleKeyStack` preservation; same proof structure
             since neither stack is touched by body operations,
             only by nested `[`/`]`/`{`/`}` push/pop pairs that
             cancel).
          4. **C-foundation-EmitScansInFlow-gated** (~1 step): add
             ...
             stack restore lemmas to recover the gated facts at the
             post-CLOSE state.
          The original "~1 step" estimate underestimated the
          helper-level preservation work plus the stack-restore
          machinery.  Once all 4 sub-steps land, **C-compose** is
