   ...
   - **2d-key-chain-Part2-body (proof body for the first-key stub)**:
     ...
     3. **2d-key-chain-Part2-body-C (compose + dispatch in
        ...
        * **C-foundation-EmitScansInFlow** — scope investigation
          ...
          The original "~1 step" estimate (now refined to 5
          sub-steps in body-C foundation: defns-helpers-size +
          defns-prove + stack-restore + gated + gated-discharge,
          with 2 remaining follow-on discharges)
          underestimated the helper-level preservation work plus
          the stack-restore machinery.  With **gated-discharge**
          landed (sorry count 11), **discharge-colon** is the
          next blocker for **C-compose** (~0.5–1 step,
          mechanical once colon-step pkRec preservation is
          sorry-free).  **preservation-chain** is independent
          (orthogonal cleanup of `scanDoubleQuoted_preserves_pendingKeyStack_sorry`).
