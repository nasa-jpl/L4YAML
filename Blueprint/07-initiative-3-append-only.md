        ...
        - 6c-ii. **Part3-final-discharge-bridge-inversion** — decomposed
          ...
          - 6c-ii-α. ✓ **bracket-balance algebra helpers** [done
            ...
            * `flowBracketBalance_splice_unchanged` — appending
            ...
            consumed in 6c-ii-β).
          - 6c-ii-β. **bracket-balance preservation lemma** ← *current
            next step*: prove the parallel induction
            `linearise_go_walk_flowBracketBalance` over `linearise.go`'s
            lex-measure: for any walk transport `(j, p, acc) → (j', p',
            acc')`, `flowBracketBalance acc' acc.size acc'.size =
            flowBracketBalance tokens j j'`.  Cases: (n=0) both sides
            trivially zero; (splice step) `flowBracketBalance_splice_
            unchanged` keeps the splice piece at zero, IH absorbs the
            rest; (copy step) `flowBracketBalance_push_extend` picks up
            `flowBracketDelta tokens[j].val`, matched by `flowBracket
            Balance_compose` against `flowBracketBalance tokens j (j+1)
            + flowBracketBalance tokens (j+1) j'`.  Likely needs an
            additional helper `flowBracketBalance_append_left` (balance
            over a range fully in `xs` is unchanged by `xs ++ ys`).
            Sorry count unchanged.
          - 6c-ii-γ. **inversion enumeration + Part (3) discharge**:
            given an arbitrary outer-level `.flowEntry` at linearise
            position `k`, identify the unique pair index `i ≥ 1` such
            that `k + 1 = pks[qs[i]].insertBeforeIdx + (pks.foldl 0 0
            qs[i])`.  Consumes 6c-ii-β to translate the outer-level
            condition (balance = 0 on linearise) to balance = 0 on
            `s'.tokens`, then uses chain-side `qs` enumeration (from
            `_h_first_qs`) to pin down `i`.  Discharges the Part (3)
            sorry at `emitPairList_body_linearise_characterization`
            (line ~12550).  Sorry count: 8 → 7.
...
1. **2d-key-chain-Part3** (after-flowEntry splice locator; refined
   ~10–11 cadence steps; scope-investigation done 2026-05-02,
   ...
   2026-05-02) — Part3 is now decomposed into 10 cadence-sized
   sub-steps (sub-steps 1–5 + 6a + 6b + 6c-i + 6c-ii-α ✓ done; 6c-ii-β
   and 6c-ii-γ remaining):
