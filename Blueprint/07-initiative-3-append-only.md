              ...
              - 6c-ii-γ-3b-ii. **chain-side flow-context kind restriction**
                ...
                **Decomposition rationale (2026-05-04):** the original
                ...
                into four smaller cadences:
                  ...
                  - 6c-ii-γ-3b-ii-leaves-A (PENDING).  Add new-kind conjunct
                    to `scanNextToken_flow_scanDoubleQuoted` (scalar leaf)
                    + its `_pkPush` variant + `scanNextToken_flow_open_nested`
                    (`[`/`{`).  Discharge in each leaf's internal
                    `h_content` proof using the foundation helper +
                    `scanDoubleQuoted_preserves_pendingKeys` /
                    `scanFlowSequenceStart_preserves_pendingKeys` +
                    `setPendingKeyEndLine_kind`.  ~120 lines; no sorry change.
