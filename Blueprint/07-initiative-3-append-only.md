          ...
          - 6c-ii-γ. **inversion enumeration + Part (3) discharge** —
            ...
            *exhaustiveness* fact needed to invert `k → i`.  This is a
            ...
            - 6c-ii-γ-3b. **discharge Part (3) using γ-3a + γ-1**:
              apply `linearise_outer_flowEntry_decode_top` to extract
              `j_k` with `s'.tokens[j_k] = .flowEntry`; relate
              balance via the translation equation; apply the γ-1
              chain-side exhaustiveness conjunct (in `_h_first_qs`)
              to identify pair index `i ≥ 1` with `j_k + 1 =
              pks[qs[i]].insertBeforeIdx`; walk to the matching
              `(pks[qs[i]].insertBeforeIdx, qs[i], acc_witness)`
              state via `linearise_go_walk_eq_top` and verify
              `acc_witness.size = k + 1` via
              `linearise_go_walk_size`; conclude via
              `linearise_splice_keyonly_at_index`.  Sorry count:
              9 → 8.
...
1. **2d-key-chain-Part3** (after-flowEntry splice locator; refined
   ...
   done 2026-05-04) — Part3 is now decomposed into 17 cadence-sized
   sub-steps (sub-steps 1–5 + 6a + 6b + 6c-i + 6c-ii-α + 6c-ii-β +
   6c-ii-γ-1 + 6c-ii-γ-2a + 6c-ii-γ-2b + 6c-ii-γ-2c + 6c-ii-γ-2d +
   6c-ii-γ-3a ✓ done; 6c-ii-γ-2d-ii (emitPairList cons stub
   discharge) + 6c-ii-γ-3b (Part (3) discharge using γ-3a)
   remaining):
