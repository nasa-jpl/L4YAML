     ...
     6. **Part3-final-discharge** (split into 6a + 6b — bridge sub-cadence
        ...
        - 6c-ii. **Part3-final-discharge-bridge-inversion** — decomposed
          ...
          - 6c-ii-γ. **inversion enumeration + Part (3) discharge** ←
            *current next step*: given an arbitrary outer-level
            `.flowEntry` at linearise position `k`, identify the unique
            pair index `i ≥ 1` such that `k + 1 =
            pks[qs[i]].insertBeforeIdx + (pks.foldl 0 0 qs[i])`.
            Consumes 6c-ii-β to translate the outer-level condition
            (balance = 0 on linearise) to balance = 0 on `s'.tokens`,
            then uses chain-side `qs` enumeration (from `_h_first_qs`)
            to pin down `i`.  Discharges the Part (3) sorry at
            `emitPairList_body_linearise_characterization` (line
            ~12550).  Sorry count: 8 → 7.
...
1. **2d-key-chain-Part3** (after-flowEntry splice locator; refined
   ...
   done 2026-05-02) — Part3 is now decomposed into 10 cadence-sized
   sub-steps (sub-steps 1–5 + 6a + 6b + 6c-i + 6c-ii-α + 6c-ii-β ✓
   done; 6c-ii-γ remaining):
