            ...
            - 6c-ii-γ-3b. **discharge Part (3) using γ-3a + γ-1**.
              ...
              **only if** `k = j_k + foldlSum 0 qs[i]`.  This size relation
              ...
              - 6c-ii-γ-3b-iii. **final discharge** (decomposed
                ...
                - **γ-3b-iii-D** (~80 lines, sorry 9 → 8): final Part
                  (3) discharge in `emitPairList_body_linearise_characterization`.
                  Apply γ-3b-iii-A's strengthened decode to obtain
                  `(j_k, p_k, acc')` + the new conjuncts.  Translate
                  balance hypothesis from linearise to s'.tokens.
                  Apply γ-1 exhaustiveness for pair index `i ≥ 1` with
                  `j_k + 1 = pks[qs[i]].insertBeforeIdx`.  Use
                  γ-3b-iii-A's `pks[r].insertBeforeIdx ≤ j_k` for `r <
                  p_k` to derive `qs[i] ≥ p_k`.  Apply γ-3b-iii-C's
                  walk lemma at `(j_k+1, p_k, acc', k+1)` with `q =
                  qs[i]` to read off `linearise[k+1] = .key`.  Sorry
                  count: 9 → 8.
...
1. **2d-key-chain-Part3** (after-flowEntry splice locator; refined
   ...
   done 2026-05-04) — Part3 is now decomposed into 23 cadence-sized
   ...
   6c-ii-γ-3b-iii-B + 6c-ii-γ-3b-iii-C
   ✓ done; 6c-ii-γ-2d-ii (emitPairList cons stub discharge) +
   6c-ii-γ-3b-iii-D (final Part (3) discharge — sorry 9 → 8)
   remaining):
