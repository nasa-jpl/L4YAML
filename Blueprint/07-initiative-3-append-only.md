              ...
              - 6c-ii-γ-3b-ii. **chain-side flow-context kind restriction**
                ...
                **Decomposition rationale (2026-05-04):** the original
                ...
                into four smaller cadences:
                  ...
                  - 6c-ii-γ-3b-ii-leaves-C (PENDING).  Update
                    `scanNextToken_flow_value_pkResolve` (`:`).  Since
                    `s'.pendingKeys.size = s.pendingKeys.size` already
                    holds (no push at colon), the new-kind conjunct is
                    vacuous — just expose it.  Also handle
                    `scanNextToken_preprocess_flow_ws1` (which has
                    `s₁.pendingKeys = s.pendingKeys`, vacuous).  ~30 lines;
                    no sorry change.
