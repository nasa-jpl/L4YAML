...
**Next session**: Step 6d.1e.12b — add the ~60 leaf-scanner
helpers (`_preserves_simpleKey`, `_preserves_simpleKeyStack`,
`_clears_simpleKey`, `_simpleKey_cleared` / `_stack_pushed` for
flow start, `_simpleKey_restored` / `_stack_popped` for flow end,
`_adds_tokens` / `_preserves_prefix` where missing) for the ~13
indexed leaf scanners. Most are 1–5 LOC (`rfl` or
`unfold; split <;> rfl`); the harder ones are `scanValueIx` (clear
via composition) and `unwindIndentsLoopIx_preserves_simpleKey`
(induction on fuel). Budget: ~600–800 LOC, 1–2 sessions.
...
| **6d.1e.12b** | **Per-scanner facts for `AllKeysPlaceholderInvIx` chain** — Add ~60 missing Ix-suffixed leaf-scanner helpers needed by 12c's dispatcher composition. Per the 2026-05-21 audit: each of ~13 indexed leaf scanners (`scanDocumentStartIx`, `scanDocumentEndIx`, `scanDirectiveIx`, `scanFlowSequenceStartIx`/`EndIx`, `scanFlowMappingStartIx`/`EndIx`, `scanFlowEntryIx`, `scanBlockEntryIx`, `scanKeyIx`, `scanValueIx`, `scanAnchorOrAliasIx`, `scanTagIx`, dispatchContent's block-scalar/DQ/SQ/plain arms) + meta-helpers (`skipToContentS`, `unwindIndentsIx`) needs the appropriate combination of `_preserves_simpleKey` / `_preserves_simpleKeyStack` / `_clears_simpleKey` (block-context clearing scanners) / `_simpleKey_cleared` + `_stack_pushed` (flow start) / `_simpleKey_restored` + `_stack_popped` (flow end) / `_adds_tokens` / `_preserves_prefix`. Most are 1–5 LOC (`rfl` or `unfold; split <;> rfl`); harder ones are `scanValueIx` (cleared via `scanValueClearKeyIx + scanValuePrepareIx` composition) and `unwindIndentsLoopIx_preserves_simpleKey` (induction on fuel). **DONE criteria**: all per-scanner helpers landed sorry-free; `lake build` green; axiom count unchanged at 2. | `Proofs/Production/IndexedScannerPlainScalarValid.lean` (~600–800 LOC of new helpers) | ~600–800 | 1–2 |
| **6d.1e.12c** | **Dispatcher composition for `AllKeysPlaceholderInvIx`** — Mirror legacy `Proofs/Production/ScannerPlainScalarValid.lean:4430–4958`: `saveSimpleKeyIx_preserves_AllKeysPlaceholderInvIx` (~50 LOC), `scanNextTokenIx_preprocess_preserves_AllKeysPlaceholderInvIx` (~50 LOC), `scanNextTokenIx_dispatchStructural_preserves_AllKeysPlaceholderInvIx` (~80 LOC), `flowStart_preserves_AllKeysPlaceholderInvIx` + `flowEnd_preserves_AllKeysPlaceholderInvIx` (~120 LOC combined), `scanNextTokenIx_dispatchFlowIndicators_preserves_AllKeysPlaceholderInvIx` (~60 LOC), `scanNextTokenIx_dispatchBlockIndicators_preserves_AllKeysPlaceholderInvIx` (~75 LOC — includes the `scanValueIx` arm using `SimpleKeyTokenDisjointIx` to bound the overwrite range), `scanNextTokenIx_dispatchContent_preserves_AllKeysPlaceholderInvIx` (~100 LOC). The `scanValueIx` arm requires an indexed twin of legacy `scanValue_preserves_prefix` (which uses `SimpleKeyTokenDisjoint` to show stacked-key positions are outside the overwrite range). **DONE criteria**: all dispatcher composition theorems landed sorry-free; `lake build` green; axiom count unchanged at 2. | `Proofs/Production/IndexedScannerPlainScalarValid.lean` (~400 LOC) | ~400 | 1 |
| **6d.1e.12d** | **Discharge the 2 staging axioms + refactor consumers** — Promote `scanNextTokenIx_preprocess_preserves_SimpleKeyPlaceholderInvIx` and `scanNextTokenIx_preserves_SimpleKeyPlaceholderInvIx` to theorems by deriving them from 12c's `_preserves_AllKeysPlaceholderInvIx` lemmas (projecting `.1` of the 4-tuple). Refactor 6 callers — `scanLoopIx_preserves_FlowContextPSVIx` / `_FlowNestingInvIx`, `scan_flow_aware_psv_ix_axiom`, `scan_flow_brackets_matched_ix_axiom`, plus the `scanNextTokenIx_preserves_*` chain — to thread `AllKeysPlaceholderInvIx` instead of just `SimpleKeyPlaceholderInvIx`. Initial state satisfies `AllKeysPlaceholderInvIx` via `mk'_AllKeysPlaceholderInvIx` (already landed in 12a) + emit-preservation of the 4 conjuncts (needs `emit_preserves_AllKeysPlaceholderInvIx` helper, ~20 LOC). **DONE criteria**: both staging axioms promoted to theorems; `lake build` green; **Phase 3 closure has 0 remaining axioms**, ready for Step 6f cutover. | `Proofs/Production/IndexedScannerPlainScalarValid.lean` (axiom → theorem promotion + consumer refactor) | ~150 | 1 |
...
- **12b** *(planned, ~600–800 LOC, 1–2 sessions)* — Add the
  per-scanner facts needed by 12c. The audit (2026-05-21) shows
  ~60 absent Ix-suffixed helpers vs. the legacy chain:
  `<scanner>_preserves_simpleKey`, `_preserves_simpleKeyStack`,
  `_clears_simpleKey`, `_simpleKey_cleared` / `_stack_pushed` (flow
  start), `_simpleKey_restored` / `_stack_popped` (flow end),
  `_adds_tokens` / `_preserves_prefix` (where missing) for
  ~13 leaf scanners (`scanDocumentStartIx`, `scanDocumentEndIx`,
  `scanDirectiveIx`, `scanFlowSequenceStartIx`/`EndIx`,
  `scanFlowMappingStartIx`/`EndIx`, `scanFlowEntryIx`,
  `scanBlockEntryIx`, `scanKeyIx`, `scanValueIx`,
  `scanAnchorOrAliasIx`, `scanTagIx`, dispatchContent's
  block-scalar/DQ/SQ/plain arms) + meta-helpers (`skipToContentS`,
  `unwindIndentsIx`). Most are 1–5 LOC (`rfl` or
  `unfold; split <;> rfl`). The harder ones are `scanValueIx`
  (clear via `scanValueClearKeyIx + scanValuePrepareIx` composition)
  and the loop-based `unwindIndentsLoopIx_preserves_simpleKey`
  (induction on fuel).

- **12c** *(planned, ~400 LOC, 1 session)* — Port the dispatcher
  chain from legacy lines 4430–4958: mirror
  `saveSimpleKey_preserves_AllKeysPlaceholderInv` (~50 LOC),
  `preprocess_preserves_AllKeysPlaceholderInv` (~50 LOC),
  `dispatchStructural_preserves_AllKeysPlaceholderInv` (~80 LOC),
  `flowStart_preserves_AllKeysPlaceholderInv` (~55 LOC),
  `flowEnd_preserves_AllKeysPlaceholderInv` (~65 LOC),
  `dispatchFlowIndicators_preserves_AllKeysPlaceholderInv` (~60
  LOC), `dispatchBlockIndicators_preserves_AllKeysPlaceholderInv`
  (~75 LOC), `dispatchContent_preserves_AllKeysPlaceholderInv`
  (~100 LOC) as Ix variants. The `dispatchBlockIndicators` arm
  for `scanValue` requires the indexed-twin of the legacy
  `scanValue_preserves_prefix` (which uses
  `SimpleKeyTokenDisjoint` to bound the overwrite range).

- **12d** *(planned, ~150 LOC, 1 session)* — Discharge the 2
  staging axioms as theorems using 12c's composed lemmas.
  Refactor consumers (`scanLoopIx_preserves_FlowContextPSVIx`,
  `_FlowNestingInvIx`, `scan_flow_aware_psv_ix_axiom`,
  `scan_flow_brackets_matched_ix_axiom`) to thread
  `AllKeysPlaceholderInvIx` instead of just
  `SimpleKeyPlaceholderInvIx`; consumers needing only the current
  key use `.1` of the 4-tuple. The initial-state `mk' ▶ emit
  .streamStart` satisfies `AllKeysPlaceholderInvIx` via
  `mk'_AllKeysPlaceholderInvIx` + emit-preservation of the 4
  conjuncts.

**Total revised budget**: ~1,400–1,600 LOC across 4 sub-sessions
(12a landed; 12b ~1–2 sessions; 12c + 12d ~2 sessions). The
~6× expansion over the original estimate stems from the original
plan misclassifying the flow-end arms as "vacuous" and from the
~60 missing leaf-scanner helpers that the indexed side never
landed.

**DONE criteria** (12d): both staging axioms promoted to theorems;
`lake build` green at 385/385; **Phase 3 closure has 0 remaining
