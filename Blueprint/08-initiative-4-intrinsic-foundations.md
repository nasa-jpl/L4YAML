...
**Next session**: Step 6d.1e.12d — discharge the 2
`scanNextTokenIx_*_preserves_SimpleKeyPlaceholderInvIx`
staging axioms as theorems by projecting `.1` of 12c.2's
`_preserves_AllKeysPlaceholderInvIx` lemmas. Refactor 6
consumers to thread `AllKeysPlaceholderInvIx` (or project
`.1` when only the current-key invariant is needed). Initial
state via `mk'_AllKeysPlaceholderInvIx` + emit-preservation
of the 4 conjuncts (new helper, ~20 LOC). **DONE criteria**:
both staging axioms promoted to theorems; lake build green at
385/385; **Phase 3 closure has 0 remaining axioms**, ready
for Step 6f cutover.
...
**Total revised budget**: ~2,400–2,700 LOC across 5
sub-sessions (12a landed; 12b landed; 12c-scout landed; 12c.1
landed; 12c.2 landed; 12d ~1 session remaining). The ~9× expansion
over the original estimate stems from the original plan
misclassifying the flow-end arms as "vacuous" (Reflection 89),
the ~60 missing leaf-scanner helpers (Reflection 90), the
record-update / dependent-bracket motive wall surfaced by
12c-scout (Reflection 91), and the `apply`-ordering pitfall
surfaced by 12c.2 (Reflection 93). After 12c.2's landing, the
8 dispatcher composition theorems are in place and ready for
12d to project `SimpleKeyPlaceholderInvIx` preservation out
as `.1` of the 4-tuple.

**Next session — Step 6d.1e.12d**: discharge the 2 staging
axioms — `scanNextTokenIx_preprocess_preserves_SimpleKeyPlaceholderInvIx`
and `scanNextTokenIx_preserves_SimpleKeyPlaceholderInvIx` —
as theorems by projecting `.1` of 12c.2's
`_preserves_AllKeysPlaceholderInvIx` lemmas. Refactor 6
consumers (`scanLoopIx_preserves_FlowContextPSVIx`,
`_FlowNestingInvIx`, `scan_flow_aware_psv_ix_axiom`,
`scan_flow_brackets_matched_ix_axiom`, plus the
`scanNextTokenIx_preserves_*` chain) to thread
`AllKeysPlaceholderInvIx` (or to project `.1` when only the
current-key invariant is needed). Initial state via
`mk'_AllKeysPlaceholderInvIx` (already landed) + emit-preservation
of the 4 conjuncts (new helper, ~20 LOC).
