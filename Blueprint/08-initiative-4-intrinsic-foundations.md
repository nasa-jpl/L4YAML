...
**Next session**: Step 6d.1e.12 — discharge the 2 new staging
axioms from 6d.1e.10
(`scanNextTokenIx_preprocess_preserves_SimpleKeyPlaceholderInvIx`
and `scanNextTokenIx_preserves_SimpleKeyPlaceholderInvIx`). After
Step 6d.1e.11d's landing (2026-05-21), the 3 §11h dispatcher
sorries are discharged as real theorems and **axiom count is
unchanged at 2** (both remaining axioms are the
SimpleKeyPlaceholderInvIx-preservation targets above). The chain
mirrors the legacy `scanNextToken_preserves_AllKeysPlaceholderInv`
(~250 LOC across 13 leaf scanners + 5 dispatchers); each leaf
either clears `simpleKey.possible := false` (vacuous) or leaves
`simpleKey` untouched while pushing tokens (mono via the
`emit_preserves_SimpleKeyPlaceholderInvIx` recipe already proven in
6d.1e.10). Realistic budget for 12: ~250 LOC.
