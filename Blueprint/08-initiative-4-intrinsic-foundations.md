...
**Next session**: Step 6d.1e.12c.1 — substrate fix for the
per-scanner `_preserves_prefix` family (~400 LOC). Model each
proof on the legacy
`unwindIndentsLoopIx_preserves_prefix` shape — both bound
proofs explicit, the conclusion uses `Array.getElem_push_lt`
/ `Array.getElem_setIfInBounds` directly on the unfolded
`.tokens.tokens.push _` form. Targets: 11 leaf scanners
(`scanDocumentEndIx`, `scanYamlDirectiveIx`,
`scanTagDirectiveIx`, `scanDirectiveIx`, `scanBlockEntryIx`,
`scanKeyIx`, `scanValueClearKeyIx`, `scanValuePrepareIx`,
`scanValueIx`, `scanFlowEntryIx`, `skipToContentS`) plus the
5 already-bridged via `_tokens_eq`. The bounded
`scanValuePrepareIx` / `scanValueIx` / `scanFlowEntryIx`
prefix lemmas need an `n ≤ simpleKey.tokenIndex` precondition
mirroring legacy `scanValue_preserves_prefix`.
...
| **6d.1e.12c.1** | **Per-scanner `_preserves_prefix` substrate fix** — Write 11 per-scanner prefix-preservation Ix lemmas in legacy `unwindIndentsLoopIx_preserves_prefix` shape (both bound proofs explicit, no intermediate `_tokens_eq` `rw` — Reflection 91 substrate fix). Targets: `scanDocumentStartIx_preserves_prefix`, `scanDocumentEndIx_preserves_prefix`, `scanYamlDirectiveIx_preserves_prefix`, `scanTagDirectiveIx_preserves_prefix`, `scanDirectiveIx_preserves_prefix`, `scanBlockEntryIx_preserves_prefix`, `scanKeyIx_preserves_prefix`, `scanValueClearKeyIx_preserves_prefix`, `scanValuePrepareIx_preserves_prefix` (bounded by `n ≤ simpleKey.tokenIndex`), `scanValueIx_preserves_prefix` (bounded), `scanFlowSequenceStartIx_preserves_prefix`, `scanFlowSequenceEndIx_preserves_prefix`, `scanFlowMappingStartIx_preserves_prefix`, `scanFlowMappingEndIx_preserves_prefix`, `scanFlowEntryIx_preserves_prefix` (bounded), `skipToContentS_preserves_prefix` (trivial). The 5 `_tokens_eq` rfl-bridges landed in 12c-scout compose into these proofs (one direction of the bridge is fine; the dependent-bracket `rw` motive is the broken direction). **DONE criteria**: all 11 per-scanner prefix lemmas landed sorry-free; `lake build` green; axiom count unchanged at 2. | `Proofs/Production/IndexedScannerPlainScalarValid.lean` (~400 LOC) | ~400 | 1 |
| **6d.1e.12c.2** | **Dispatcher composition for `AllKeysPlaceholderInvIx`** — Mirror legacy `Proofs/Production/ScannerPlainScalarValid.lean:4430–4958`: `saveSimpleKeyIx_preserves_AllKeysPlaceholderInvIx` (~50 LOC), `scanNextTokenIx_preprocess_preserves_AllKeysPlaceholderInvIx` (~50 LOC), `scanNextTokenIx_dispatchStructural_preserves_AllKeysPlaceholderInvIx` (~80 LOC), `flowStart_preserves_AllKeysPlaceholderInvIx` + `flowEnd_preserves_AllKeysPlaceholderInvIx` (~120 LOC combined), `scanNextTokenIx_dispatchFlowIndicators_preserves_AllKeysPlaceholderInvIx` (~60 LOC), `scanNextTokenIx_dispatchBlockIndicators_preserves_AllKeysPlaceholderInvIx` (~75 LOC — includes the `scanValueIx` arm using `SimpleKeyTokenDisjointIx` + 12c.1's bounded `scanValueIx_preserves_prefix` to bound the overwrite range), `scanNextTokenIx_dispatchContent_preserves_AllKeysPlaceholderInvIx` (~100 LOC). Depends on 12c.1's per-scanner prefix lemmas. **DONE criteria**: all 8 dispatcher composition theorems landed sorry-free; `lake build` green; axiom count unchanged at 2. | `Proofs/Production/IndexedScannerPlainScalarValid.lean` (~400 LOC) | ~400 | 1 |
| **6d.1e.12d** | **Discharge the 2 staging axioms + refactor consumers** — Promote `scanNextTokenIx_preprocess_preserves_SimpleKeyPlaceholderInvIx` and `scanNextTokenIx_preserves_SimpleKeyPlaceholderInvIx` to theorems by deriving them from 12c.2's `_preserves_AllKeysPlaceholderInvIx` lemmas (projecting `.1` of the 4-tuple). Refactor 6 callers — `scanLoopIx_preserves_FlowContextPSVIx` / `_FlowNestingInvIx`, `scan_flow_aware_psv_ix_axiom`, `scan_flow_brackets_matched_ix_axiom`, plus the `scanNextTokenIx_preserves_*` chain — to thread `AllKeysPlaceholderInvIx` instead of just `SimpleKeyPlaceholderInvIx`. Initial state satisfies `AllKeysPlaceholderInvIx` via `mk'_AllKeysPlaceholderInvIx` (already landed in 12a) + emit-preservation of the 4 conjuncts (needs `emit_preserves_AllKeysPlaceholderInvIx` helper, ~20 LOC). **DONE criteria**: both staging axioms promoted to theorems; `lake build` green; **Phase 3 closure has 0 remaining axioms**, ready for Step 6f cutover. | `Proofs/Production/IndexedScannerPlainScalarValid.lean` (axiom → theorem promotion + consumer refactor) | ~150 | 1 |
...
- **12c.1** *(planned, ~400 LOC, 1 session)* — Substrate fix
  for `_preserves_prefix` family. Introduce a non-dependent
  `tokens_eq_below` form `s'.tokens.tokens[i]'h_lhs = s.tokens.tokens[i]'h_rhs`
  (carrying both bounds explicitly) so that `rw` motives don't
  capture the LHS bound, OR introduce per-scanner
  `_preserves_tokens_at` helpers in the legacy
  `emit_preserves_tokens_at` shape (already verified to work
  for `emit` / `emitAt` / `unwindIndentsLoopIx` /
  `pushSequenceIndentIx` / `pushMappingIndentIx` /
  `saveSimpleKeyIx` / `scanAnchorOrAliasIx` / `scanTagIx` in
  §5/§6/§7). Targets:
  `scanDocumentStartIx`, `scanDocumentEndIx`,
  `scanYamlDirectiveIx`, `scanTagDirectiveIx`,
  `scanDirectiveIx`, `scanBlockEntryIx`, `scanKeyIx`,
  `scanValueClearKeyIx`, `scanValuePrepareIx` (bounded),
  `scanValueIx` (bounded), `scanFlowSequenceStartIx`,
  `scanFlowSequenceEndIx`, `scanFlowMappingStartIx`,
  `scanFlowMappingEndIx`, `scanFlowEntryIx` (bounded),
  `skipToContentS` (trivial). The 5 §12f rfl-bridges already
  established by 12c-scout compose into the leaf-scanner
  prefix proofs.

- **12c.2** *(planned, ~400 LOC, 1 session)* — Port the
  dispatcher chain from legacy lines 4430–4958: mirror
  `saveSimpleKey_preserves_AllKeysPlaceholderInv` (~50 LOC),
  `preprocess_preserves_AllKeysPlaceholderInv` (~50 LOC),
  `dispatchStructural_preserves_AllKeysPlaceholderInv` (~80
  LOC), `flowStart_preserves_AllKeysPlaceholderInv` (~55 LOC),
  ...
  (~75 LOC) — the `scanValue` arm uses
  `SimpleKeyTokenDisjointIx` to bound the overwrite range —
  `dispatchContent_preserves_AllKeysPlaceholderInv` (~100 LOC)
  as Ix variants. Depends on 12c.1's per-scanner prefix
  infrastructure.

- **12d** *(planned, ~150 LOC, 1 session)* — Discharge the 2
  staging axioms as theorems using 12c.2's composed lemmas.
  ...
  `SimpleKeyPlaceholderInvIx`; consumers needing only the
  current key use `.1` of the 4-tuple. The initial-state `mk'
  ▶ emit .streamStart` satisfies `AllKeysPlaceholderInvIx` via
  `mk'_AllKeysPlaceholderInvIx` + emit-preservation of the 4
  conjuncts.

**Total revised budget**: ~1,800–2,100 LOC across 5
sub-sessions (12a landed; 12b landed; 12c-scout landed; 12c.1
+ 12c.2 + 12d ~3 sessions). The ~7× expansion over the
original estimate stems from the original plan misclassifying
the flow-end arms as "vacuous" (Reflection 89), the ~60
missing leaf-scanner helpers (Reflection 90), and the
record-update / dependent-bracket motive wall surfaced by
12c-scout (Reflection 91).
