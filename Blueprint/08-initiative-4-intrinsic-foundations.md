...
**Next session**: **Step 6d.2** — `Proofs/Parser/IndexedWfa.lean`
(~1,692 LOC, mechanical once 6d.1c's WB mutual block is sorry-free).
Re-proves `WellFormedAnchors`/`Scannable`/`AllAliasesResolve`
preservation through `parseNode`. Consumes three WellBehaved lemmas
directly (`parseNode_wb_all`, `parseNodeContent_wb`,
`parseNodeProperties_tokens`). Then 6d.3 (Correctness + Completeness
+ Grammable) closes out 6d, then 6e (IndexedComposition), then 6f
(atomic cutover, ~−30,000 LOC delete).

**Previous next-session pointer**: Step 6d.1e.12d — discharge the 2
`scanNextTokenIx_*_preserves_SimpleKeyPlaceholderInvIx` staging
axioms as theorems by projecting `.1` of 12c.2's
`_preserves_AllKeysPlaceholderInvIx` lemmas (now landed).
