...
content as first-class sections rather than linking out to a
sidecar site.

*4a. Artifact split (L4YAML.FGM side).*
...
currently downloads the single tarball into
`doc/Doc/L4YAML/graphs/` and Verso's `theorem-graphs` section in
[`Verification.lean`](../../doc/Doc/L4YAML/Verification.lean)
merely *describes the tool*. Update both:

1. Workflow switches to `theorem-graphs-headlines.tar.gz`.
2. The `theorem-graphs` Verso section is rewritten. For each
   headline it renders an inline block with:
   - the theorem's plain-English summary (from the catalogue
     `description`),
   - the ChainDepth tag (`deep` / `propBridge` / `weak`) with the
     same honesty labelling the DOT uses,
   - the chain SVG embedded via `<object>` (scoped to the
     headlines tarball, ~20 kB per headline),
   - links out to the headline's module page and the full
     catalogue release asset.

The Verso page stops being a procedure-manual for the tool and
becomes the "What does L4YAML prove?" front-door for a non-expert
reader.

Acceptance: headlines tarball fits in a small size budget (target
2 MB uncompressed, vs. today's ~30 MB); a reader can see all 7
...
**7. Full Verso integration (deferred polish)**

Step 4b embeds the headlines tarball into Verso but keeps the
full catalogue as a tarball-served sidecar (the `index.html` /
`modules.html` / `group/*.html` / `module/*/*.html` files
generated by `theoremgraph`). Step 7 promotes the sidecar into
native Verso pages:

- A Verso page per blueprint group (Group 1 Pipeline, Group 2
  Scanner, …) listing every catalogue entry in that group with
  its bipartite + chain graphs embedded.
- A Verso page per Lean module with key theorems, same content as
  today's `module/<module.path>.html`.
- A Verso-native full catalogue table replacing `all.html`.
- Cross-links so a reader browsing by module can jump to the
  blueprint group and back.

This is bigger than step 4b: it requires teaching the Verso genre
about the `@[key_theorem]` catalogue (or generating Verso source
from it), and it duplicates information that L4YAML.FGM already
renders statically. Deferred because the sidecar model works and
