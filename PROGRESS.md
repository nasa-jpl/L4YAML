> **Historical log — frozen 2026-03-19.** For current status see
> [README.md](README.md) and `docs/reports/stats.json` (regenerated by CI's
> `collect-stats`); the proof-status SSOT is
> [Blueprint/04-capstones.md](Blueprint/04-capstones.md). The library has been
> sorry-free since 2026-07-04; the theorem, guard, and build-job counts below
> are snapshots of their dates. Paths below predate the 2026-04 folder
> reorganization, and non-capstone `theorem` declarations were renamed to
> `lemma` on 2026-07-31.
...
Three-phase plan (D → B → A) (see [SPEC-GAP-ANALYSIS.md](./SPEC-GAP-ANALYSIS.md) for details)
