"""Python FFI integration tests for liblean4yaml.
...
try:
    import lean4yaml
    from lean4yaml.types import YamlValue, YamlDocument

    _LIB_AVAILABLE = True
except (OSError, ImportError):
    lean4yaml = None  # type: ignore[assignment]
...
needs_lib = pytest.mark.skipif(
    not _LIB_AVAILABLE,
    reason="liblean4yaml.so or lean4yaml package not available",
)
...
class TestLimitPresets:
    ...
    def test_preset_parses(self, preset: str) -> None:
        v = lean4yaml.load("hello", limits=preset)
        assert v.as_str() == "hello"
...
class TestErrorHandling:
    ...
    def test_error_message_nonempty(self) -> None:
        with pytest.raises(lean4yaml.ParseError) as exc_info:
            lean4yaml.load("[unclosed")
...
class TestValueEquality:
    ...
    def test_equal_scalars(self) -> None:
        v1 = lean4yaml.load("hello")
        v2 = lean4yaml.load("hello")
        assert v1 == v2

    def test_unequal_scalars(self) -> None:
        v1 = lean4yaml.load("hello")
        v2 = lean4yaml.load("world")
        assert v1 != v2

    def test_equal_mappings(self) -> None:
        v1 = lean4yaml.load("a: 1\nb: 2")
        v2 = lean4yaml.load("a: 1\nb: 2")
        assert v1 == v2
...
class TestMemorySafety:
    ...
    def test_many_parses(self) -> None:
        ...
        for i in range(50):
            v = lean4yaml.load(f"key{i}: value{i}")
