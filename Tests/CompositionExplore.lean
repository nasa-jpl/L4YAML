...
/-!
# Composition Exploration  (Step 5.4.5)

Exploration file for composition theorems — proving intermediate lemmas
that chain the per-parser specs, fuel sufficiency, and parseYaml bridge
into top-level completeness theorems.

Key discoveries will be migrated to `Proofs/Composition.lean`.
-/

namespace Lean4Yaml.Proofs.CompositionExplore
...
theorem next_setPosition_id (s s' : YamlStream) (c : Char)
    ...
    Parser.Stream.setPosition s' (Parser.Stream.getPosition s) = s := by
  -- Reuse existing anyToken roundtrip: anyToken s = .ok s' c → setPosition s' pos = s
...
theorem skipBOM_noop (s : YamlStream)
  ...
  | some p =>
    ...
    -- Goal: .ok (setPosition s' (getPosition s)) PUnit.unit = .ok s ()
    -- Need: setPosition s' (getPosition s) = s
    -- Use anyToken_setPosition_roundtrip with the anyToken form
    have h_at : (anyToken (m := Id) : YamlParser Char) s = .ok s' c := by
...
/-- `endOfInput` succeeds (returning `.unit`) when `Stream.next?` is `none`. -/
...
/-!
`Parser.test p` always succeeds: returns `true` when `p` succeeds,
`false` (with position restored) when `p` fails.

For the generic case, the proof requires fully reducing the `optionM`
chain including `Id` monad operations. We specialize to `endOfInput`
below, where the step-by-step `have` chain avoids the reduction issue
by unfolding `optionM` and relying on beta-iota reduction (`rfl`) on
concrete `Sum.inl`/`Sum.inr` values.
-/

/-- Helper: the inner parser `endOfInput *> return true` succeeds on EOF. -/
...
theorem test_endOfInput_eof (s : YamlStream)
  ...
  -- Step 1: eoption wraps success as .inl
  ...
  -- Step 2: optionM chains through .inl → return true
...
end Lean4Yaml.Proofs.CompositionExplore
