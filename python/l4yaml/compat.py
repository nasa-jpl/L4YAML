...
Usage::
    ...
    # After:
    from l4yaml.compat import safe_load, safe_load_all

    data = safe_load("key: [1, true, null]")
    # → {"key": [1, True, None]}

Parser limits and dump configuration are specified as YAML strings
or file paths, matching l4yaml's configuration-as-YAML model::
...
from l4yaml.exceptions import ConfigError, L4YAMLError
...
def _load_configured_single(input_str: str, config_yaml: str) -> YamlValue:
    ...
    result: int = lib.l4yaml_parse_configured(
    ...
    if not ok:
        ...
        from l4yaml.exceptions import LimitError, ParseError
        if "limit" in msg.lower() or "exceeded" in msg.lower():
            raise LimitError(msg)
        raise ParseError(msg)
    # l4yaml_parse_configured returns a multi-doc result;
    ...
    dh: int = lib.l4yaml_docs_get(docs_h, 0)
    val_h: int = lib.l4yaml_doc_root(dh)
    lib.l4yaml_free(dh)
    lib.l4yaml_free(docs_h)
    lib.l4yaml_free(result)
...
def safe_load(
    ...
    *,
    ...
    Raises:
        l4yaml.ParseError: On syntax/grammar errors.
        l4yaml.LimitError: When a security limit is exceeded.
        l4yaml.ConfigError: If *limits* YAML is invalid.
    """
