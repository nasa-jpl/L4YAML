...
Strategy: dump a value using the style-aware `dump` function, parse
it back with `parseYamlSingle`, and verify `contentEq` holds.

Unlike the canonical emitter round-trip (RoundTrip.lean §4), the dump
function produces block-style YAML with plain scalars, testing the
