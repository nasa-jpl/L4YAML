...
private def canonicalRoundTrips (n : ValidNode) : Bool :=
  let v := toYamlValue n
...
/-- Verify `stripAnnotations` equality through the pipeline.
    Only succeeds for canonical nodes (DQ scalars + flow collections). -/
...
/-- Verify that `parseYamlRaw (emit (toYamlValue n))` produces exactly
    one document. -/
...
/-! ## §5: Cross-Style Content Preservation

Verify that the emitter preserves content even when the original
`ValidNode` uses a different style (plain, single-quoted, block).
The emitter converts everything to DQ + flow, but content is preserved.

These use `canonicalRoundTrips` which checks `contentEq`, not
`stripAnnotations ==`, since the style will differ.
-/
