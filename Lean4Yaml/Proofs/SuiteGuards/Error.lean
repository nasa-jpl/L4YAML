...
-- 5LLU:0 [PASS] Block scalar with wrong indented line after spaces only
...
-- 5U3A:0 [PASS] Sequence on same Line as Mapping Key
...
-- 6S55:0 Invalid scalar at the end of sequence (§9.2 bare document rejection)
#guard match parseYaml "key:\n - bar\n - baz\n invalid\n" with
  | .ok _ => false
  | .error _ => true
...
-- 7MNF:0 Missing colon (§9.2 bare document rejection)
#guard match parseYaml "top1:\n  key1: val1\ntop2\n" with
  | .ok _ => false
  | .error _ => true

-- 8XDJ:0 Comment in plain multiline value (§9.2 bare document rejection)
#guard match parseYaml "key: word1\n#  xxx\n  word2\n" with
  | .ok _ => false
  | .error _ => true

-- 9C9N:0 [PASS] Wrong indented flow sequence
#guard match parseYaml "---\nflow: [a,\nb,\nc]\n" with
  | .ok _ => false
  | .error _ => true

-- 9CWY:0 Invalid scalar at the end of mapping (§9.2 bare document rejection)
#guard match parseYaml "key:\n - item1\n - item2\ninvalid\n" with
  | .ok _ => false
  | .error _ => true
...
-- 9KBC:0 [PASS] Mapping starting at --- line
...
-- BD7L:0 [PASS] Invalid mapping after sequence
#guard match parseYaml "- item1\n- item2\ninvalid: x\n" with
  | .ok _ => false
  | .error _ => true

-- BF9H:0 Trailing comment in multiline plain scalar (§9.2 bare document rejection)
#guard match parseYaml "---\nplain: a\n       b # end of scalar\n       c\n" with
  | .ok _ => false
  | .error _ => true

-- BS4K:0 Comment between plain scalar lines (§9.2 bare document rejection)
...
-- CML9:0 Missing comma in flow (§7.3.3 #-after-fold terminates plain scalar)
#guard match parseYaml "key: [ word1\n#  xxx\n  word2 ]\n" with
  | .ok _ => false
  | .error _ => true
...
-- CXX2:0 [PASS] Mapping with anchor on document start line
...
-- G9HC:0 [PASS] Invalid anchor in zero indented sequence
#guard match parseYaml "---\nseq:\n&anchor\n- a\n- b\n" with
  | .ok _ => false
  | .error _ => true

-- GDY7:0 Comment that looks like a mapping key (§9.2 bare document rejection)
#guard match parseYaml "key: value\nthis is #not a: key\n" with
  | .ok _ => false
  | .error _ => true

-- GT5M:0 Node anchor in sequence (§9.2 bare document rejection)
#guard match parseYaml "- item1\n&node\n- item2\n" with
  | .ok _ => false
  | .error _ => true

-- H7J7:0 [PASS] Node anchor not indented
...
-- KS4U:0 Invalid item after end of flow sequence (§9.2 bare document rejection)
#guard match parseYaml "---\n[\nsequence item\n]\ninvalid item\n" with
  | .ok _ => false
  | .error _ => true
...
-- QLJ7:0 [PASS] Tag shorthand used in documents but only defined in the first
...
-- SR86:0 Anchor plus Alias (alias unconsumed → §9.2 bare document rejection)
...
-- SU74:0 Anchor and alias as mapping key (alias unconsumed → §9.2 bare document rejection)
#guard match parseYaml "key1: &alias value1\n&b *alias : value2\n" with
  | .ok _ => false
  | .error _ => true

-- SY6V:0 [PASS] Anchor before sequence entry on same line
...
-- U44R:0 Bad indentation in mapping (2) (§9.2 bare document rejection)
#guard match parseYaml "map:\n  key1: \"quoted1\"\n   key2: \"bad indentation\"\n" with
  | .ok _ => false
  | .error _ => true
...
-- VJP3:0 [PASS] Flow collections over many lines
#guard match parseYaml "k: {\nk\n:\nv\n}\n" with
  | .ok _ => false
  | .error _ => true

-- W9L4:0 [PASS] Literal block scalar with more spaces in first line
...
-- ZCZ6:0 [PASS] Invalid mapping in plain single line value
#guard match parseYaml "a: b: c: d\n" with
  | .ok _ => false
  | .error _ => true

-- ZL4Z:0 [PASS] Invalid nested mapping
#guard match parseYaml "---\na: 'b': c\n" with
  | .ok _ => false
  | .error _ => true
